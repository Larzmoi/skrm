...
exports.isActiveLiveLot = isActiveLiveLot;
...
async function finalizeLiveAuctionSale(sellerId, productId, price, winnerId) {
...
function isActiveLiveLot(showId, productId) {
...
const moderators = new Map(); // showId -> moderaattorien userId:t
...
async function getMutedWords(showId) {
    const cached = mutedWords.get(showId);
    if (cached)
        return cached;
    const show = await prisma_1.prisma.show.findUnique({ where: { id: showId }, select: { mutedWords: true } });
    const words = show?.mutedWords ?? [];
    mutedWords.set(showId, words);
    return words;
}
function containsMutedWord(words, message) {
    if (!words || words.length === 0)
        return false;
    const lower = message.toLowerCase();
    return words.some(w => w && lower.includes(w));
}
function broadcastViewerCount(io, showId) {
...
function broadcastViewerList(io, showId, sellerId) {
...
function setupSocket(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('join_show', async (payload) => {
            ...
            const show = await prisma_1.prisma.show.findUnique({ where: { id: showId }, select: { sellerId: true } });
            if (userId && removedFromShow.get(showId)?.has(userId)) {
                socket.emit('join_rejected', { reason: 'removed' });
                return;
            }
            socket.join(`show:${showId}`);
            if (!socketShows.has(socket.id))
                socketShows.set(socket.id, new Set());
            socketShows.get(socket.id).add(showId);
            if (userId && username) {
                if (!viewerIdentities.has(showId))
                    viewerIdentities.set(showId, new Map());
        ...
        socket.on('leave_show', (showId) => {
            ...
            broadcastViewerCount(io, showId);
            prisma_1.prisma.show.findUnique({ where: { id: showId }, select: { sellerId: true } }).then(show => {
                broadcastViewerList(io, showId, show?.sellerId);
            }).catch(() => { });
        });
        // Chat-viesti
        socket.on('chat_message', async ({ showId, message, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                if (mutedUsers.get(showId)?.has(decoded.userId))
                    return;
        ...
        socket.on('delete_chat_message', async ({ showId, messageId, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
        ...
        socket.on('mute_user', async ({ showId, userId, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
                if (!show || !isSellerOrMod(showId, show.sellerId, decoded.userId))
                    return;
                if (!mutedUsers.has(showId))
                    mutedUsers.set(showId, new Set());
        ...
        socket.on('assign_moderator', async ({ showId, userId, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
                if (!show || show.sellerId !== decoded.userId)
                    return;
                if (!moderators.has(showId))
                    moderators.set(showId, new Set());
        ...
        socket.on('remove_moderator', async ({ showId, userId, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
                if (!show || show.sellerId !== decoded.userId)
        ...
        socket.on('remove_from_show', async ({ showId, userId, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
                if (!show || !isSellerOrMod(showId, show.sellerId, decoded.userId))
                    return;
                if (!removedFromShow.has(showId))
                    removedFromShow.set(showId, new Set());
        ...
        socket.on('set_muted_words', async ({ showId, words, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
                if (!show || show.sellerId !== decoded.userId)
                ...
                await prisma_1.prisma.show.update({ where: { id: showId }, data: { mutedWords: cleaned } });
                socket.emit('muted_words_saved', { words: cleaned });
            }
            catch { }
        });
        // Myyjä pidentää käynnissä olevan huudon aikaa (+10s pikatoiminto)
        socket.on('extend_timer', async ({ showId, seconds, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
                if (!show || show.sellerId !== decoded.userId)
        ...
        socket.on('start_auction', async ({ showId, productId, startPrice, duration, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
                if (!show || show.sellerId !== decoded.userId)
                ...
                if (product?.currentBid != null && product.currentBidderId && product.currentBid >= initialBid) {
                    ...
                    const leader = await prisma_1.prisma.user.findUnique({ where: { id: product.currentBidderId }, select: { username: true } });
        ...
        socket.on('place_bid', async (payload) => {
            ...
            try {
                ...
                const [product, user] = await Promise.all([
                    prisma_1.prisma.product.findUnique({ where: { id: productId }, select: { sellerId: true } }),
                    prisma_1.prisma.user.findUnique({ where: { id: decoded.userId }, select: { id: true, username: true } }),
                ]);
                if (!product || !user)
                    return;
                ...
                prisma_1.prisma.bid.create({ data: { productId, showId, userId: user.id, amount } })
        ...
        socket.on('stop_auction', async ({ showId, token }) => {
            try {
                const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                const show = await prisma_1.prisma.show.findUnique({ where: { id: showId } });
                if (!show || show.sellerId !== decoded.userId)
