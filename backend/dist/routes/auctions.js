...
router.post('/:id/watch', auth_1.authMiddleware, async (req, res) => {
    ...
    if (existing) {
        await prisma_1.prisma.productWatch.delete({ where: { id: existing.id } });
    }
    else {
        await prisma_1.prisma.productWatch.create({ data: { productId, userId: req.userId } });
    }
    const watchCount = await prisma_1.prisma.productWatch.count({ where: { productId } });
