...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    ...
    const presets = await prisma_1.prisma.productPreset.findMany({
        where: {
            sellerId: req.userId,
            ...(search ? { OR: [
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    ...
    const preset = await prisma_1.prisma.productPreset.create({
        data: {
            name: name.trim(),
...
router.post('/bulk', auth_1.authMiddleware, async (req, res) => {
    ...
    if (typeof text !== 'string' || !text.trim())
        return res.status(400).json({ error: 'Tekstiä ei annettu' });
    const blocks = text.split(/\n\s*\n/).map((b) => b.split('\n').map(l => l.trim()).filter(l => l.length > 0)).filter((b) => b.length > 0);
    if (blocks.length === 0)
        return res.status(400).json({ error: 'Ei tunnistettuja pohjia' });
    if (blocks.length > 2000)
        return res.status(400).json({ error: 'Kerralla voi tuoda enintään 2000 pohjaa' });
...
router.put('/:id', auth_1.authMiddleware, async (req, res) => {
    ...
    const preset = await prisma_1.prisma.productPreset.findUnique({ where: { id } });
    if (!preset || preset.sellerId !== req.userId)
        return res.status(403).json({ error: 'Ei oikeutta' });
    const updated = await prisma_1.prisma.productPreset.update({
        where: { id },
        data: {
...
router.post('/:id/use', auth_1.authMiddleware, async (req, res) => {
    const id = String(req.params.id);
    const preset = await prisma_1.prisma.productPreset.findUnique({ where: { id } });
    if (!preset || preset.sellerId !== req.userId)
        return res.status(403).json({ error: 'Ei oikeutta' });
    const updated = await prisma_1.prisma.productPreset.update({ where: { id }, data: { lastUsedAt: new Date() } });
    res.json(updated);
});
// DELETE /presets/:id
router.delete('/:id', auth_1.authMiddleware, async (req, res) => {
    ...
    if (!preset || preset.sellerId !== req.userId)
        return res.status(403).json({ error: 'Ei oikeutta' });
    await prisma_1.prisma.productPreset.delete({ where: { id } });
    res.json({ ok: true });
});
