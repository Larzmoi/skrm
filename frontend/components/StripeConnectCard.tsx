...
// aloituksen jos seller.stripeAccountId puuttuu, ks. backend/src/routes/orders.ts). Jaettu
// komponentti — käytetään sekä /dashboard/tilitykset (alkuperäinen paikka) että
// /dashboard/profiili:ssa (lisätty 2026-09-09). Käännetty t.stripeCard-nimiavaruudella
// 2026-09-10 (ks. CLAUDE.md käännösauditointi) - oli aiemmin kokonaan hardkoodattua suomea.
export default function StripeConnectCard() {
  const { C } = useTheme()
  const { t } = useLang()
  const s = t.stripeCard
  ...
  async function connect() {
    ...
    try {
      ...
      setError(e.message ?? s.errConnect)
      setBusy(false)
    }
  }
  ...
  async function openDashboard() {
    ...
    try {
      ...
      setError(e.message ?? s.errDashboard)
    }
    setBusy(false)
  }
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        ...
        {status.transfersEnabled ? (
          <button onClick={openDashboard} disabled={busy} style={{ background: C.surface2, color: C.text, border: `1px solid ${C.border}`, padding: '10px 18px', borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: busy ? 'default' : 'pointer', opacity: busy ? 0.7 : 1, whiteSpace: 'nowrap' }}>
            {busy ? s.btnOpening : s.btnOpenDashboard}
          </button>
        ) : (
          <button onClick={connect} disabled={busy} style={{ background: C.accentSolid, color: C.accentText, border: 'none', padding: '10px 18px', borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: busy ? 'default' : 'pointer', opacity: busy ? 0.7 : 1, whiteSpace: 'nowrap' }}>
            {busy ? s.btnRedirecting : status.connected ? s.btnContinue : s.btnConnect}
          </button>
        )}
      </div>
