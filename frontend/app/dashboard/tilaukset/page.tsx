...
function addressLine(b: SellingOrder['buyer'], noAddress: string) {
  return [b.address, b.postalCode, b.city].filter(Boolean).join(', ') || noAddress
}
...
function PendingPaymentCard({ order, C, now, s, dateLocale }: { order: SellingOrder; C: Record<string, string>; now: number; s: any; dateLocale: string }) {
  ...
  return (
    <div className="hb-card" style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderLeft: `4px solid ${C.warn}`, borderRadius: 12, padding: '14px 18px', marginBottom: 10, boxShadow: '0 1px 2px rgba(0,0,0,0.08)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 9 }}>
        ...
        <span style={{ fontSize: 11, color: C.muted }}>{new Date(order.createdAt).toLocaleDateString(dateLocale)}</span>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: `1px solid ${C.border}`, paddingTop: 9 }}>
        ...
        <span style={{
          ...
          {remaining !== null ? (remaining > 0 ? timeLeftLabel(remaining) : s.statusExpired) : s.statusWaiting}
        </span>
      </div>
    </div>
  )
...
function OrderCard({ order, showTracking, C, stripe, badge, trackingValue, onTrackingChange, onSubmitTracking, pickupValue, onPickupChange, onSubmitPickup, pakettikokoValue, onPakettikokoChange, onCreateShipment, shipmentBusy, busy, review, onRefund, refundBusy, s, dateLocale }: {
  ...
  onRefund: () => void; refundBusy: boolean; s: any; dateLocale: string
}) {
  return (
        ...
        {showTracking && order.shippingSize === 'nouto' && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', flex: '1 1 240px', justifyContent: 'flex-end' }}>
            <input
              ...
              placeholder={s.pickupCodePlaceholder}
        ...
        {order.shippingSize === 'postitus' && (order.labelUrl || order.sendingCode || order.trackingCode) && (
          order.labelUrl ? (
            ...
            <span style={{ fontSize: 12, color: C.accent, fontWeight: 700 }}>
              {order.sendingCode ? `${s.shippingCodeLabel} ${order.sendingCode}` : `${s.trackingCodeLabel} ${order.trackingCode}`}
            </span>
          )
        )}
      </div>
      ...
      {review && (
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.border}` }}>
          {review.alreadyReviewed ? (
            <div style={{ fontSize: 13, color: C.accent, fontWeight: 600 }}>{s.reviewThanks}</div>
          ) : review.open ? (
            <div>
...
export default function TilauksetPage() {
  ...
  async function submitReview(orderId: string) {
    ...
    if (rating < 1) { setError(s.errSelectRating); return }
    ...
    try {
      ...
      await load()
    } catch (e: any) { setError(e.message ?? s.errReviewFailed) }
    setReviewBusy(null)
  }
  ...
  const sumOf = (list: SellingOrder[]) => list.reduce((sum, o) => sum + orderTotal(o), 0)

  const shippedBadge = (o: SellingOrder) => ({
    text: o.labelUrl ? s.badgeLabel : (o.sendingCode ?? o.trackingCode ?? s.badgeInTransit),
    bg: C.surface2, color: C.textSub,
  })
  const doneBadge = { text: s.badgeDone, bg: C.accentLight, color: C.accent }

  return (
    <div style={{ color: C.text }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontFamily: 'var(--font-display), sans-serif', fontSize: 24, fontWeight: 800, color: C.text, letterSpacing: '-0.01em' }}>{s.title}</h1>
        <p style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>{s.subtitle}</p>
      </div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: 40, color: C.muted }}>{s.loading}</div>
      ) : (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(270px, 1fr))', gap: 18, alignItems: 'start' }}>
          {pendingPayment.length > 0 && (
            <div style={{ minWidth: 0 }}>
              <ColumnHeader name={s.colPendingPayment} count={pendingPayment.length} sum={pendingPayment.reduce((sum, o) => sum + o.productTotal, 0)} stripe={C.warn} C={C} />
              {pendingPayment.map(o => <PendingPaymentCard key={o.id} order={o} C={C} now={now} s={s} dateLocale={dateLocale} />)}
            </div>
          )}

          <div style={{ minWidth: 0 }}>
            <ColumnHeader name={s.colNewOrders} count={ready.length} sum={sumOf(ready)} stripe={C.accent} C={C} />
            {ready.length === 0
              ? <EmptyColumn text={s.emptyReady} C={C} />
              : ready.map(o => (
                <OrderCard
                  key={o.id} order={o} showTracking C={C} stripe={C.accent} badge={doneBadge} s={s} dateLocale={dateLocale}
                  trackingValue={trackingInput[o.id] ?? ''}
                  onTrackingChange={v => setTrackingInput(state => ({ ...state, [o.id]: v }))}
          ...
          <div style={{ minWidth: 0 }}>
            <ColumnHeader name={s.colShipped} count={shipped.length} sum={sumOf(shipped)} stripe={C.muted} C={C} />
            {shipped.length === 0
              ? <EmptyColumn text={s.emptyShipped} C={C} />
              : shipped.map(o => (
                <OrderCard
                  key={o.id} order={o} showTracking={false} C={C} stripe={C.muted} badge={shippedBadge(o)} s={s} dateLocale={dateLocale}
                  trackingValue={trackingInput[o.id] ?? ''}
                  onTrackingChange={v => setTrackingInput(state => ({ ...state, [o.id]: v }))}
                  onSubmitTracking={() => submitTracking(o.id)}
                  pickupValue={pickupInput[o.id] ?? ''}
          ...
          <div style={{ minWidth: 0 }}>
            <ColumnHeader name={s.colDelivered} count={delivered.length} sum={sumOf(delivered)} stripe={C.accent} C={C} />
            {delivered.length === 0
              ? <EmptyColumn text={s.emptyDelivered} C={C} />
              : delivered.map(o => (
                <OrderCard
                  key={o.id} order={o} showTracking={false} C={C} stripe={C.accent} badge={doneBadge} s={s} dateLocale={dateLocale}
                  trackingValue={trackingInput[o.id] ?? ''}
                  onTrackingChange={v => setTrackingInput(state => ({ ...state, [o.id]: v }))}
                  onSubmitTracking={() => submitTracking(o.id)}
                  pickupValue={pickupInput[o.id] ?? ''}
                  ...
                  review={{
                    ...
                    onRatingChange: v => setReviewRating(state => ({ ...state, [o.id]: v })),
                    onCommentChange: v => setReviewComment(state => ({ ...state, [o.id]: v })),
