  ...
  return (
    <div style={{ color: C.text }}>
            ...
            <tbody>
              {orders.map((o: any, i: number) => (
                <tr key={o.id} style={{ borderBottom: i < orders.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                  <td style={{ padding: '12px 16px', color: C.text, fontWeight: 500 }}>{o.product}</td>
                  <td style={{ padding: '12px 16px', color: C.muted }}>{o.buyer}</td>
                  <td style={{ padding: '12px 16px', color: C.text, fontWeight: 600 }}>{o.amount}€</td>
                  <td style={{ padding: '12px 16px', color: C.muted }}>−{o.commission.toFixed(2)}€</td>
                  <td style={{ padding: '12px 16px', color: C.accent, fontWeight: 700 }}>{o.net.toFixed(2)}€</td>
                  ...
                  <td style={{ padding: '12px 16px', color: C.muted }}>{o.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ marginTop: 16, padding: '12px 16px', background: C.surface, borderRadius: 8, fontSize: 12, color: C.muted }}>
        Välityspalkkio on 3% + maksunkäsittelykulut (n. 1,5% + 0,25€), enintään 20€ per kauppa.
      </div>
    </div>
  )
}
