...
interface ShowData { id: string; title: string; viewerCount: number; seller: { username: string }; products: ShowProduct[] }
...
export default function LivePage({ params }: { params: Promise<{ showId: string }> }) {
  ...
  useEffect(() => {
    fetch((process.env.NEXT_PUBLIC_BACKEND_URL ?? 'http://localhost:4000') + `/shows/${showId}`)
      .then(r => r.json())
      .then((data: ShowData) => {
  ...
  const ChatArea = ({ dark }: { dark: boolean }) => (
