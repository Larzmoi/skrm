...
export default function LiveKaikki() {
  ...
  useEffect(() => {
    ...
    fetch((process.env.NEXT_PUBLIC_BACKEND_URL ?? 'http://localhost:4000') + `/shows?status=${status}`)
      .then(r => r.json())
  ...
  const displayShows = shows.map((s: any) => ({
    ...
    title: s.title, viewers: s.viewerCount ?? 0,
    ...
    <div style={{ minHeight: '100vh', background: C.bg }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
          ...
          <span style={{ fontSize: 15, color: C.muted }}>{displayShows.length} lähetystä</span>
        </div>
