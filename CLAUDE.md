...
- **Käännökset — OSITTAIN TEHTY 2026-09-05, EI TÄYSI AUDITOINTI.** Korjattu tuotesivun (`tuotteet/[id]`) jaa/tarjous/osto-virheviestit, perinteisen huutokaupan (`huutokauppa/[id]`) huuto/automaattihuuto/osta-viestit, ja huutokauppalistan (`huutokaupat`) otsikko/lajittelu/laskuri/"Päättynyt" — kaikki olivat raakaa suomea sivuilla jotka muuten käyttävät `t.xxx`:ää läpi koko sivun, joten kielenvaihto ei vaikuttanut näihin lainkaan. **Ei kattava koko-sivuston auditointi** — `/ostot` (dashboard, ostohistoria) löytyi sisältävän ~30 vastaavaa kääntämätöntä merkkijonoa (tilarivien otsikot, virheviestit, napit) joita ei ehditty tässä erässä, ja muitakin sivuja ei käyty systemaattisesti läpi. Jäljellä oleva täysi auditointi on oma, isompi tehtävänsä — sama rajaus kuin aiemminkin dokumentoitu.
