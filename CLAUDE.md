...
**⚠️ JATKOKORJAUS SAMANA PÄIVÄNÄ — omistaja vahvisti ettei ensimmäinen versio korjannut mitään, sivu hyppäsi yhä alkuun.** Yksi kertaluontoinen `requestAnimationFrame`-pohjainen palautus ei riittänyt — kilpaili selaimen omaa automaattista back-scroll-restaurointia vastaan, joka laukeaa `popstate`-tapahtumassa VÄLITTÖMÄSTI (ennen kuin React ehtii renderöidä tuotegridin), yrittää palauttaa liian lyhyelle "Ladataan..."-dokumentille eikä koskaan yritä uudestaan kun sisältö sitten kasvaa. **Korjaus:** `history.scrollRestoration = 'manual'` ottaa selaimen oman restauroinnin kokonaan pois käytöstä tällä sivulla (palautetaan alkuperäiseksi kun poistutaan sivulta), ja oma palautuslogiikka pakottaa tallennetun sijainnin uudestaan 50ms välein ~500ms ajan (kunnes se pysyy tai 10 yritystä täynnä) yhden kertaluontoisen yrityksen sijaan — kestää läpi mahdolliset Next.js:n omat, ajoitukseltaan vaihtelevat scroll-yritykset.
