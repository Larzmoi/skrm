...
- ✅ **KORJATTU 2026-08-14 — kolme löydöstä `dashboard/tuotteet`-sivun (myyjän tuotehallinta) testauksesta, omistajan raportoima.**
  ...
  - **Ei muutettu:** poisto-yritys tuotteelle jolla on jo tilaus antaa virheen "Tuote on osa tilausta, ei voida poistaa" — tämä on TARKOITUKSELLINEN suojaus (`backend/src/routes/products.ts`), ei bugi. Estää rahaliikenteen historian poistamisen. Virhe tulee suoraan backendiltä suomeksi riippumatta valitusta kielestä — sama koskee kaikkia backendin virheviestejä koko sivustolla, ei tälle sivulle ominainen puute, ei korjattu tässä yhteydessä (vaatisi laajemman backend-virhekoodien i18n-arkkitehtuurin).
