...
## Puuttuva ilmoitus "huutokaupassa olleesta kohteesta" 2026-09-09 — tutkittu, selitys löytyi: oma testi
...
**Löytyi oikea selitys `Notification`-taulusta:** "Charmeleon (DR 99)" (myyjä `michaelbacklund`) poistettiin admin-oikeuksilla **2026-09-08 klo 19:28 Suomen aikaa**, syyksi kirjattu "muuten vaan" — ajallisesti täsmää täsmälleen samana päivänä deployattuun "Poista (admin)" -napin (ks. yllä) ensimmäiseen testiklikkaukseen. Myyjä sai oikein `LISTING_REMOVED`-ilmoituksen. **Löydetty todellinen puute:** `DELETE /admin/products/:id`/`shows/:id` ilmoittaa vain myyjälle, ei koskaan sillä hetkellä johdossa olevalle tarjoajalle (`currentBidderId`) jos tuotteella oli aktiivinen tarjous. **Omistaja vahvisti ettei "Charmeleonilla" ollut oikeaa tarjoajaa (pelkkä oma testiklikkaus) — ei korjattu, tietoinen päätös.** Jos vastaava tilanne toistuu oikealla tarjoajalla tulevaisuudessa, korjaus olisi lisätä sama ilmoitus myös `currentBidderId`:lle ennen poistoa.
