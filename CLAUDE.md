...
- **11 (kriittinen):** `/lahetys`-sivun tilantarkistus ajettiin vain ensimmäisellä mountilla — Next.js:n router-cache saattoi palauttaa vanhentuneen tilan takaisin-navigoinnissa. Lisätty `checkForActiveShow()` joka ajetaan uudelleen `pageshow`/`visibilitychange`-tapahtumissa, varmistaa aina palvelimelta.
  ...
  - **✅ OIKEA JUURISYY LÖYTYI JA KORJATTU 2026-08-13:** "←"-nappi on tavallinen Next.js `<Link href="/dashboard">` — normaali reitinvaihto, joka unmounttaa/remounttaa `/lahetys`-komponentin täysin normaalisti (ei mikään router-cache-oikku, se koski vain aiempaa selaimen takaisin-nappi-hypoteesia). Mount-efekti JO tunnisti oikein käynnissä olevan Show:n paluun jälkeen (`checkForActiveShow()`). Todellinen puuttuva pala: myyjän socket-yhteys liittyi huoneeseen uudestaan (`join_show`) mutta **ei koskaan kuunnellut `auction_state`-tapahtumaa** jonka palvelin lähettää takaisin jos huutokauppa on käynnissä — sama tapahtuma jota katsojan `/live/[showId]`-sivu jo käytti samaan tarkoitukseen. Ilman tätä konsoli näytti aina "ei huutokauppaa käynnissä" -tilaa vaikka OBS lähetti ja huuto oli oikeasti aktiivinen palvelimella. Lisätty `auction_state`-kuuntelija joka palauttaa `auction`-tilan JA `currentProductId`:n.
...
**23. Videon laatu tuntuu heikolta puhelimella — pelkkä visuaalinen vaikutelma, ei mitattu lukema. Puuttuu näkyvät laatuasetukset/tilastot, jotta tätä voisi oikeasti tarkistaa.** Omistaja tarkensi: "200-360" ei ollut mitään näytöllä näkyvää lukemaa, vain kokemusperäinen arvio siitä miltä laatu näytti. **Tämä paljastaa oikean puutteen: ei ole mitään tapaa nähdä tai säätää striimin laatua käyttöliittymästä.** Ehdotus: lisää OBS-asetukset-paneeliin (tai omaan kohtaansa) näkyvät laatutilastot (esim. nykyinen resoluutio/bittinopeus jota katsojalle toistetaan) sekä mahdollisuus tarkistaa/säätää LiveKitin simulcast-tasoja. Tämä poistaisi arvailun tarpeen kokonaan — sekä omistaja että myöhemmin myyjät näkisivät suoraan onko laatu odotetulla tasolla.

## Uudet löydökset 2026-08-13, osa 5 (Shop-PiP-käytös, tuotetiedot, ja KRIITTINEN Paytrail-regressio) — tutkittu 2026-08-13/14
...
**✅ TARKISTETTU 2026-08-13 — EI VAATINUT KORJAUSTA:** koodi tarkistettu `frontend/app/live/[showId]/page.tsx`:stä — PiP-kontainerissa on jo `onClick={() => setShopOpen(false)}` eikä mikään sisempi elementti (mm. ei oma X-nappi PiP:in päällä) pysäytä tapahtuman kuplintaa (`stopPropagation`) tieltä. Klikkaus mihin tahansa kohtaan pientä PiP-videota siis jo palauttaa täyden näkymän — ei löytynyt bugia, ei tehty muutosta jotta ei rikota jotain toimivaa spekulatiivisesti.
...
**✅ SAMA JUURISYY KUIN KOHTA 18, KORJAUTUI SAMALLA:** "LOT #0" / ei kuvaa oli sama `Product.showId`-liittämättömyys-bugi kuin kohta 18 (Shop-paneeli) — katsoja luki nykyisen tuotteen tiedot samasta `Show.products`-relaatiosta, joka oli tyhjä ennen `claim-products`-korjausta. Vahvistettava tuoreella testikierroksella että myös tämä oire on poistunut nyt kun tuotteet oikeasti liittyvät showhun, mutta erillistä koodimuutosta ei tarvittu.

**21. KRIITTINEN REGRESSIO: Paytrail-maksuvirtaus rikki, 404-virhe kesken maksua.** Voitettu huuto siirtyi oikein ostoskoriin, maksu aloitettiin normaalisti, mutta kesken Paytrail-maksuvirtaa tuli **404 "Etsimääsi sivua ei löytynyt"** osoitteessa `services.paytrail.com/payments/{id}/osuu...` (URL katkesi kuvakaappauksessa, todennäköisesti tietyn maksutavan/pankin välisivu). **Omistaja vahvistaa: tämä toimi aiemmin oikein** — tämä on siis regressio, ei koskaan-toiminut-bugi. **Tutki:** onko maksutapavalikossa/redirect-URL:n rakentamisessa jotain rikkoutunut viimeisimpien muutosten myötä? Liittyykö tämä aiempaan turvallisuuslöydökseen (ei-OP-pankkien testitilan epävakaudesta, ks. "3. Paytrail" -kohdan turvallisuuslöydös) — testaa ensin OP:lla jos ei jo testattu sillä, eristä ongelma ennen laajempaa korjausta.
**⚠️ EI KORJATTU 2026-08-13 — ODOTTAA OMISTAJAN OP-TESTIÄ.** `backend/src/lib/paytrail.ts` käyty läpi kokonaan: `createPayment()` palauttaa `redirectUrl`:n suoraan Paytrailin omasta API-vastauksesta (`data.href`), ei rakenneta paikallisesti — eli itse maksun luontikutsu ja sen palauttama URL ovat SKRM:n koodin ulkopuolella, Paytrailin oma vastaus. 404 tulee siis joko Paytrailin omalta välisivulta (todennäköisin, koska osoite on `services.paytrail.com`, ei `skrm.fi`) tai testitilan tietyn pankin epävakaudesta — ei löytynyt mitään SKRM:n koodissa rikkoutunutta joka selittäisi regression. **En tehnyt spekulatiivista korjausta maksukoodiin ilman varmuutta** — väärä arvaus maksuvirtauksessa on liian riskialtista. Vaatii omistajan oikean selaimen läpiviennin OP:lla (ei Danskella, ks. tunnettu rajoitus) ennen kuin voidaan päätellä onko kyse Paytrailin testiympäristöstä vai jostain oikeasti korjattavasta.
...
**✅ TEHTY 2026-08-13:** rivin klikkaus valitsee edelleen nykyisen tuotteen (ei muutettu, tärkeä livetyönkulku) — lisätty erillinen pieni "⤢"-nappi jokaiselle riville, avaa `QueueProductModal`in (kuva/kuvaus/hinta, samantyylinen kuin katsojan suurennusmodaali) + "Muokkaa tuotetta →" -linkki. `dashboard/tuotteet`-sivu tukee nyt `?edit=<id>`-syväliitosta, avaa muokkauslomakkeen automaattisesti.
...
**✅ JUURISYY LÖYTYI JA KORJATTU 2026-08-13:** `/lahetys` käytti `useTheme()`:n `C.accent`/`C.accentBright`:ä, jotka vaihtelevat käyttäjän vaalea/tumma-sivustoteeman mukaan (vaalea teema = tummempi vihreä `#2ECC71` vs. tumma teema `#4ADE80`) — mutta konsoli on AINA tumma taustaltaan (musta overlay-UI) riippumatta käyttäjän valinnasta, joten vaalean teeman vihreä näytti tunkkaiselta. Korjattu käyttämään kiinteitä väriarvoja (`GREEN`/`GREEN_DIM`/`GREEN_BG`) koko konsolissa — sama kirkas vihreä aina, ei enää sidoksissa sivustoteemaan. Vastaava riski on todennäköisesti myös `/live/[showId]`:llä (sama `useTheme()`-kuvio), ei korjattu nyt koska ei raportoitu — tarkista jos vastaava "väärän värinen" -oire ilmenee siellä.
...
**✅ JUURISYY LÖYTYI JA KORJATTU 2026-08-13 — syvempi kuin epäilty:** ei ollut kyse siitä ettei dataa haettaisi palvelimelta (katsojan Shop-paneeli haki jo `GET /shows/:id`:n `products`-relaatiota oikein) — ongelma oli että **tuotteita ei koskaan liitetty tietokannassa oikeaan Show-riviin ollenkaan**. Myyjän `/lahetys`-konsolin jono tulee `GET /products/mine`:sta (kaikki myyjän PENDING-tuotteet, riippumatta mistä showsta), eikä mikään reitti (ei tuotteen luonti, ei muokkaus, ei jonon raahausjärjestys) koskaan asettanut/päivittänyt `Product.showId`:tä vastaamaan käynnissä olevaa lähetystä — `Show.products`-relaatio oli siis lähes aina tyhjä/vajaa kaikille PAITSI myyjän omalle selaimelle, joka näytti jonon suoraan paikallisesta Reactin tilasta riippumatta tietokannan totuudesta.
- Uusi `POST /shows/:id/claim-products` (myyjä) — liittää kaikki myyjän PENDING-tuotteet käynnissä olevaan showhun, kutsutaan automaattisesti aina kun `/lahetys`-konsoli avautuu (myös paluun jälkeen)
- `POST /products` hyväksyy jo `showId`:n bodyssä, mutta `/lahetys`:n pikalisäys ei koskaan lähettänyt sitä — korjattu
- Lisätty `products_updated`-socket-tapahtuma (emittaa `POST /products` kun showId annettu, ja `claim-products`) — katsojan `/live/[showId]` kuuntelee ja hakee tuoreen tuotelistan palvelimelta automaattisesti, ei vaadi sivun päivitystä
- **Sivuvaikutuksena huomattu, ei vielä korjattu:** jonon raahausjärjestys (`handleDrop` / `Product.order`) on myös vain paikallista Reactin tilaa, ei koskaan tallennu tietokantaan — järjestys nollautuu jos konsoli ladataan uudelleen. Ei osa tätä korjausta, kirjattu talteen.
