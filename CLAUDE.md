...
## Käyttäjähaku case-insensitive + @-etuliite, oikeinkirjoitustarkistus pois tuotelomakkeista 2026-09-14 — ✅ TEHTY JA DEPLOYATTU

Kaksi omistajan raportoimaa kritiikkiä samassa viestissä.

**1. Käyttäjähaku.** `/selaa`-sivun hakukenttä tekee jo suoran käyttäjähaun (`userApi.getPublic(q)`, ks. "Etusivu/haku"-osio) mutta kaikki neljä `users.ts`:n `:username`-reittiä (`GET /:username`, `/reviews`, `/followers`, `POST /follow`) käyttivät `decodeURIComponent(req.params.username)`:ia suoraan `where`-ehtona — täsmällinen, kirjainkoosta riippuvainen osuma. "@Larzmoi" tai "larzmoi" ei koskaan löytänyt "Larzmoi"-profiilia. **Korjaus:** uusi jaettu `usernameFilter()`-apufunktio (`backend/src/routes/users.ts`) trimmaa syötteen, poistaa mahdollisen `@`-etuliitteen ja palauttaa Prisman `{equals, mode:'insensitive'}`-suodattimen — kaikki neljä reittiä käyttävät nyt samaa. **Tietoinen riski, hyväksytty pienenä:** `username` on `@unique` schema.prismassa mutta Postgresin oletuscollation on case-sensitive, joten teoriassa kaksi tiliä joiden nimet eroavat vain kirjainkoolla (esim. "Larzmoi"/"larzmoi") voisivat molemmat olla olemassa ja case-insensitive haku palauttaisi vain toisen — tuotannossa on tällä hetkellä vain ~9 käyttäjää, ei yhtään tällaista törmäystä, riski käytännössä olematon.

**2. Oikeinkirjoituksen tarkistus/autokorjaus pois tuotelomakkeista.** Omistaja: "kun teen ilmoitusta niin kokoajan joku ihmeen oikeinkirjoitus paska yrittää sekaantua enkä saa kirjoitettua rauhassa" — mobiilin näppäimistön (todennäköisesti Android/Gboard) automaattinen korjausehdotusvalikko häiritsi kirjoittamista/korjaamista tuotteen nimi-/kuvauskentissä. Lisätty `spellCheck={false} autoCorrect="off" autoCapitalize="off"` kaikkiin tuotteen vapaatekstikenttiin: `dashboard/tuotteet`:n nimi+kuvaus+bulkkituonnin liimauskenttä, `dashboard/esiasetukset`:n nimi+kuvaus, `/lahetys`-konsolin pikalisäyksen nimikenttä. `autoCorrect`/`autoCapitalize` ovat React/TS:n hyväksymiä läpivietäviä DOM-attribuutteja (ei React-omia, mutta tyypitetty `InputHTMLAttributes`:ssa) — `spellCheck={false}` kytkee pois myös itse selaimen punaisen aaltoviivan tarkistuksen, ei vain näppäimistön ehdotukset.

Typecheck+build vihreä molemmilla puolilla, deployattu. Käyttäjähaku testattu suoraan tuotanto-API:a vasten kolmella muodolla (`@Larzmoi`/`larzmoi`/`LARZMOI`) — kaikki löytävät saman profiilin, olematon käyttäjänimi palauttaa yhä siistin 404:n.

## Tilausten yhdistämisikkuna 6h → 24h + näkyvyys FAQ:ssa ja tilauskorteissa 2026-09-14 — ✅ TEHTY JA DEPLOYATTU

Omistajan pyyntö, lähtökohtana live-huutokauppaa pitävän myyjän oman asiakkaan kysymys: asiakas huusi useasta kohteesta samassa livessä ja kysyi myyjältä voiko ne yhdistää samaan pakettiin, koska mistään ei löytynyt ohjetta. Tarkistettu koodista ennen vastaamista — **järjestelmä yhdisti tämän jo täysin automaattisesti** (ei koskaan tarvinnut olla yhteydessä myyjään), mutta kahdesta syystä tämä ei näkynyt kellekään: (1) yhdistämisikkuna oli vain 6h, liian lyhyt kattamaan koko illan mittaisen livelähetyksen, (2) mikään UI ei koskaan vahvistanut ostajalle että yhdistäminen oikeasti tapahtui.

**1. Yhdistämisikkuna 6h → 24h.** `SHIPPING_MERGE_WINDOW_MS` päivitetty sekä `backend/src/routes/cart.ts`:ssä (tavallinen ostoskori-checkout) että `backend/src/lib/auctionOrder.ts`:ssä (`createOrderForAuctionWin()`, jota myös live-huudon voitto käyttää `socket.ts`:n `finalizeLiveAuctionSale()`:n kautta) — molemmat käyttävät samaa vakiota, molemmat päivitetty yhdessä.

**2. Selvitetty ja dokumentoitu: omistajan pyytämä "tai kunnes myyjä on luonut lähetyksen Postin palvelussa, jos se tapahtuu ennen 24h:aa" -raja on rakenteellisesti jo mahdoton ylittää, ei tarvinnut erillistä koodia.** Yhdistäminen (`findFirst`-haku molemmissa tiedostoissa) etsii AINA vain `status: 'PENDING_PAYMENT'` (maksamattomia) tilauksia — kun ensimmäinen tilaus maksetaan, se siirtyy `PENDING_SHIPPING`-tilaan eikä ole enää koskaan yhdistämisen kohde, uusi ostos samalta myyjältä aloittaa aina oman erillisen tilauksensa. `order.trackingNumber` (oikean Posti-lähetyksen merkki) taas voidaan asettaa VAIN `POST /:id/create-shipment`-reitin kautta, joka vaatii eksplisiittisesti `status === 'PENDING_SHIPPING'` (ks. `orders.ts` rivi ~420). Nämä kaksi ehtoa yhdessä tarkoittavat: maksamattomalla, yhä yhdistettävissä olevalla tilauksella ei voi KOSKAAN olla `trackingNumber`:a — "kunnes lähetys luotu" -raja on siis aina jo toteen käynyt sillä hetkellä kun tilaus maksetaan, paljon ennen kuin Posti-lähetys voisi edes olla olemassa.

**3. FAQ-kysymys lisätty (fi/en/sv), toimitus-osioon.** Kertoo suoraan: yhdistäminen on automaattista, ei tarvitse olla yhteydessä myyjään, toimii 24h sisällä niin kauan kuin ensimmäistä ostosta ei ole vielä maksettu — maksun jälkeen yhdistäminen lukkiutuu siihen mennessä kertyneisiin tuotteisiin.

**4. Näkyvä vahvistus tilauskorteissa, uusi `t.purchases.mergedOrderNote`/`t.salesPage.mergedOrderNote`-avain (fi/en/sv).** Kun `order.items.length > 1`, sekä ostajan `/ostot`-sivu että myyjän `dashboard/tilaukset`-sivu (molemmat kortit, `PendingPaymentCard` ja `OrderCard`) näyttävät nyt rivin "Tämä tilaus yhdistää N ostosta samalta myyjältä — postikulut veloitetaan vain kertaalleen." — ostajan/myyjän ei tarvitse enää arvailla yhdistyikö ostos, se näkyy suoraan.

**Sivulöydös samalla haulla, korjattu yhtenäisyyden vuoksi:** rekisteröitymislomakkeen kaupankäyntipolitiikka-checkboxin `policyPoint4`-teksti ("Yhdistetty lähetys: sama myyjä 6h sisällä...") mainitsi yhä vanhan 6h-arvon kaikissa kolmessa kielessä — päivitetty 24h:ksi samassa yhteydessä.

**Sivutuote, korjattu ohimennen samalla deployllä:** edellisen, kesken jääneen "Nouto mahdollinen" -badge-tehtävän (ks. alempana, ei vielä valmis) `t.product.pickupBadge`-avain oli lisätty vain `fi.ts`:ään, mikä esti koko frontendin typecheckin (kolmen kielitiedoston tyyppien pitää täsmätä). Lisätty sama avain myös `en.ts`:ään ("Pickup available") ja `sv.ts`:ään ("Avhämtning möjlig") jotta build pysyy vihreänä — **itse badge-ominaisuus (näkyminen tuotekorteissa/-listauksissa) on yhä toteuttamatta**, vain kääntäjärjestelmän typeyhteensopivuus korjattu.

**Deployn sivuhuomio:** GitHub-repo taas hetken yksityinen deployn aikana (sama toistuva kuvio, ks. "Työskentelytapa"-osio) — käytetty jo opittua `scp`+CRLF-normalisointi+md5-vertailu-kaavaa. Yksi oma virhe matkalla: ensimmäinen scp-komento (yritti kopioida kaksi tiedostoa yhteen tiedostonimeen) loi vahingossa hakemiston `backend/src/lib/auctionOrder.ts.tmp/` palvelimelle, joka tsc:n include-polku nappasi mukaan ja aiheutti vääriä moduulivirheitä buildissa — huomattu heti virheviestistä, poistettu ennen uudelleenrakennusta, ei jäänyt roikkumaan. Palvelimen git-historia on nyt hetken jäljessä origin/mainista (sisältö on jo identtinen, seuraava onnistunut `git pull` synkronoi ilman konfliktia).

Typecheck+build vihreä molemmilla puolilla, deployattu ja vahvistettu (`/health` 200, `/faq` 200).

## Hyvitysten kaksi rahansuojaussääntöä: postimaksu jäädytetty lähetyksen luonnin jälkeen + komissio ei koskaan osa hyvitystä 2026-09-14 — ✅ TEHTY JA DEPLOYATTU

Jatkoa samana päivänä tehtyyn osittaishyvitysominaisuuteen (ks. seuraava osio). Omistaja kysyi suoraan: "eihän sielä voi sitten hyvittää postimaksua jos se lähetys on luotu?" — vastaus oli tekniseltä kannalta "kyllä voi" (mitään estoa ei ollut), mikä paljasti kaksi todellista, ei vielä olemassa ollutta rahansuojaussääntöä joita omistaja halusi heti lukkoon:

**1. Postimaksua ei voi enää hyvittää sen jälkeen kun Posti-lähetys on oikeasti luotu.** Kun `POST /orders/:id/create-shipment` on ajettu, Habahub on jo maksanut Postille oikeasta, laskutettavasta lähetyksestä — jos postimaksu silti hyvitetään ostajan kortille sen jälkeen, Habahub jää suoraan tappiolle (Postille maksettu summa ei koskaan palaudu). Omistajan oma sanamuoto: "tässä olisi mahdollisuus asiakkailla käyttää meidän varoja väärin." **Korjaus:** `POST /orders/:id/refund` tarkistaa nyt `order.trackingNumber`:n (asetetaan VAIN oikeasta Posti-API-kutsusta, ei myyjän manuaalisesta `trackingCode`-syötöstä) — jos se on asetettu, `shippingRefundEuros > 0` hylätään `400`:llä selkeällä viestillä, ja "koko tilaus" -hyvitys (ilman eksplisiittistä euromäärää) jättää postiosuuden automaattisesti pois summasta sen sijaan että yrittäisi hyvittää sitä.

**2. Tuotehinnan (kauppahinnan) hyvitys ei koskaan saa pienentää Habahubin alkuperäistä komissiota — hyvitys tulee aina kokonaan myyjän omasta osuudesta, "pidetään se omana komponenttina" (omistajan sanamuoto).** Tutkittu tarkasti Stripen "separate charges and transfers" -rahavirta ennen koodausta: myyjälle siirretään aina täsmälleen `productTotal − commissionCents/100` (ks. webhooks.ts), komissio EI KOSKAAN kulje myyjän Stripe-tilin kautta — se jää suoraan Habahubin omalle saldolle jo maksuhetkellä. **Vanha hyvityslogiikka teki tästä huolimatta koko pyydetyn summan (esim. koko kauppahinnan) hyvityksen ostajalle, mutta perii myyjältä takaisin vain hänen oman, komissiolla jo vähennetyn osuutensa** — koska Stripen `transfers.createReversal()` ei koskaan voi periä myyjältä ENEMPÄÄ kuin mitä hänelle alun perin siirrettiin, tämä tarkoitti että Habahubin oma saldo kutistui täsmälleen komission verran jokaisella täydellä hyvityksellä — komissio "vuoti pois" jokaisen hyvityksen mukana ilman että kukaan pyysi sitä.

**Korjaus, ainoa Stripen rajoitteiden puitteissa tekninen tapa taata "komissio ei koskaan liiku":** ostajalle takaisin maksettava summa JA myyjältä transfer-reversalilla perittävä summa ovat nyt AINA sama, komissiolla netotettu luku (`transferAmountEuros × (hyvitettävä osuus / productTotal)`, missä `transferAmountEuros = productTotal − commissionCents/100`) — kaikissa kolmessa hyvityshaarassa (koko tilaus, per-tuoterivi, vapaa euromäärä). **Käytännön seuraus, tietoinen ja selkeästi kommunikoitu käyttöliittymässä:** kun myyjä valitsee "Koko summa" tuoteosuudelle, ostaja saa takaisin `productTotal − komissio`, ei koko alkuperäistä kauppahintaa — komissio ei enää koskaan liiku takaisin kumpaankaan suuntaan, riippumatta hyvityksen koosta.

**Frontend (`dashboard/tilaukset`):** hyvitysdialogi näyttää nyt tuotekentän alla live-lasketun huomautuksen ("Ostaja saa tästä takaisin X€ — Habahubin Y€ komissio ei koskaan sisälly hyvitykseen"), ja piilottaa/selittää toimituskentän kokonaan kun `order.trackingNumber` on asetettu ("Postimaksua (X€) ei voi enää hyvittää — lähetys on jo luotu Postiin"). Alareunan yhteissumma laskee nyt aina oikean, komissiolla netotetun luvun — ei enää näytä harhaanjohtavaa "koko summa" -lukua jota backend ei todellisuudessa hyvittäisi.

**Ei kosketettu:** legacy-hyvityspolku (`order.stripeTransferId === null`, tilaukset jotka maksettiin ennen 2026-09-11 destination-charge-arkkitehtuurimuutosta) käyttää yhä Stripen omaa `reverse_transfer`+`refund_application_fee`-automatiikkaa, joka EI noudata tätä uutta sääntöä (Stripe palauttaa komission automaattisesti suhteutettuna) — erittäin harvinainen, historiallinen tapaus, ei muutettu koska vaatisi täysin eri, manuaalisen mekanismin eikä näitä tilauksia enää synny.

Typecheck+build vihreä molemmilla puolilla, deployattu. **Ei testattu oikealla Stripe-maksulla** (sama varovaisuusperiaate kuin muillakin live-rahaa liikuttavilla muutoksilla) — laskukaava on sama, jo aiemmin oikeaa hyvitystä vasten vahvistettu `transferAmountEuros × fraction` -kaava (ks. seuraava osio, Michael Tunturin 7,90€ postimaksuhyvitys), vain sovellettu nyt myös `refundAmountEuros`:iin eikä pelkkään `transferReversalAmountEuros`:iin.
...
**⚠️ KAKSI JATKOKORJAUSYRITYSTÄ SAMANA PÄIVÄNÄ, MOLEMMAT VÄÄRIN DIAGNOSOITU — kolmas kerta löysi oikean juurisyyn oikealla selaimella (Playwright asennettu kertaluontoisesti tähän ympäristöön), ei enää arvaamalla.**

Ensimmäinen ja toinen korjausyritys (rAF-kertapalautus, sitten `history.scrollRestoration='manual'` + 500ms uudelleenyrityssilmukka) olettivat molemmat että ongelma on PALAUTUKSESSA — kilpailu selaimen/Next.js:n oman scroll-restauroinnin kanssa. Omistaja vahvisti kummankin jälkeen ettei mikään muuttunut. Koska tässä ympäristössä ei ole selaintyökalua, asennettiin Playwright kertaluontoisesti (`npm install playwright` + `npx playwright install chromium`, ei jäänyt projektiin) ja ajettiin oikea selain tuotantoa vasten: scroll alas → klikkaa tuotekortti → tarkista `sessionStorage` heti klikkauksen jälkeen.

**Paljastui: TALLENNUS oli koko ajan rikki, ei palautus.** Vanha toteutus kuunteli `window`:n `scroll`-tapahtumaa jatkuvasti koko sivulla olon ajan. Kun tuotekorttia klikattiin, `sessionStorage`-arvo muuttui **"0":ksi VÄLITTÖMÄSTI, jo ennen kuin edes navigoitiin pois `/selaa`-sivulta** — todistettu suoraan `sessionStorage`-arvon muutoslokilla. Syy: React ei irrota vanhan sivun tapahtumankuuntelijaa synkronisesti navigoinnin yhteydessä — uuden sivun oma "vieritä ylös" tapahtuu ja laukaisee `scroll`-tapahtuman, jonka VANHAN `/selaa`-komponentin kuuntelija ehtii vielä nähdä ja kirjoittaa sen `0`:n juuri tallennetun oikean arvon (esim. 1200) päälle — kolmen aiemman korjausyrityksen palautuslogiikka oli siis koko ajan yrittämässä palauttaa arvoa joka oli jo tuhoutunut ennen kuin sivulle edes palattiin.

**Lopullinen korjaus:** ei jatkuvaa `scroll`-kuuntelijaa ollenkaan. Sijainti tallennetaan VAIN sillä hetkellä kun tuotekorttia klikataan — `onClickCapture` tuotegridin kääre-`<div>`:ssä (capture-vaihe ajoittuu ennen Next.js:n oman `<Link>`-komponentin klikkauskäsittelijää, joten tallennus on aina synkronisesti ajoissa ennen kuin mikään navigointiin liittyvä ehtii tapahtua — ei mitään kilpailevaa myöhempää kirjoitusta voi enää syntyä). `history.scrollRestoration='manual'`-toggle ja uudelleenyrityssilmukka palautuksessa säilytetty (harmiton lisävarmuus, mutta ei ollut koskaan varsinainen syy).

**Testattu OIKEALLA, automatisoidulla selaimella tuotantoa vasten kahdesti peräkkäin (ei koodikatselmus, ei arvaus):** scroll 1200px alas → klikkaa näkyvissä oleva tuotekortti (huom: Playwright-testissä piti erikseen varoa klikkaamasta korttia joka oli jo vieritetty pois näkyvistä, koska Playwright itse vierittää sen takaisin näkyviin ennen klikkausta — vaikutti aluksi samalta bugilta, osoittautui oman testiskriptin virheeksi) → `sessionStorage` säilyy oikeana (`"1200"`) tuotesivulla → selaimen "takaisin" → `scrollY` palautuu täsmälleen `1200`:aan heti kun sisältö (docHeight 1070→4107) on latautunut, molemmilla ajokerroilla. Playwright poistettu asennuksesta käytön jälkeen, ei jäänyt projektin riippuvuudeksi.
...
**Auditointi paljasti todellisen puutteen, ei vain arvailtu:** `NotificationType`-enumissa on 30 arvoa, kaikki 29 aktiivisesti käytössä (`DELIVERY_CONFIRMED` on ainoa orpo — jätetty koskematta, päällekkäinen `ORDER_DELIVERED`/`ORDER_AUTO_COMPLETED`+`PAYMENT_RELEASED`-parin kanssa jotka jo kattavat saman tilanteen, ei todellinen puute). **Mutta oikeaa käyttöjärjestelmätason push-ilmoitusta** (`sendPushToUser()`, `backend/src/lib/push.ts` — eri kanava kuin in-app `Notification`-rivi+socket-tapahtuma, toimii myös kun sivusto ei ole auki) **lähetettiin vain kahdesta 30:stä tyypistä**: `SELLER_LIVE` (myyjä meni liveen) ja `AUCTION_ENDING_SOON` (huutokauppa päättymässä). Kaikki muut — mukaan lukien `ORDER_PAID`/`ORDER_WON`/`AUCTION_SOLD` (myynnit), `MESSAGE` (uusi viesti), `OFFER_RECEIVED`/`OFFER_ACCEPTED`/`OFFER_COUNTERED` (tarjoukset), `OUTBID` (huudon ohitus), `BAN_ISSUED`, `ORDER_SHIPPED`, `DISPUTE_OPENED`, jne. — loivat vain in-app-rivin, joka näkyy vasta kun käyttäjä sattuu avaamaan sivuston avoimessa välilehdessä. Käytännössä tämä tarkoitti että suurin osa markkinapaikan tärkeimmistä tapahtumista (joku osti tuotteesi, sait viestin, tarjouksesi hyväksyttiin) ei koskaan kantautunut puhelimeen asti.

**Korjaus, keskitetty yhteen kohtaan sen sijaan että ~35 erillistä kutsupaikkaa muokattaisiin yksitellen** (pienempi, vähemmän virhealtis muutos — ei riskiä että jokin kutsupaikka unohtuisi): `backend/src/lib/notify.ts`:n `notifyUser()` kutsuu nyt itse `sendPushToUser()`:ia jokaisen ilmoituksen yhteydessä, paitsi `NEW_FOLLOWER`:lle (tietoinen poikkeus, dokumentoitu jo alkuperäisessä "Push-ilmoitukset"-osiossa 2026-08-17 — omistajan pyynnöstä vain "myyjä meni liveen" piti kantautua puhelimeen uuden seuraajan saamisesta, ei itse seuraamistapahtuma). Ei odoteta valmiiksi ennen `notifyUser()`:n paluuta (`sendPushToUser` hoitaa omat virheensä sisäisesti, `.catch(()=>{})` päälle varmuudeksi) — sama "ei blokkaa kutsujaa" -periaate kuin `push.ts`:n omalla dokumentoidulla käytöksellä. Kahden aiemman erillisen `sendPushToUser()`-kutsun (`shows.ts`:n `notifyFollowersOfLiveShow()`, `jobs/auctionEndingSoon.ts`) poistettu tarpeettomina duplikaatteina — keskitys hoitaa nyt nekin, tuplapushia ei enää lähtisi.

**"Peru ilmoitukset" -nappi:** `/ilmoitukset`-sivun tila luettiin aiemmin `Notification.permission`:sta — **väärä mittari**, koska selainlupa jää pysyvästi `'granted'`-tilaan vaikka itse `PushSubscription`-tilaus peruttaisiin koodista (`subscription.unsubscribe()` ei koskaan peruuta selainlupaa, vain poistaa tilausobjektin). Nappi olisi siis näyttänyt yhä "käytössä"-tilaa heti perumisen jälkeisellä sivunlatauksella. Korjattu: uusi `unsubscribeFromPush()` (`frontend/lib/push.ts`, käyttää olemassa ollutta `POST /push/unsubscribe`-reittiä), ja `/ilmoitukset`-sivun tila luetaan nyt oikein `registration.pushManager.getSubscription()`:lla (`null`/olemassa) — kertoo onko AKTIIVINEN tilaus juuri nyt, ei pelkkää historiallista lupaa. Nappi vaihtaa tekstin/tyylin "Ota ilmoitukset käyttöön" ↔ "Peru ilmoitukset" tilan mukaan.

**Testattu tuotannossa oikealla push-lähetyksellä, ei vain koodikatselmuksella:** kertakäyttöinen `notifyUser(testi3userId, 'MESSAGE', ...)`-kutsu suoraan palvelimella (`node dist/lib/notify.js` polun kautta, `require`illa) — testi3userilla on oikea, aiemmin luotu `PushSubscription`-rivi (Mozillan push-palvelu). Ei virhettä `pm2 logs`:ssa (`Pushin lähetys epäonnistui`-lokiviestiä ei ilmestynyt), ja tilausrivi säilyi tietokannassa muuttumattomana testin jälkeen (jos push olisi hylätty 410/404:llä, `sendPushToUser()` olisi poistanut sen automaattisesti) — vahvistaa että Mozillan push-palvelu hyväksyi lähetyksen. In-app-ilmoitusrivi siivottu heti testin jälkeen. Typecheck+build vihreä molemmilla puolilla, deployattu.
...
**Juurisyy löytyi suoraan tuotannon datasta, ei arvattu:** `POST /orders/:id/select-shipping` (`backend/src/routes/orders.ts`) tarkisti onko `order.shippingWindowEnd` yhä voimassa ennen kuin salli toimitustavan valinnan — mutta tämä kenttä on tarkoitettu ihan eri asiaan: se kertoo onko UUDEN ostoksen liittäminen TÄHÄN samaan tilaukseen vielä mahdollista (`cart.ts`:n 6h-yhdistämisikkuna, LUKITTU-sääntö "sama myyjä + 6h = yksi tilaus"). Kun maksuaika (`paymentDeadline`) nostettiin 2h:sta 12h:iin 2026-09-11 (ks. "Maksuaika 2h → 12h kaikkialla" -osio), `select-shipping`-reittiä ei päivitetty samalla — se jäi yhä tarkistamaan vanhaa 6h-kenttää. Seuraus: **6-12h ikkunassa tilauksen luonnista ostaja näki yhä olevan maksuaikaa jäljellä, mutta EI PYSTYNYT enää valitsemaan toimitustapaa lainkaan** (`400 "Yhdistetyn lähetyksen 6h ikkuna on umpeutunut"`) — ja koska `POST /orders/:id/pay` vaatii `shippingPrice`:n olevan jo asetettuna (`initiateCheckout()`:n oma tarkistus), ostaja ei koskaan päässyt edes yrittämään maksaa. Täysin hiljainen lukkiutuminen, ei mitään virhettä joka olisi näkynyt lokeissa (käsitelty 400-vastaus, ei console.error).

**Löydetty konkreettinen tapaus:** tilaus luotu 2026-09-12 klo 05:16 (`shippingWindowEnd` 11:16, `paymentDeadline` 17:16) — tarkistushetkellä klo 15:13 ostaja oli ollut jumissa jo n. 4 tuntia, vaikka maksuaikaa olisi ollut vielä ~2h jäljellä.

**Korjaus:** `select-shipping`-reitin tarkistus vaihdettu käyttämään `order.paymentDeadline`:ia `order.shippingWindowEnd`:n sijaan — oikea, ainoa relevantti takaraja toimitustavan valinnalle on koko tilauksen maksudeadline, ei erillinen merge-ikkuna. `shippingWindowEnd`-kenttä jätetty koskemattomaksi omaan alkuperäiseen tarkoitukseensa (`cart.ts`:n uuden ostoksen yhdistämispäätös).

**Testattu tuotannossa oikealla, aiemmin jumissa olleella tilauksella (ei synteettisellä testidatalla):** `POST /orders/:id/select-shipping` (postitus, tuotteella `allowPickup:false` joten ainoa sallittu vaihtoehto) → `200`, `shippingPrice:7.9` tallentui oikein. `POST /orders/:id/pay` heti perään → `200`, oikea `checkout.stripe.com`-uudelleenohjausosoite (live-tila) — sessio expiroitu heti testin jälkeen `stripe.checkout.sessions.expire()`:lla (ei vaikuta seuraavaan oikeaan maksuyritykseen, koska `initiateCheckout()` luo aina tuoreen session joka kutsulla, ei koskaan käytä uudelleen vanhaa `stripeSessionId`:ta). Ostaja pääsee siis nyt oikeasti maksamaan.
...
**1. Alaraja — KORJATTU SAMANA PÄIVÄNÄ: väärinymmärretty ensin, oikea sääntö on min 70% pyynnistä (enintään 30% alennusta), ei min 30%.** Omistaja täsmensi heti: "minimissään 70% pyynnistä pitää tarjota, eli max 30% alennusta voi yrittää". `POST /offers` (`backend/src/routes/offers.ts`) laski aiemmin vain kiinteän Stripe-alarajan (0,50€), ei mitenkään suhteessa tuotteen hintaan. `MIN_OFFER_PERCENT` asetettu lopulta `0.7`:ksi (ensimmäinen versio oli virheellisesti `0.3`, korjattu heti samana päivänä) — alaraja on `Math.max(0.5€, 70% × Product.startPrice)` (`startPrice` on buy_now-tuotteen kiinteä myyntihinta, ks. `dashboard/tuotteet`:n "salePriceLabel" — eri merkitys kuin auction-tuotteilla, mutta `POST /offers` on jo rajattu vain `saleType:'buy_now'`:lle). Sama laskukaava peilattu frontendiin (`tuotteet/[id]/page.tsx`) sekä esikatselu-vihjeeksi tarjouskentän alle ("Väh. X€ (70% pyyntihinnasta)") että asiakaspuolen nopeaksi validoinniksi ennen palvelinkutsua — palvelin on silti aina lopullinen totuus. **Testattu uudelleen korjauksen jälkeen:** 100€:n tuotteella 69,99€ ja vanha (väärä) 30€ hylätty oikein, täsmälleen 70,00€ hyväksytty.
...
**Testattu oikealla API-ketjulla tuotantoa vasten, kahdessa erässä (alkuperäinen 30%-versio, sitten korjattu 70%-versio):** luotu 100€:n buy_now-testituote → alkuperäisellä 30%-versiolla tarjous 29,99€ hylätty (`400`) ja 30,00€ hyväksytty (`201`); korjauksen jälkeen sama tuote uudelleen: 69,99€ JA vanha 30€ molemmat hylätty oikein (`400`), täsmälleen 70,00€ hyväksytty (`201`). Myyjän saama `OFFER_RECEIVED`-ilmoituksen `link`-kenttä vahvistettu olevan `/dashboard/tarjoukset`. Kaikki testidata siivottu joka kerta, testiuserin `verified`-tila aina palautettu.
...
**Juurisyy löytyi koodista:** `/lahetys`-sivun `checkForActiveShow()` (mount-hetkellä ja takaisin-navigoinnissa ajettava tarkistus, ks. aiempi "KRIITTINEN: selaimen 'edellinen sivu' -navigointi" -korjaus) haki myyjän lähetykset ja kohteli SCHEDULED- ja LIVE-tilaisia lähetyksiä täsmälleen samalla tavalla — kummassakin tapauksessa `setIsLive(true)` asetettiin heti, mikä hyppää suoraan täyden konsolin renderöintiin (`HlsPreview`-videokomponentti + kaikki huutokauppa-/chat-overlayt). Tämä on oikein LIVE-tilaiselle lähetykselle (myyjä striimaa jo oikeasti, konsoliin pitää päästä suoraan takaisin). **Mutta SCHEDULED-tilainen lähetys joka on luotu DASHBOARDIN ajastuslomakkeella** (ei `/lahetys`-sivun oman "Luo lähetys ja testaa yhteys" -napin kautta) **ei ole koskaan aloittanut mitään julkaisua mihinkään** — kameraa ei ole testattu, OBS ei ole yhdistetty. Konsoliin hyppääminen suoraan näytti siis tyhjää videokuvaa, koska mikään lähde ei koskaan alkanut lähettää sinne dataa.

**Korjaus:** `checkForActiveShow()` erotettu kahdeksi haaraksi. `status === 'LIVE'` toimii kuten ennen (suora `isLive=true`). `status === 'SCHEDULED'` sen sijaan **ei enää hyppää konsoliin** — uusi `pendingShowId`-tila tallentaa olemassa olevan lähetyksen ID:n, ja esikatselu-/asetusnäkymä (`!isLive`, sama näkymä jota "Luo lähetys ja testaa yhteys" -virta jo käyttää) esitäytetään olemassa olevan lähetyksen nimellä/kategorialla/kaupungilla — nämä kentät ovat nyt myös lukittuja (`disabled`) koska backendillä ei ole PATCH-reittiä lähetyksen tietojen päivitykseen (vain `POST /shows` luontiin ja `PATCH /shows/:id/status` tilanvaihtoon), joten muokkaus täällä ei koskaan olisi tallentunut mihinkään. Näkyvä vihreä banneri ("Tämä lähetys on jo ajastettu...") kertoo tästä. Myyjä testaa kameran/OBS:n normaalisti (sama `!camReady`-esto kuin uudella lähetyksellä), ja painike ("Jatka ajastettuun lähetykseen", uusi `sc.continueScheduledBtn`) kutsuu `createShow()`:ia — joka nyt tarkistaa `pendingShowId`:n ja, jos asetettu, **ei luo uutta Show-riviä** vaan käyttää olemassa olevaa ID:tä siirtyessään `isLive=true`:hen. `cameraTestRequired`-virheteksti yleistetty ("...ennen kuin voit jatkaa" aiemman "...ennen kuin voit luoda lähetyksen" sijaan) koska sama teksti näkyy nyt molemmissa tapauksissa. `pendingShowId` nollataan myös `doEndShow()`:ssa.

Typecheck+build vihreä, deployattu. **Ei visuaalisesti vahvistettu selaimessa** (ei selaintyökalua tässä ympäristössä) — omistajan kannattaa testata: ajasta lähetys dashboardista, klikkaa se auki, vahvista että näkyviin tulee kamera-/OBS-esikatselunäkymä (ei tyhjä musta konsoli) ja että "Jatka ajastettuun lähetykseen" -painikkeen jälkeen konsoli toimii normaalisti.
...
**Ensimmäinen korjausyritys (natiivi `<input type="time">`) osoittautui vääräksi, korjattu heti samana päivänä omistajan palautteella: "ei saa olla noita am/pm, elämme Suomessa emmekä käytä niitä".** Natiivi time-input muotoilee itsensä KÄVIJÄN selaimen/käyttöjärjestelmän paikallisasetuksen mukaan, ei sivun kielen (`t.xxx`) mukaan — jos kävijän laite on esim. en-US-lokaalilla, kenttä näyttäisi AM/PM-muodon riippumatta siitä että koko sivu on suomeksi. Tätä ei voi luotettavasti pakottaa 24h-muotoon pelkällä `lang`-atribuutilla kaikissa selaimissa.

**Lopullinen korjaus:** palautettu kaksi `<select>`-laatikkoa (tunnit 00-23, minuutit 00/15/30/45) — oma, aina-24h-muotoinen valitsin ei riipu koskaan kävijän laitteen lokaalista, joten AM/PM ei voi koskaan ilmestyä. Alkuperäinen kapeus-bugi (jonka takia ensin kokeiltiin native time-inputia) korjattu suoraan: molemmilla selecteillä nyt kiinteä `width:76px` + `flexShrink:0` + `fontVariantNumeric:'tabular-nums'` jaetun `width:'100%'`-tyylin sijaan, joka aiemmin sai molemmat laatikot pyytämään yhteensä 200% tilaa samalla flex-rivillä ja kutistumaan liian kapeiksi.

Typecheck+build vihreä molemmilla kierroksilla, deployattu. Ei koskettu backendiä — `showApi.create()`:n `scheduledAt`-kenttä ottaa edelleen pelkän ISO-yhteensopivan merkkijonon (`${date}T${hour}:${minute}`), muutos oli puhtaasti frontendin syöttötavassa.
...
1. **Jäljellä oleva määrä näkyviin.** `ShowProduct`-rajapintaan lisätty `saleType`/`quantity`. `ShopPanel`-rivit näyttävät nyt "{t.live.remainingStock}" (esim. "12 kpl jäljellä") kun `quantity > 1` — piilossa yksittäiskappaleilla, koska "1 kpl jäljellä" on turha oletusarvo.
2. **Määrän valinta ostettaessa.** `buyNow()` ottaa nyt valinnaisen `quantity`-parametrin (oletus 1, taaksepäinyhteensopiva). Uusi jaettu `buyQty: Record<string, number>` -tila (`LivePage`-komponentin juuressa, avaimena tuotteen ID) välitetään sekä `ShopPanel`iin (rivikohtainen +/- -stepperi "Osta heti" -napin viereen, näkyy kun `quantity > 1`) että `BidPanel`iin (sama periaate pääkohteen näkymässä).
3. **Pääkohteen näkymä eriytetty huutokaupasta.** `BidPanel` haarautuu nyt eksplisiittisesti `currentProduct.saleType === 'buy_now'`:n perusteella — aiemmin `currentProduct` saattoi olla `products.find(...) ?? products[0]`-fallbackin kautta puhdas suoramyyntituote, jolloin paneeli näytti silti aina huutokauppasuuntautunutta tekstiä (nykyinen huuto, johtajan nimi, huutosumma-stepperi, slide-to-bid, ja erityisesti `t.live.waitAuction` "Odota huutokauppaa" kun mikään huuto ei ollut käynnissä) — tämä oli juurisyy raportoituun bugiin. `buy_now`-haara on täysin oma, kevyt näkymä: kuva/nimi/kunto-rivi + hinta, sitten "{t.live.remainingStock}" + sama +/- -stepperi (kun `quantity > 1`) + "{t.live.buyNowQty}" ("Osta heti N kpl") -nappi joka kutsuu `onBuyNow(currentProduct.id, selected)`. Ei LOT-numeroa, ei nykyistä huutoa, ei ajastinta — mikään ei viittaa huutokauppaan. Kaikki muut myyntitavat (`'live'`/`'auction'`/`'both'`) käyttävät ennallaan alkuperäistä auktio-näkymää, koska niissä huutaminen on aidosti mahdollista.
- Uudet i18n-avaimet `t.live.remainingStock`/`t.live.buyNowQty` (`{n}`-templaatilla) lisätty kaikkiin kolmeen kieleen.

**Testattu:** typecheck+build vihreä molemmilla ajolla, deployattu. **Ei vielä visuaalisesti vahvistettu selaimessa oikealla 150 kpl:n suoramyyntituotteella livessä** (ei selaintyökalua tässä ympäristössä, ei myöskään pystytty ajamaan täyttä OBS+live-simulaatiota tästä ympäristöstä) — omistajan pyytämä testi (listaa 150 kpl suoramyyntituote, aloita live, tarkista määrän näkyminen ja 5 kpl:n osto ilman auktio-tekstejä) jää omistajan itse tarkistettavaksi.

## Yhdistetty ostoskorimaksu — usea myyjä, yksi Stripe-maksu 2026-09-11 — ✅ TEHTY JA DEPLOYATTU, EI VIELÄ OIKEALLA MAKSULLA TESTATTU

**Isoin yksittäinen arkkitehtuurimuutos maksujärjestelmään Stripe-siirron jälkeen.** Aiemmin ostoskori jossa oli tuotteita useammalta myyjältä vaati ostajan maksavan JOKAISEN myyjän erikseen, yksi Stripe-uudelleenohjaus kerrallaan (`/kori`:n `payGroup()` per myyjäryhmä). Omistajan pyynnöstä yhdistetty YHDEKSI Stripe Checkout Sessioniksi — ostaja näkee koko ostoskorin yhteissumman (per myyjä eriteltynä) ja maksaa kerralla, jonka jälkeen raha jaetaan myyjille erikseen.

**⚠️ Arkkitehtuurin ydinmuutos, vahvistettu suoraan Stripen dokumentaatiosta ennen koodausta (ei arvattu):** Stripen "destination charge" -malli (`payment_intent_data.transfer_data.destination`, jota koko Stripe-integraatio on käyttänyt 2026-09-09 lähtien) tukee VAIN YHTÄ kohdetiliä per PaymentIntent — ei riitä kun samassa maksussa on useamman myyjän tuotteita. Vaihdettu Stripen **"separate charges and transfers"** -malliin (`docs.stripe.com/connect/separate-charges-and-transfers`, WebFetch-haettu ja luettu kokonaan ennen koodausta): koko summa laskeutuu ensin Habahubin OMALLE Stripe-saldolle (ei `transfer_data`/`application_fee_amount` enää ollenkaan Checkout Sessionissa), ja VASTA `checkout.session.completed`-webhookissa luodaan ERILLINEN `stripe.transfers.create()`-kutsu jokaiselle myyjälle. Stripen dokumentaatio vahvisti suoraan kriittisen yksityiskohdan joka olisi muuten helposti mennyt väärin: `source_transaction`-parametri (alkuperäisen chargen ID, `paymentIntent.latest_charge`) sallii useamman Transferin jakaa saman chargen summan ilman että transfer epäonnistuisi saldon "vapautumista" odotellessa — "You can create multiple transfers with the same source_transaction, as long as the sum of the transfers doesn't exceed the source charge" ja "the transfer request returns success regardless of your available balance if the related charge hasn't settled yet".

**Toteutus, vaiheittain (kuten omistaja pyysi):**
1. **`backend/src/lib/stripe.ts`:** `createCheckoutSession()` ottaa nyt `orders: CheckoutOrderInput[]`-taulukon yhden Orderin sijaan — rakentaa yhden yhdistetyn `line_items`-listan (tuotteet per Order + oma toimitusrivi per Order, nimetty `"Toimitus – {myyjä}"` kun mukana on useampi myyjä muuten pelkkä `"Toimitus"`) + yhden `Maksunkäsittelymaksu`-rivin koko summasta. `success_url`/`cancel_url` eivät enää sisällä `orderId`:tä — vahvistettu ettei `/ostot`-sivu koskaan lukenut sitä (vain `?payment=success/cancel`), joten se oli aina pelkkä koriste, ei olisi edes voinut valita yhtä orderId:tä mielekkäästi useamman joukosta. Uudet `getLatestChargeId()` (hakee `PaymentIntent.latest_charge`) ja `createSellerTransfer()` (yksi Transfer, `source_transaction`-sidottu) -apufunktiot.
2. **`refundPayment()` kirjoitettu kokonaan uusiksi** — tämä oli pakollinen seuraus, ei omistajan alkuperäisessä listassa mutta välttämätön: vanha `reverse_transfer:true`/`refund_application_fee:true` toimii VAIN destination-chargelle, Stripen oma dokumentaatio sanoo suoraan "refunding a charge has no impact on any associated transfers" uudessa mallissa. `refundAmountEuros` on nyt AINA pakollinen (ei koskaan "koko PaymentIntent" -oletus, koska PaymentIntent voi kattaa useamman Orderin — vahingossa toisen myyjän rahojen hyvittäminen olisi ollut mahdollista muuten). **Taaksepäinyhteensopivuus vanhoille tilauksille säilytetty:** jos `Order.stripeTransferId` on `null` (maksettu ENNEN tätä muutosta, destination-chargella), käytetään yhä vanhaa `reverse_transfer`/`refund_application_fee`-polkua — uusille tilauksille (transferId asetettu) erillinen `stripe.transfers.createReversal()` + tavallinen `refunds.create()`. Osittaishyvitys laskee suhteellisen transferin perumisen manuaalisesti (Stripe ei enää tee tätä automaattisesti) — tämä polku ei ole minkään nykyisen UI:n käyttämä (dashboard/tilaukset:n "Hyvitä"-nappi tekee aina koko tilauksen hyvityksen), mutta pidetty oikeana ettei jää piilevää bugia.
3. **`backend/src/routes/orders.ts`:** uusi jaettu `initiateCheckout()`-apufunktio KAIKKI-TAI-EI-MITÄÄN-validoinnille (jos yksikin tilaus epäonnistuu, ei luoda Stripe-sessiota lainkaan) — käytössä sekä vanhassa `POST /orders/:id/pay`:ssä (säilytetty ennallaan `/ostot`-sivun yksittäisille maksuille, esim. huutokauppavoitot) että uudessa `POST /orders/pay-multiple`:ssa (`/kori`:n yhdistetty maksu). Komissio lasketaan ERIKSEEN per Order/myyjä (jokaisella oma `customCommissionRate`/`Cap`/14pv-promon tila) ja tallennetaan `Order.commissionCents`:iin per rivi ennen Stripe-kutsua.
4. **`backend/src/routes/webhooks.ts`:** `checkout.session.completed` hakee nyt kaikki tähän sessioniin liittyvät Orderit `stripeSessionId`:n kautta (ei enää `session.metadata.orderId`) — toimii identtisesti sekä yhden että usean Orderin tapauksessa. Jokainen Order käsitellään ERIKSEEN omassa try/catch-lohkossaan: jos yhden myyjän Transfer epäonnistuu, se EI estä muiden onnistumista — epäonnistunut Order jää `PENDING_PAYMENT`-tilaan, admin saa ilmoituksen (uusi `NotificationType.ADMIN_TRANSFER_FAILED`) ja webhook palauttaa ei-200:n jotta Stripe yrittää koko tapahtumaa uudelleen (idempotenssin ansiosta re-yritys koskee enää vain sitä yhtä jumissa olevaa Orderia, muut on jo merkitty `PENDING_SHIPPING`:ksi eikä niitä käsitellä toiseen kertaan).
5. **Schema:** uusi `Order.stripeTransferId` (tämän Orderin oma Transfer-ID, tarvitaan hyvityksiin) + `NotificationType.ADMIN_TRANSFER_FAILED`. `Order.stripePaymentIntentId` voi nyt olla SAMA useammalla Orderilla (yhdistetty maksu).
6. **`frontend/lib/api.ts` + `frontend/app/kori/page.tsx`:** uusi `orderApi.payMultiple(orderIds)`. Per-myyjä "Maksa"-napit poistettu kokonaan, korvattu YHDELLÄ "Maksa kaikki" -napilla alimmassa yhteissumma-palkissa — käy läpi kaikki myyjäryhmät peräkkäin (`cartApi.checkout` + `selectShipping` per ryhmä, kuten ennenkin), kerää kaikki syntyneet orderId:t, kutsuu `payMultiple()`:ia kerran. Postitus/nouto pysyy per-myyjä-valintana kuten pyydettiin — per-myyjä hintaerittely (tuotteet+toimitus+maksunkäsittely per ryhmä) säilytetty ennallaan, vain nappi poistui. Painike pois käytöstä jos yksikin ryhmä on aidosti ristiriitainen toimitustavoiltaan (`hasDeliveryConflict`, sama olemassa oleva reunatapaus kuin ennenkin).

**⚠️ TypeScript-sudenkuoppa löytyi ja korjattiin kesken työn:** tämän projektin `tsconfig.json`:ssa on `"strict": false` — vahvistettu erillisellä minimitoistolla (`/tmp/narrow_test.ts`) että TÄSSÄ nimenomaisessa asetuksessa TypeScriptin control-flow-narrowing EI toimi luotettavasti boolean-literaalidiskriminantilla (`{ok:true;...} | {ok:false;...}` + `if (!result.ok)`), vaikka sama koodi toimisi oikein `strict:true`:lla. `initiateCheckout()`:n paluuarvo kirjoitettiin sen sijaan nollattavina kenttinä (`{redirectUrl: string|null, error: string|null, status: number}`) + pelkkä totuusarvotarkistus (`if (result.error)`), joka ei riipu narrowing-ominaisuudesta ollenkaan.

**Testattu tuotannossa deployn jälkeen (turvallisesti, ilman rahan liikuttamista):** `prisma db push` ajettu ensin (puhdas lisäys — uusi kenttä + uusi enum-arvo, ei koskenut olemassa olevaan dataan). Kertakäyttöisellä diagnostiikkaskriptillä (`dist/lib/stripe.js`:n `createCheckoutSession()` suoraan, poistettu heti käytön jälkeen) luotiin OIKEA Stripe Checkout Session LIVE-avaimella kahdella synteettisellä myyjällä (eri tuotenimet, yksi toimitusmaksullinen, toinen ilman) — Stripe hyväksyi yhdistetyn `line_items`-rakenteen ilman `transfer_data`/`application_fee_amount`-parametreja täysin ongelmitta, palautti oikean `cs_live_...`-session-ID:n ja toimivan `checkout.stripe.com`-uudelleenohjausosoitteen. Sessio EI koskaan avattu selaimessa eikä maksettu — vahvistettu heti perään `stripe.checkout.sessions.expire()`:lla ettei se jää roikkumaan. `getLatestChargeId`/`createSellerTransfer` vahvistettu oikein vietynä funktioina (`typeof === 'function'`).

**⚠️ EI TESTATTU, tarkoituksella (sama varovaisuusperiaate kuin projektin muillakin live-rahaa liikuttavilla muutoksilla, ks. esim. "Paytrail -> Stripe Connect" -osio): oikea, päästä-päähän suoritettu usean myyjän yhdistetty maksu.** `stripe.transfers.create({source_transaction})` vaatii OIKEAN, onnistuneen chargen ID:n toimiakseen mielekkäästi — sen synnyttäminen turvallisesti vaatisi oikean maksun, mikä liikuttaisi oikeaa rahaa. Tätä ei automatisoida. **Suositus omistajalle ensimmäiseksi oikeaksi testiksi:** osta kaksi tuotetta kahdelta ERI myyjältä (esim. omilla testitileillä, molemmilla oikea Stripe-onboarding tehtynä) samaan ostoskoriin, valitse toimitustavat molemmille, paina "Maksa kaikki", maksa oikeasti pienellä summalla — tarkista jälkikäteen Stripe-dashboardista että (a) yksi charge syntyi koko summasta, (b) kaksi erillistä Transfer-objektia syntyi oikeilla summilla molemmille myyjätileille, (c) molemmat Orderit siirtyivät `PENDING_SHIPPING`-tilaan tietokannassa. Jos jompikumpi Transfer epäonnistuisi, admin saa siitä heti ilmoituksen `/ilmoitukset`-sivulle.

**Sivuhuomio:** `git`-historiassa oli päässyt syntymään pieni jälkijättöisyys — `backend/dist/`-kansio on tarkoituksella git-seurattu tässä projektissa (ei `.gitignore`ssa), mutta muutaman aiemman istunnon aikana lähdekoodin (`src/`) muutokset committoitiin ilman vastaavaa `npm run build`+commit-kierrosta `dist/`:lle, jolloin palvelimen oma `git pull` alkoi törmätä paikallisiin (palvelimen deployn aikana syntyneisiin) `dist/`-muutoksiin. Korjattu tämän deployn yhteydessä (`git checkout -- backend/dist/` palvelimella ennen pullia, koska ne ovat pelkkää generoitua koodia — turvallista hylätä ja rakentaa uudelleen) ja tästä eteenpäin `dist/` committoidaan aina saman commitin mukana kuin vastaava `src/`-muutos.
...
**1. Pakettikoon valinta poistettu lähetyksen luontivaiheesta.** Myyjä valitsi aiemmin PIENI/ISO-pakettikoon `dashboard/tilaukset`-sivulla juuri ennen Posti-lähetyksen luontia (`POST /orders/:id/create-shipment`) — kaksi eri `serviceId`:tä (PO2102/PO2103) samalle `packageCode`:lle ("PKT"). **Vahvistettu koodikommenteista ennen poistoa: PO2102 (PIENI) oli aina vain vahvistamaton arvaus** — Postin oma curl-esimerkki käytti vain ISO:a (PO2103), ja koko flow todistettiin toimivaksi päästä päähän 2026-09-04 nimenomaan ISO-konfiguraatiolla. **Poistettu kokonaan, korvattu yhdellä kiinteällä konfiguraatiolla** (`backend/src/lib/postiClient.ts`: `POSTI_SERVICE_ID='PO2103'`, `POSTI_PACKAGE_CODE='PKT'`, `POSTI_WEIGHT_KG=5`, kahden `Record<Pakettikoko,...>`-taulukon sijaan). `create-shipment`-reitti ei enää lue/vaadi `pakettikoko`-kenttää pyynnön bodystä, kirjoittaa aina `'ISO'`:n `Order.pakettikoko`-kenttään. Frontendin `<select>`-valitsin poistettu kokonaan `dashboard/tilaukset`-sivulta, `orderApi.createShipment()` ei enää ota toista parametria. **`Order.pakettikoko`-enum jätetty schemaan koskemattomana** (vanhat PIENI-arvot pysyvät historiadatassa) — sama "ei pudoteta historiadatan takia" -periaate kuin muuallakin projektissa. Orvoiksi jääneet `packageSizeSmall`/`packageSizeLarge`-i18n-avaimet poistettu (fi/en/sv).

**2. Postitusmaksu 6,90€ → 7,90€.** Päivitetty molemmissa lähdetaulukoissa (`backend/src/lib/shipping.ts`:n `PAKETTIKOOT` — palvelinpuolinen totuuden lähde, ei koskaan luoteta clientiltä — ja sen frontend-peilikuvassa `frontend/lib/pakettikoot.ts`, käytetty vain näyttöön ennen maksua) — kaikki muu koodi (`/kori`, `/ostot`, `Order.shippingPrice`) laskee hinnan näiden kautta `getShippingPrice()`:llä, ei vaatinut muita koodimuutoksia. Tekstisisältö päivitetty: FAQ (fi/en/sv, `frontend/app/faq/page.tsx`) ja `valityspalkkiot`-sivun esimerkkitaulukko. **Ei muutettu käyttöehtoihin** — tarkistettu ettei `kayttoehdot/content.ts` mainitse tarkkaa euromäärää missään kohtaa (vain yleisluontoisia "toimitusmaksu"-mainintoja), joten siellä ei ollut mitään päivitettävää. Historialliset koodikommentit jotka kuvaavat 2026-09-10 rahanjako-bugin korjausta (`stripe.ts`) jätetty koskemattomiksi tarkoituksella — ne kuvaavat mitä silloin todella tapahtui 6,90€:lla, muuttaminen vääristäisi historiaa.

**3. Mainostila muutettu karuselliksi.** Aiemmin `AdSlot` oli aina täsmälleen yksi upsertattu rivi (`id:"main"`) — nyt tavallinen monirivinen taulukko (`id String @id @default(cuid())`, uusi `createdAt`-kenttä järjestystä varten). Admin voi luoda/muokata/poistaa minkä tahansa määrän mainoksia `/admin`-paneelin "Mainos"-välilehdeltä, kukin itsenäisesti päälle/pois kytkettävissä — `AdminAdManagement.tsx` kirjoitettu uudelleen listamuotoiseksi (`AdEditor`-alikomponentti per mainos, oma tallenna/poista-tila kullekin, `ConfirmDialog` poistoa varten). Backend: `GET /ad` (julkinen) palauttaa nyt taulukon (`enabled:true`-rivit, `createdAt asc`) yhden objektin/`null`:in sijaan; `GET /admin/ad` listaa kaikki; uudet `POST /admin/ad` (luo tyhjän, `enabled:false`-oletus) ja `DELETE /admin/ad/:id`; `PATCH /admin/ad` muuttui `PATCH /admin/ad/:id`:ksi. **`AdBanner`-komponentti (`frontend/app/page.tsx`) pyörittää mainokset automaattisesti 6s välein** (`setInterval`, moduloi indeksiä), CSS-läpihäivytys (`hb-ad-fade`-luokka `globals.css`:ssä, `key={ad.id}`-pohjainen remount käynnistää animaation uudestaan joka vaihdolla, kunnioittaa `prefers-reduced-motion`) + klikattavat pisteosoittimet alareunassa kun mainoksia on useampi kuin yksi. Yhden mainoksen tapauksessa karuselli ei tee mitään havaittavaa (sama vanha käytös). **Vanha tuotannon "main"-mainos säilyi ennallaan migraation yli** (id-arvo ei muutu retroaktiivisesti, vain UUSIEN rivien oletus vaihtui) — vahvistettu `prisma db push`:n jälkeen `GET /ad`:llä, sisältö (yritysmainonta-CTA, video) täsmälleen sama kuin ennen.

**Testattu tuotannossa deployn jälkeen:** kertakäyttöisellä admin-JWT:llä täysi CRUD-kierros (`GET /admin/ad` → `POST` uusi tyhjä mainos → `PATCH` otsikolla+`enabled:true` → vahvistettu ilmestyvän julkiseen `GET /ad`:hen → `DELETE` → vahvistettu katoavan julkisesta listasta) — kaikki onnistuivat, testimainos siivottu pois. `prisma db push` ajettu tuotantoon ennen rebuildia, ei data-katkoa (pelkkä lisäys: uusi `createdAt`-sarake + `id`:n oletusarvon generointitavan vaihto, ei koske olemassa olevia rivejä). Typecheck+build vihreä molemmilla puolilla. **Ei testattu erikseen:** täyttä osto/lähetys-polkua uudella 7,90€:n hinnalla tai uudella kiinteällä pakettikoolla oikealla Posti-lähetyksellä — molemmat ovat pieniä, suoraviivaisia arvomuutoksia jo aiemmin päästä-päähän-vahvistetun koodin sisällä, ei arvattu mitään uutta.
...
2. **"Aloita julkinen lähetys" oma, isompi tyylinsä.** Uusi `goPublicBtnStyle`-vakio (isompi padding, `fontSize:14`, `fontWeight:800`, kirkas varjo `${GREEN}66`) korvaa aiemman `{...pillBtn, background: GREEN_DIM}`-kierrätyksen, joka teki napista visuaalisesti samanarvoisen kuin "Jaa striimi"/"Moderointi"/"OBS" — vaikka se on konsolin tärkein yksittäinen painallus (tekee lähetyksestä julkisen).
3. **Huutokaupan aikalaskuri ei enää katoa yläpalkin taakse.** Etsitty koko koodikannasta ("jäljellä"-tekstillä, `t.selaa.timeLeft`/`t.purchases.remainingSuffix`) — ainoa relevantti näyttö myyjän puolella on `/lahetys`:n oma kellumaan aseteltu huutoajastin (`{fmt(auction.timer)}`), joka käytti kiinteää `top: isMobile?52:60`-arvausta yläpalkin (LIVE/ESIKATSELU + tilastot + Jaa/Moderointi/OBS/Aloita julkinen lähetys/Lopeta -napit) korkeudesta. Kun napit kääriytyivät kahdelle riville kapealla näytöllä, yläpalkin (zIndex 10) todellinen korkeus ylitti arvauksen ja peitti ajastimen (zIndex 5) alleen. **Korjaus samalla periaatteella kuin aiempi `bottomBarHeight`-korjaus (ks. "Uudet löydökset 2026-08-13, osa 4" kohta 1):** uusi `topBarRef`+`ResizeObserver`-mitattu `topBarHeight`, ajastin sijoitetaan `top: topBarHeight + 8`:aan ja nostettu `zIndex: 11`:een (yläpalkin ylle), ei enää arvaa.
4. **Jono-paneelin avausnappi suurennettu ja väritetty.** `padding:'10px 6px'`→`'18px 11px'`, `fontSize:11`→`13`, `fontWeight:700`→`800`, tausta `rgba(0,0,0,0.6)`→`GREEN_DIM` (sama vihreä jota koodi jo käyttää mm. "Aloita huutokauppa"/"Lisää tuote" -napeissa) + kevyt varjo. Napin teksti vaihdettu myös käyttämään `t.streamConsole.queueLabel`:ia — oli jäänyt kääntämättömäksi aiemmasta i18n-läpikäynnistä (vain paneelin sisäinen "Jono (N)" -otsikko oli käännetty, ei tätä ulompaa avausnappia).

Typecheck+build vihreä molemmilla puolilla, deployattu. Ei visuaalisesti vahvistettu selaimessa (ei selaintyökalua tässä ympäristössä) — omistajan kannattaa tarkistaa mobiililla.

## Koko sivuston i18n-läpikäynti käynnissä 2026-09-10 — käyttäjä listaa kovakoodatut kohdat, korjataan sivu kerrallaan

Omistaja aloitti systemaattisen läpikäynnin: "nyt rupean listaamaan kohtia joissa käännökset eivät toimi, varmaan kovakoodattu" — pitkä lista puuttuvia/rikkinäisiä käännöksiä etusivulta, Ilmoitukset-, Live- (`/lahetys`), Dashboard-, Tilaukset-, Tilitykset- ja Profiili-sivuilta. Tämä on isompi, monivaiheinen urakka — päivitetään tätä osiota sitä mukaa kun sivuja valmistuu.

**✅ TEHTY JA DEPLOYATTU:**
- **Etusivu:** `allKats`-objekti (kategoriavalitsin) käytti kovakoodattua `{fi:'Kaikki',en:'All'}`:tä sv-kielelle väärin — korjattu käyttämään `t.selaa.allCategories`:ia kaikissa kolmessa kielessä. Kirjaimellinen `<span>Kaikki</span>` → `{t.selaa.allCategories}`. Uusi `t.home.emptyCategory`-avain ("Ei tuotteita tai lähetyksiä tässä kategoriassa") korvasi kovakoodatun suomen. `Näytä kaikki` → olemassa oleva `t.home.showAll`, ei ollut vain kytketty.
- **`frontend/lib/kategoriat.ts` — juurisyy suurimmalle osalle "Keräilykortit / Släbit / Sealed / Irtokortit / Tarvikkeet / Pokémon / Magic / Yu-Gi-Oh! / Lorcana / One Piece / Urheilukortit / Muut keräilytuotteet" -listasta.** Koko tiedostosta puuttui `sv`-kieli kokonaan jokaisesta `nimi: {fi, en}` -objektista (14 pääkategoriaa + kaikki alakategoriat + Keräilykorttien peli-/tyyppitasot, ~80+ kohtaa) — `Lang`-tyyppi oli myös vain `'fi' | 'en'`, ei koskaan tukenut `'sv'`:tä ollenkaan tällä tiedostolla vaikka muu sivusto on tukenut ruotsia jo pitkään. Korjattu: `sv`-nimi lisätty joka ikiseen kohtaan, `Lang`-tyyppi levennetty `'fi' | 'en' | 'sv'`:ksi. `getKatNimi`/`getAlaNimi`/`getTyyppiNimi`-apufunktiot käyttävät jo `nimi[lang] ?? nimi.fi`-fallbackia, ei vaatinut muutosta niihin — vain data puuttui.
  - **Huom, tarkoituksellinen poikkeus, ei kirjoitusvirhe:** "Släbit" on jätetty samaksi suomeksi kaikissa kolmessa kielessä — vakiintunut yhteisötermi (ei "Slabs"/"Slaber"), sama kuin muuallakin sivustolla.
- **Dashboard-etusivu:** `+ Ajasta` -nappi käytti kovakoodattua tekstiä vaikka `t.dashboard.schedule`-avain oli jo olemassa kaikissa kolmessa kielessä valmiiksi — vain unohtunut kytkeä. Yksi rivi.
- **Dashboard/Tuotteet:** `Tuo CSV/TXT-tiedostosta` sama tilanne — lisätty uusi `t.dashboardProducts.bulkImportLink`-avain (ensimmäinen yritys viittasi vahingossa väärään nimiavaruuteen, `presetsPage.bulkImport`, TypeScript nappasi virheen buildissa).
- **`/dashboard/tilitykset` (Tilitykset) — koko sivu käännetty.** Uusi `t.payoutsPage`-nimiavaruus (otsikko, alaotsikko, taulukon sarakeotsikot, tilabadget PENDING_SHIPPING/SHIPPED/DELIVERED/DISPUTED, tyhjä tila, footer-huomautus). Päivämäärät nyt kieliriippuvia (`lang==='en'?'en-GB':lang==='sv'?'sv-SE':'fi-FI'`) aiemman aina-`'fi-FI'`:n sijaan.
- **`components/StripeConnectCard.tsx` (jaettu, näkyy sekä Tilitykset- että Profiili-sivulla) — käännetty.** Uusi `t.stripeCard`-nimiavaruus (tila-tekstit, napit, virheviestit).
- **`/dashboard/profiili` (Profiili) — koko sivu käännetty.** Uusi `t.profilePage`-nimiavaruus (~35 avainta: perustiedot, käyttäjänimen vaihtorajoitus templaattina `{date}`:lla, yhteystiedot, Lomamoodi, Uutiskirje, kaikki painikkeet/virheviestit). Tämä poisti aiemman, koodikommentissa dokumentoidun "tarkoituksellisen poikkeuksen" (sivu käytti hardkoodattua suomea koska "ei tuoda i18n:ää vain tähän yhteen kohtaan") — omistajan uusi pyyntö korvaa aiemman päätöksen.
- **`/dashboard/tilaukset` (Tilaukset, myyjän tilaushallinta) — koko sivu käännetty.** Uusi `t.salesPage`-nimiavaruus (~40 avainta: sarakeotsikot Odottaa maksua/Uudet tilaukset/Lähetetty/Toimitetut, tyhjät tilat, Hyvitä/Vahvista nouto/Lisää seurantakoodi/Luo lähetys -painikkeet, pakettikoko-valinnat, badget Tarra/Matkalla/✓ Valmis, arvostelulomake, virheviestit, hyvityksen vahvistusdialogi). Sama kieliriippuva päivämääräkaava kuin Tilityksissä/Profiilissa.
- **`/lahetys` (koko live-striimauskonsoli myyjälle) — koko sivu käännetty, suurin yksittäinen osa tätä urakkaa.** Uusi `t.streamConsole`-nimiavaruus (~95 avainta: esikatselu-/asetusnäkymä, OBS-asetuskortti, kielletyt sanat -paneeli, tuotejono+esiasetuspoiminta, pikalisäyslomake, chat-syöte, yläpalkin tilastot/napit, alapalkin huutokauppakontrollit, virheviestit, vahvistusdialogit). Sivulla on kolme erillistä moduulitason apukomponenttia (`QueueProductModal`, `BackButton`, `HlsPreview`) jotka eivät peri `LahetysPage`-pääkomponentin `t`-muuttujaa — jokainen kutsuu `useLang()`:ia itse. Koodikommentit (esim. "Testaa kamera", "Aloita julkinen lähetys" viittauksina toiseen kohtaan koodia) jätetty suomeksi, ne eivät ole käyttäjälle näkyvää tekstiä.

**Koko i18n-läpikäynti tältä listalta nyt valmis** (etusivu, kategoriat, Ilmoitukset [selvitys, ei koodimuutosta — ks. alla], dashboard-etusivu, dashboard/tuotteet, Tilaukset, Tilitykset, Profiili, Lähetys).

**⏳ EI PÄÄTETTY, vaatii omistajan valinnan ennen toteutusta — Ilmoitukset-sivun otsikko-ongelma.** Omistaja kysyi: ilmoituksen otsikko (esim. "Uusi seuraaja", "Voitit huudon") ei käänny kielenvaihdolla, ja kysyi voiko sitä ylipäätään kääntää, ehdottaen profiiliin "preferred language" -kenttää. **Rakenteellinen syy:** `notifyUser()`-funktiota kutsutaan ~33 kohdassa backendissä, ja se kirjoittaa aina valmiiksi suomenkielisen `title`/`message`-tekstin suoraan `Notification`-tauluun LUONTIHETKELLÄ — ei mitään i18n-avainta talteen, joten kääntäminen jälkikäteen on mahdotonta ilman jompaakumpaa: (a) `User.preferredLanguage`-kenttä + backendin oma käännössanakirja, käännettäisiin vain UUDET ilmoitukset lähtien, tai (b) tyyppi+parametrit talteen ja renderöinti vasta näyttöhetkellä (isompi, mutta korjaisi myös vanhat ilmoitukset takautuvasti). **Ei toteutettu kumpaakaan** — annettu suositus omistajalle, odottaa päätöstä kummasta suunnasta edetä.
...
## Oletusteema vaaleaksi + kielivalikko leikkautui navbarin overflow:hidden-rajaukseen 2026-09-10 — ✅ TEHTY JA DEPLOYATTU

**1. Oletusteema vaihdettu tummasta vaaleaksi** (`theme-context.tsx`, omistajan pyyntö) — `localStorage`-tallennettu oma valinta (`habahub_theme`) ohittaa tämän edelleen normaalisti, muuttaa vain sitä mitä käyttäjä näkee ENNEN kuin on koskaan itse vaihtanut teemaa.

**2. Mobiilin (ja desktopin) kielivalikko leikkautui näkymättömiin muun sisällön (esim. mainosbannerin) alle — todellinen z-index/leikkausbugi, ei vain kerrosjärjestys.** Omistaja raportoi ettei kieltä saanut vaihdettua mistään, mainos "tuli sen päälle". **Juurisyy:** mobiilin `<nav>`-elementillä on `overflow:'hidden'` (pyöristettyjä alakulmia varten) — CSS:n overflow-leikkaus tapahtuu AINA riippumatta z-indexistä: lapsi jonka `zIndex:9999` ylittää esi-isän rajat leikkautuu silti pois esi-isän `overflow:hidden`-rajauksen mukaan. Kielivalikko oli `position:absolute` suhteessa nappiin, ulottui navbar-elementin 52px-korkeuden ulkopuolelle, ja leikkautui siksi pois riippumatta omasta korkeasta z-indexistään.

**Korjaus:** valikko renderöityy nyt `position:'fixed'`:llä (mitattu `getBoundingClientRect()`:llä napin oikeasta sijainnista), ei enää `position:'absolute'` suhteessa nappiin. `position:fixed` laskee sijaintinsa aina näyttöruudun mukaan eikä siihen vaikuta esi-isän `overflow:hidden` — vahvistettu ettei mikään navbarin väliin jäävistä elementeistä (`.glass-panel-dark`/`-light`-luokat käyttävät vain `backdrop-filter`:iä) aseta `filter`/`transform`/`perspective`:iä, jotka loisivat uuden containing blockin ja nappaisivat `position:fixed`-lapsen takaisin loukkuun — ilman näitä `position:fixed` pakenee luotettavasti. Sama korjaus tehty sekä mobiilin että desktopin kielivalikkoon.
...
**Tutkittu Stripe Tax + Connect suoraan Stripen dokumentaatiosta ennen koodausta — vastaus: Stripe Tax on väärä työkalu tähän, ei tarvita ollenkaan.** Stripe Tax on rakennettu LASKEMAAN JA LISÄÄMÄÄN veron erillisenä rivinä ilmoitetun hinnan PÄÄLLE — Habahubin hinnat ovat jo ALV-sisältäviä (suomalainen kuluttajahinnoittelukäytäntö), eikä ALV:tä koskaan näytetä erillisenä lisättävänä summana. Lisäksi suurin osa myynnistä on käytettyä tavaraa EU:n marginaaliverotuksen (margin scheme) piirissä, jossa ALV lasketaan vain myyjän katteesta eikä koskaan näytetä ostajalle erillisenä prosenttina — Stripe Tax ei osaa mallintaa tätä. Kyseessä on siis puhdas MERKINTÄ-/läpinäkyvyysominaisuus, ei maksulaskentaominaisuus — Stripeä ei tarvita mihinkään tässä.

**Sivutuotteena löytyi ja korjattiin todellinen bugi:** "Hinta sisältää alv 25,5%" -teksti näytettiin AINA jokaiselle tuotteelle jolla myyjällä oli businessId asetettuna — riippumatta siitä oliko KYSEINEN tuote oikeasti uusi/ALV-sisältävä vai käytetty/marginaaliverotettu. Väärin juuri sen tapauksen kohdalla jonka omistaja itse kuvasi (yleensä 0%, joskus uusi tuote) — teksti valehteli ALV:n sisältymisestä lähes joka tuotteella.

**Toteutus:** uusi `Product.vatIncluded Boolean @default(false)`. Uusi tuotelomakkeen valintaruutu (`dashboard/tuotteet`) näkyy VAIN kun `user?.businessId` on asetettu (yksityismyyjä ei näe tätä ollenkaan — ei ole ALV-rekisteröity, ei voi koskaan väittää hintansa sisältävän ALV:tä). **Palvelinpuoli pakottaa saman säännön riippumattomasti** — uusi `resolveVatIncluded()`-apufunktio (`backend/src/routes/products.ts`, käytössä sekä `POST /products` että `PUT /products/:id`) hakee myyjän `businessId`:n tuoreena kannasta ja pakottaa `vatIncluded`:in aina falseksi jos sitä ei ole, riippumatta mitä pyynnön body väittää — ei luoteta pelkkään frontend-piilotukseen. Kaikki neljä paikkaa jotka aiemmin näyttivät tekstin businessId:n perusteella (`ProductCard.tsx` × 2, `tuotteet/[id]`, `huutokauppa/[id]`) vaihdettu käyttämään uutta per-tuote `vatIncluded`-kenttää — "Yritysmyyjä"-badge (DSA art. 31 -merkintä, eri tarkoitus) pysyy ennallaan businessId-perusteisena, ei sekoiteta näitä kahta.

**Ei laajennettu bulkkituontiin** — bulkkituonti on tarkoitettu suurille käytetyn tavaran erille (esim. kortit), joissa ALV-sisältyvyys ei ole ajankohtainen; yksittäinen uusi tuote lisätään aina manuaalisella lomakkeella, jossa toggle nyt on.

**Testattu tuotannossa read-only-logiikkatestillä ennen deployta:** `resolveVatIncluded()`-logiikka ajettu suoraan tuotannon dataa vasten — käyttäjä ilman businessId:tä (`testi3user`) yritti `requested:true`, palautti oikein `false`. Tuotannossa ei tällä hetkellä ole yhtään käyttäjää jolla businessId on asetettu (yritysmyyjät eivät ole vielä täyttäneet Y-tunnustaan profiiliinsa), joten `true`-polkua ei voitu vahvistaa oikealla tilillä — logiikka on kuitenkin suoraviivainen ja symmetrinen `requireVerifiedSeller()`:n kanssa.
...
**1. Stripen EUR-maksujen 0,50€-alaraja puuttui validoinnista kokonaan.** Testissä tuli Stripeltä virhe liian pienestä summasta — Stripe hylkää koko PaymentIntentin, ei vain varoita, jos loppusumma jää alle 0,50€:n. Ei ollut mitään estettä listata tuotetta tätä pienemmällä hinnalla. **Korjattu:** `MIN_PRICE_EUROS = 0.5` -validointi lisätty `POST /products`, `POST /products/bulk`, `PUT /products/:id` (`startPrice`+`buyNowPrice`) sekä `POST /offers` ja `POST /offers/:id/counter` (`amount`/`counterAmount`) — kaikki reitit jotka voivat päätyä sellaisenaan ostajan maksettavaksi. Frontend: sama tarkistus + selkeä virheviesti `dashboard/tuotteet`-lomakkeessa (manuaalinen + bulkkituonti), `lahetys`-pikalisäyksessä, ja tuotesivun "Tarjoa hintaa" -lomakkeessa, plus vihjetekstit hintakenttien vieressä ("väh. 0,50€") kaikissa kolmessa kielessä.

**2. Tilitykset-sivu ei ollut koskaan oikeasti kytketty mihinkään — vahvistettu koodista, ei arvattu.** `dashboard/tilitykset/page.tsx`:n tapahtumataulukko oli kovakoodattu tyhjä taulukko (`useState<any[]>([])`) jonka `useEffect`-kommentti sanoi kirjaimellisesti "Haetaan tilitykset backendistä kun order-järjestelmä on valmis" — mutta mitään hakua ei koskaan kirjoitettu, vaikka order-järjestelmä on ollut valmis pitkään. **Korjattu:** sivu hakee nyt oikeasti `GET /orders/selling`:n kautta, suodattaa maksetut tilaukset (`PENDING_SHIPPING`/`SHIPPED`/`DELIVERED`/`DISPUTED`, ei `PENDING_PAYMENT`/`CANCELLED`), näyttää tuotteen/ostajan/myyntihinnan/palkkion/netton/tilan/päivän oikealla datalla.
   - **Uusi `Order.commissionCents Int?`-kenttä** — todellinen komissio senteinä tallennetaan `POST /orders/:id/pay`:ssä TÄSMÄLLEEN samalla laskennalla joka menee `application_fee_amount`:iin, ei lasketa uudelleen jälkikäteen. Syy: myyjän efektiivinen komissioprosentti (14pv-tutustumispromo/admin-ylikirjoitus) voi muuttua ajan myötä, jolloin jälkikäteislaskenta ei enää vastaisi todella veloitettua summaa. **Vanhat, ennen tätä deployta maksetut tilaukset näyttävät Palkkio/Netto-sarakkeissa "—"** koska kenttä oli silloin vielä olemassa olematon — tämä koskee myös omistajan omaa juuri tehtyä testiostosta, se ei ole bugi vaan odotettu rajaus historiadatalle.
   - Prisma-migraatio ajettu tuotantoon (`npx prisma db push`) ennen deployta.

**3. StripeConnectCard ei koskaan antanut pääsyä myyjän omaan Stripe-saldoon.** Kun `transfersEnabled:true`, kortti näytti aiemmin vain staattisen "Stripe-tilisi on yhdistetty" -tekstin ilman mitään linkkiä — ainoa paikka jossa TODELLINEN, ajantasainen saldo/tilityshistoria on olemassa (Stripen oma Express-hallintapaneeli) oli tavoittamattomissa Habahubin kautta. **Lisätty `GET /users/me/stripe-dashboard-link`** (`stripe.accounts.createLoginLink()` — v1-API:n metodi, mutta vahvistettu suoraan tuotannossa toimivaksi v2-tilin ID:llä sellaisenaan ennen kytkentää, ei arvattu) + uusi "Avaa Stripe-hallintapaneeli" -nappi kortissa kun tili on verifioitu, avautuu uuteen välilehteen. **Testattu suoraan tuotannossa oikealla myyjätilillä** (Larzmoi, `acct_1UE4njJbr93YbWMV` — huom eri tili-ID kuin aiemmin dokumentoitu `acct_1UDrJMJZ1HMkFLjA`, koska myyjä onboardautui uudelleen edellisen `merchant.card_payments`-korjauksen jälkeen, mikä samalla vahvisti tuon korjauksen toimivan oikeasti käytännössä).

**Vastaus omistajan kysymykseen rahankulun oikeellisuudesta:** kuvattu testi (ostaja = eri Habahub-tili, myyjä = Larzmoi, sama kortti/henkilö, nouto valittuna) täsmää arkkitehtuuriin täydellisesti. Koska nouto valittiin, `shippingPrice:0` — ei toimitusmaksua ollenkaan tässä testissä. Kortilta veloitetaan koko `productTotal`, josta Habahubin oma Stripe-saldo saa `application_fee_amount` (= 3,5%/max35€ -komissio, koska toimitusta ei ollut) MIINUS Stripen oma käsittelymaksu (Habahub maksaa tämän omasta osuudestaan, koska charge on luotu Habahubin omalle tilille eikä `on_behalf_of`:ia käytetä — myyjän saama siirto ei koskaan pienene Stripen käsittelymaksun takia). Myyjän Stripe-saldo saa täsmälleen `productTotal − komissio`. Koska sama henkilö omistaa lopulta molemmat päät, nettotulos hänelle on juuri "Stripen maksu + Habahubin provikka" kuten omistaja itse päätteli — tarkka euromäärä näkyy nyt suoraan uudesta "Avaa Stripe-hallintapaneeli" -linkistä.

## Mobiilisivut vuotavat yli (Firefox) — mobiilin grid-sarake ilman minmax(0,...) 2026-09-10 — ✅ TEHTY JA DEPLOYATTU

Omistaja lähetti kuvakaappauksen tuotesivulta (mobiili, Firefox): "Lisää ostoskoriin" -nappi, ennakkotarjouslaatikon hinta/nappi, ja myyjäkortin "Profiili"-linkki kaikki leikkautuivat pois oikeasta reunasta, vaikka samalla sivulla otsikko/hinta/badget mahtuivat normaalisti näkyviin.

**Juurisyy: klassinen CSS Grid -sudenkuoppa.** Kahdeksassa paikassa koodikannassa (`tuotteet/[id]`, `huutokauppa/[id]`, `lahetys`, `dashboard`×2, `dashboard/tuotteet`×2, `dashboard/esiasetukset`) on sama kuvio: `gridTemplateColumns: isMobile ? '1fr' : ...`. Paljas `'1fr'` EI pakota saraketta oikeasti kutistumaan näytön levyiseksi — grid-elementin oletus-minimileveys on sen oman sisällön luontainen (kutistumaton) leveys, joten jos sarakkeen sisällä on MIKÄ TAHANSA elementti jonka luontainen minimileveys ei mahdu näyttöön, koko sarake (ja sen mukana koko sivu) renderöityy näyttöä leveämmäksi. Koska `globals.css`:ssä on jo `overflow-x: hidden` `body`:llä (LUKITTU "sivu ei saa scrollata"-säännön toteutus), tämä ylivuoto ei näy vaakascrollina vaan HILJAA LEIKKAANTUNEENA sisältönä oikeasta reunasta — täsmälleen kuvakaappauksen oire. Tuote-/huutokauppasivun desktop-haara käytti jo oikein `minmax(0, 1fr)`:ää samassa tiedostossa — vain mobiilihaara puuttui.

**Korjaus:** kaikki kahdeksan kohtaa muutettu käyttämään `minmax(0, 1fr)`:ää paljaan `'1fr'`:n sijaan mobiilihaarassa (huutokauppasivun desktop-haarakin yhtenäistetty samaan muotoon varmuuden vuoksi). Muutos ei vaikuta normaalin sisällön ulkoasuun mitenkään — se vain sallii sarakkeen kutistua tarvittaessa sen sijaan että pakottaisi sen ylivuotamaan, sama korjaustapa jota tuotesivun desktop-puoli on jo käyttänyt. Typecheck+build vihreä, deployattu. **Ei vielä visuaalisesti vahvistettu selaimessa** (ei selaintyökalua tässä ympäristössä) — omistajan kannattaa tarkistaa sama tuotesivu Firefoxilla uudelleen.

## Stripe live-onboarding hylkäsi tilin luonnin — merchant.card_payments-vaatimus 2026-09-10 — ✅ LÖYDETTY JA KORJATTU

Omistaja törmäsi live-tilassa oikeaa myyjän onboardingia yrittäessä Stripen virheeseen: *"The stripe_balance.stripe_transfers capability cannot be requested without the configuration.merchant.capabilities.card_payments capability. Please request configuration.merchant.capabilities.card_payments on the v2 account."* — sama `createConnectedAccount()`-koodi joka toimi testitilassa 2026-09-09 (ks. "Paytrail → Stripe Connect" -osio, pelkkä `recipient.capabilities.stripe_balance.stripe_transfers`) hylättiin nyt live-avaimella suoraan tilin luontivaiheessa.

**Ei arvattu — vahvistettu suoraan live-API:a vasten kahdella kertakäyttöisellä diagnostiikkatilillä ennen koodausta** (luotu + poistettu heti `stripe.accounts.del()`:llä, ei jäänyt tuotantoon): (1) pelkkä `recipient.stripe_transfers` toisti virheen täsmälleen samana live-avaimella — vahvistaa ettei kyse ollut hetkellisestä Stripe-häiriöstä; (2) lisäämällä `configuration.merchant.capabilities.card_payments.requested:true` rinnalle tilin luonti onnistui. Stripen omat julkiset dokumentit (`docs.stripe.com/connect/accounts-v2`) itse asiassa väittävät päinvastaista ("recipient-konfiguraation stripe_transfers... on riittävä indirect charges -käyttöön"), joten tämä on live-tilan oma, dokumentoimaton lisävaatimus — sandboxit tunnetusti "eivät aina valvo kaikkia kapasiteetteja" (Stripen oma varoitus), mikä selittää miksi testitilassa tämä toimi ongelmitta.

**Ei muuta rahankulkua eikä arkkitehtuuria.** `card_payments`-kapasiteetin pyytäminen ei tarkoita että myyjä alkaisi käsitellä korttimaksuja itse — Habahub tekee yhä KAIKKI veloitukset omalla Checkout Sessionillaan destination-chargen kautta (`createCheckoutSession()`), ei koskaan `on_behalf_of`:ia eikä myyjän omaa suoraa charge-luontia. Stripe vain vaatii tämän kapasiteettiparin OLEMASSAOLOA ennen kuin `stripe_transfers` voidaan ylipäätään myöntää live-tilassa yksityishenkilö-tilille.

**Sivuvaikutus, myös vahvistettu samalla diagnostiikkatilillä ja korjattu ilman lisäkitkaa myyjälle:** `merchant.card_payments`:n pyytäminen toi mukanaan kaksi UUTTA "routine_onboarding"-vaatimusta joita pelkkä recipient-tili ei koskaan tarvinnut — `configuration.merchant.mcc` (toimialakoodi) ja `defaults.profile.business_url`. Jos näitä ei aseteta, Stripen hostattu onboarding-lomake olisi alkanut kysyä niitä myyjältä itseltään — ylimääräistä kitkaa jota yksityishenkilömyyjä ei osaisi täyttää mielekkäästi (mikä toimialakoodi kortti-/keräilytuotteiden yksityismyynnille?). **Korjaus:** Habahub asettaa molemmat itse tilin luontihetkellä samoiksi kaikille myyjille (`mcc: '5945'` — "Hobby, Toy, and Game Shops", `business_url: 'https://habahub.com'`) — vahvistettu samalla diagnostiikkatilillä että kumpikin vaatimus katoaa kokonaan tilin requirements-listalta kun nämä asetetaan luontihetkellä. Jäljelle jäävät vaatimukset ovat täsmälleen samat kuin ennenkin (nimi/osoite/syntymäaika/puhelin/pankkitili/käyttöehtojen hyväksyntä) — myyjän onboarding-kokemus ei muutu miltään osin.

**Korjattu `backend/src/lib/stripe.ts`:n `createConnectedAccount()`:** `configuration.merchant.capabilities.card_payments.requested:true` + `configuration.merchant.mcc:'5945'` lisätty `recipient.stripe_transfers`-pyynnön rinnalle, `defaults.profile.business_url:'https://habahub.com'` lisätty `defaults`-objektiin. Typecheck+build vihreä, deployattu. **Ei vielä testattu oikealla uudella myyjä-onboardingilla loppuun asti** — korjaus vahvistettu diagnostiikkatilien luonti/vaatimustarkastuksella, ei täydellä KYC-lomakkeen läpivmillä selaimessa (sama rajaus kuin muillakin "vaatii oikean selaimen" -kohdilla tässä projektissa). Omistajan kannattaa kokeilla uudelleen samalla myyjätilillä jolla virhe alun perin ilmeni.

## ⚠️ KRIITTINEN — toimitusmaksu meni väärälle osapuolelle Stripen destination-chargessa 2026-09-10 — ✅ LÖYDETTY JA KORJATTU ENNEN ENSIMMÄISTÄ OIKEAA MAKSUA

Omistaja kysyi suoraan: onko Stripeen jo määritelty että toimitusmaksu menee aina Habahubin tilille ja 3,5%/max35€ komissio kauppahinnasta, vai pitääkö nämä vielä määritellä. Tarkistettu koodista ennen vastaamista — **löytyi todellinen, korjaamaton bugi**, ei koskaan vielä vaikuttanut oikeaan rahaan koska yhtään live-maksua ei ole vielä ajettu läpi (ks. "Paytrail → Stripe Connect" -osio, "Ei vielä testattu" -kohta).

**Juurisyy:** destination-charge-mallissa (`backend/src/lib/stripe.ts`, `createCheckoutSession()`) KOKO maksettu summa (tuotteet+toimitus) siirtyy automaattisesti myyjän Stripe-tilille MIINUS `application_fee_amount` — vain `application_fee_amount` jää Habahubille. `application_fee_amount` laski aiemmin VAIN 3,5%/35€-komission, ei toimitusmaksua. LUKITTU-sääntö "ei provisiota postista" (ks. "Postihinnat"-osio) oli tarkoitettu estämään 3,5%:n laskeminen toimitusmaksun PÄÄLLE — mutta toteutus meni pidemmälle ja jätti koko 6,90€:n toimitusmaksun kokonaan myyjän osuuteen, vaikka CLAUDE.md:n oma "Postihinnat"-sääntö on aina ollut että Habahub veloittaa 6,90€ ostajalta ja maksaa Postille itse erikseen omana kulueränään (oma ALV-vastuu, ks. "Viisi myöhemmin tarkistettavaa asiaa 2026-09-04" kohta 3) — toimitusmaksu on Habahubin omaa liikevaihtoa, ei koskaan myyjän.

**Korjaus:** `application_fee_amount = commissionCents + eurosToCents(shippingEuros)` — Habahub pitää nyt sekä komission että koko toimitusmaksun, myyjän tilille siirtyy aina täsmälleen `productTotal - komissio`, ei senttiäkään toimituksesta. Hyvityslogiikka (`refundPayment()`, `reverse_transfer + refund_application_fee`) ei vaatinut muutosta — Stripe suhteuttaa `application_fee`-palautuksen automaattisesti osittaisissa hyvityksissä riippumatta sen koosta.

**Nouto vs. postitus ei vaadi erillistä tunnistusta** — `order.shippingPrice` lasketaan aina palvelinpuolella `lib/shipping.ts`:n `PAKETTIKOOT`-taulukosta (`'nouto'` → 0€, `'postitus'` → 6,90€), ei koskaan luoteta clientiltä tulevaan arvoon. Nouto-tilauksilla `shippingEuros` on siis jo 0 tässä laskennassa, ei muuta mitään `application_fee_amount`-summaan — sama koodipolku toimii oikein molemmille ilman erillistä haaraa.

**Ei testattu oikealla maksulla** (tarkoituksella, sama varovaisuusperiaate kuin muillakin live-rahaa liikuttavilla toiminnoilla) — korjaus vahvistettu lukemalla Stripen destination-charge-dokumentaation logiikka läpi ja laskemalla rahankulku käsin (charge − application_fee = seller transfer), typecheck+build vihreä, deployattu. Ensimmäinen oikea live-maksu paljastaa lopullisesti toimiiko jako oikein — sama rajaus kuin muillakin "ei automatisoitavissa turvallisesti" -kohdilla tässä projektissa.

## Stripe-tuotantoavaimet + kolme mobiili-UI-bugia + kuntoluokituksen palautus + livekonsolin kunto 2026-09-10 — ✅ TEHTY JA DEPLOYATTU

**1. Stripe vaihdettu testitilasta oikeisiin tuotantoavaimiin.** Omistaja aktivoi Stripe-tilin itsepalveluna (ei vaatinut yhteydenottoa Stripeen, toisin kuin Paytrail/Posti aikanaan) ja toimitti oikeat `pk_live_`/`sk_live_`-avaimet. Ennen vaihtoa lisätty kriittinen puuttuva suoja: `backend/src/routes/orders.ts`:n `POST /:id/pay` ja `backend/src/routes/users.ts`:n `GET /me/stripe-status` kutsuivat `getAccountStatus()`:ia ilman try/catchia — jokainen olemassa oleva myyjän `stripeAccountId` oli luotu testitilassa, joten ne olisivat aiheuttaneet käsittelemättömän 500-virheen ensimmäisellä maksuyrityksellä testiavainten vaihdon jälkeen (Stripe palauttaa "No such account" testitilin ID:lle live-avaimella kysyttäessä). Molemmat kutsut suojattu try/catchilla, palauttavat nyt siistin `400`:n ("Myyjän Stripe-onboarding on vielä kesken") virheen sijaan.

Uusi live-tilan webhook (`checkout.session.completed`, `https://habahub.com/api/webhooks/stripe`) ja uusi live-tilan v2 Event Destination (`events_from:['@self']`, `https://habahub.com/api/webhooks/stripe-accounts`, sama v1/v2-erottelu jo dokumentoitu 2026-09-09) luotu Stripen dashboardista, salaisuudet lisätty palvelimen `.env`:iin. Google Pay + MobilePay kytketty päälle myös live-tilan `paymentMethodConfigurations`-asetuksissa (olivat oletuksena pois, Apple Pay oli jo päällä). Kahden olemassa olevan testitilin myyjän (`Larzmoi`, `michaelbacklund`) vanhentunut testitilan `stripeAccountId` tyhjennetty tuotantokannasta — molemmat onboardaavat itsensä uudelleen Stripen live-tilaan ensimmäisen myyntinsä yhteydessä. `User.verified`-lippu ei liity `stripeAccountId`-kenttään, ei kosketettu.

**Omistaja kohtasi Stripen oman este-viestin:** "Please review the responsibilities of managing losses for connected accounts" (`dashboard.stripe.com/settings/connect/platform-profile`). Tämä on Stripen puhdas dashboard-tason compliance-askel (tappiovastuun hyväksyntä ennen kuin live-tilassa voi luoda connected accounteja `losses_collector:'application'`-mallilla) — ei API:lla tehtävissä, ei koodimuutosta, omistajan pitää hyväksyä se itse Stripen dashboardista.

**2. Kolme mobiili-UI-bugia korjattu:**
- **Etusivun/Selaa-sivun tuotekortit menivät päällekkäin mobiilissa** (`frontend/components/ProductCard.tsx`) — kortti käytti samaa kaksipalstaista layoutia mobiilissa ja desktopilla, ahtaalla mobiilileveydellä nimi/hinta/myyjä/paikkakunta/badget menivät osittain päällekkäin eikä kaikki teksti mahtunut näkyviin. Korjattu: mobiilissa oma pystysuuntainen versio (nimi → hinta → myyjä+vahvistus-badge → paikkakunta → yritysmyyjä/ALV-merkinnät → kuntobadge, kukin omalla rivillään), desktop-layout koskematon.
- **Yläpalkin Hallintapaneeli-nappi vuoti yli/oli vaikea klikata mobiilissa tietyillä selaimilla** (`frontend/components/layout/Navbar.tsx`) — `LogoMark`-komponentin linkki-elementillä oli aina `flexShrink:0`, jolloin mikään mobiilin yläpalkin kuudesta elementistä (logo, teemakytkin, kielivalitsin, kolme ikoninappia, dashboard-nappi) ei voinut koskaan antaa tilaa, ja rivi ylivuoti selaimen chrome-leveyden mukaan vaihtelevasti. Lisätty `allowShrink`-propti `LogoMark`iin (kutistuu tarvittaessa, teksti typistyy `text-overflow:ellipsis`:llä), käytössä vain mobiilin yläpalkissa.
- **Mainoslaatikko ("Habahub suosittelee") ei täyttänyt koko näytön leveyttä mobiilissa** — kolme yritystä ennen oikeaa syytä: ensin väärinymmärretty suunta (kavennettiin kun piti levetä), sitten epäilty välimuistiongelmaksi (tarkistettu nginx `proxy_cache` [ei ollut], `.next/cache` [93MB vanhentunutta, tyhjennetty+puhdas rebuild, ei auttanut], Cloudflaren reunavälimuisti [tuore joka tarkistuksella], service worker [ei koskaan cachennut mitään]) — omistaja vahvisti cache-busterilla ja eri kuvalla ettei kyse ollut välimuistista. **Todellinen syy:** mainoslaatikon kääre-`<div>` oli `margin:'0 auto'`:lla suoraan `display:flex, flexDirection:'column'`-vanhemman lapsi — flex-akselilla `margin:'0 auto'` ohittaa `align-items:stretch`-oletuksen ja kutistaa elementin sisältönsä levyiseksi keskittäen sen, sen sijaan että se täyttäisi koko leveyden ja keskittyisi `maxWidth`:n sisällä (kuten muut samantyyliset kääreet samalla sivulla, joilla oli jo `width:'100%'`). Korjattu lisäämällä `width:'100%'` samaan kääre-diviin. Vahvistettu omistajan kuvakaappauksella.

**3. Mainoslaatikon badge + kuntoluokituksen palautus (sama viesti, kaksi pyyntöä):**
- "Habahub suosittelee" -badge siirretty pois läpinäkyvästä `position:absolute`-kulmasijainnista (huono kontrasti kuvatustan päällä) normaaliin sisältövirtaan, vasemmalle linjaan otsikon/kuvauksen kanssa, käyttäen samaa `C.accentSolid`/`C.accentText`-väriparia kuin sivuston napit muutenkin — taattu kontrasti riippumatta taustakuvasta.
- **Kuntoluokitus palautettu puhtaaksi 7-tunnukselliseksi Cardmarket-asteikoksi** (`frontend/lib/conditions.ts`) — 2026-09-05 lisätyt puolikunnot (NM+/NM-/EX+/EX-/GD+/GD-) poistettu, koska omistaja ei ollut niitä pyytänyt. Jäljellä vain M/NM/EX/GD/LP/PL/PO. Koska tämä on jaettu vakio (lomake+bulkkituonti+suodattimet), palautus vaikuttaa kaikkiin kolmeen automaattisesti ilman erillisiä muutoksia. Jo tallennetut puolikunta-arvot jäävät tietokantaan ennalleen, vain uusi valinta rajoittuu seitsemään.

**4. ✅ TEHTY 2026-09-10 — Livekonsolin pikalisäyksestä puuttui kunnon asetusmahdollisuus.** Omistaja huomasi: kun tuotteen lisää kesken lähetyksen `/lahetys`-konsolin "+ Lisää tuote" -pikalomakkeella, kuntoa ei voinut asettaa itse — ainoa tapa saada kunto mukaan oli ladata esiasetus (`ProductPreset`), joka toi kunnon mukanaan piilokenttänä. Vahvistettu koodista: `qaCondition`-tila oli jo olemassa ja jo mukana `quickAddProduct()`:n lähettämässä datassa, mutta lomakkeessa ei ollut mitään näkyvää valitsinta sille — vain esiasetuksen lataus saattoi täyttää sen. **Korjaus:** lisätty kunto-`<select>` pikalisäyslomakkeeseen (`frontend/app/lahetys/page.tsx`, heti minimikorotus-kentän jälkeen), käyttää samaa jaettua `CARDMARKET_KUNTOLUOKAT`-vakiota kuin muukin sivusto, "Kunto (ei valittu)" -oletus koska kaikki live-myytävät tuotteet eivät tarvitse kuntoarviota (esim. sealed-tuotteet). Esiasetuksen mahdollisesti tuoma arvo näkyy valitsimessa ja on vapaasti ylikirjoitettavissa käsin.
...
**Syy vaihtoon:** omistaja totesi suoraan: "pitää paytrail vaihtaa stripeen koska paytrail ei voi tarjota tarvitsemaamme palvelua" — tarkennettu: **Paytrailin sub-merchant-onboarding vaatii aina vähintään Y-tunnuksen** (ks. aiempi "Paytrail-alakauppiaan onboarding"-osion ratkaisematon kysymys #1), mikä sulkisi pois suurimman osan Habahubin todellisista myyjistä (yksityishenkilöt ilman toiminimeä, esim. kortteja myyvät harrastajat). Tämä oli suora este koko liiketoimintamallille, ei vain kiusallinen yksityiskohta.

**Päätös: Paytrail poistettu kokonaan, ei jätetty pois päältä olevaksi varajärjestelmäksi.** Omistajan eksplisiittinen valinta kahdesta vaihtoehdosta ("poista kokonaan" vs. "säilytä poissa päältä"): **"Poista Paytrail kokonaan"**. `backend/src/lib/paytrail.ts` poistettu tiedostosta (lähdekoodi + käännetty `dist/`), kaikki reitit siirretty käyttämään uutta `backend/src/lib/stripe.ts`:ää.

**Arkkitehtuuri, vahvistettu suoraan Stripen oikeaa API:a vasten ennen koodausta (ei arvattu):**
- **v2 Accounts API, ei vanha v1.** Stripe hylkää nykyään uusien platformien v1-tilien luonnin oletuksena — vahvistettu suoraan virheviestistä testissä: *"Stripe no longer recommends Accounts v1 for new Connect integrations. Create connected accounts with POST /v2/core/accounts instead."* Koko integraatio rakennettu v2:n varaan.
- **Jokainen myyjä saa oman "recipient"-konfiguroidun v2 Accountin** (vastine Paytrailin sub-merchant-ID:lle) — EI "merchant"-konfiguraatiota, koska myyjä ei koskaan itse vastaanota korttimaksua suoraan.
- **`identity.entity_type: "individual"`** — tämä on se kenttä joka ratkaisee alkuperäisen ongelman: yksityishenkilö voi luoda tilin ilman mitään Y-tunnusta/toiminimeä. Vahvistettu toimivaksi `country: "fi"`:llä oikealla testitilin luonnilla.
- **`dashboard: "express"`** — myyjä saa Stripen oman brändätyn Express-hallintapaneelin saldon/tilitysten seurantaan, vaikka tili on luotu v2 API:lla.
- **Maksu: yksi Checkout Session per tilaus** (tuote+toimitus yhdessä, sama LUKITTU-sääntö kuin Paytraililla — ei kahta erillistä maksua) + **destination charge** (`payment_intent_data.transfer_data.destination` + `application_fee_amount`). Tämä on suora arkkitehtuurinen vastine Paytrailin Shop-in-Shopille: koko summa siirtyy ensin myyjän tilille, sitten komissio-osuus takaisin Habahubille. **EI `on_behalf_of`-parametria** — vaatisi `card_payments`-kapasiteetin, joka ei ole sallittu recipient-tileille.
- **Checkout Session on hostattu, koko sivun uudelleenohjaus** (`session.url`) — sama malli kuin Paytrailin `redirectUrl`, joten `frontend/app/kori/page.tsx`:n `window.location.href = redirectUrl` ei vaatinut YHTÄÄN koodimuutosta.
- **Hyvitykset yksinkertaisempia kuin Paytraililla:** `reverse_transfer: true` + `refund_application_fee: true` — Stripe laskee automaattisesti PROPORTIONAALISEN komissio-osuuden osittaisessa hyvityksessä (vahvistettu Stripen dokumentaatiosta), ei tarvitse enää manuaalista per-tuote-komissiolaskentaa kuten Paytrailin Shop-in-Shopissa.
- **Webhook-allekirjoituksen varmistus vaatii RAA'AN pyyntörungon** (`Buffer`, ei jäsennettyä JSON:ia) — tämä on uusi rakenteellinen vaatimus jota Paytrailin GET+query-parametri-webhookilla ei koskaan ollut. `backend/src/index.ts`: `/webhooks/stripe`-reitti rekisteröity `express.raw({type:'application/json'})`:lla ENNEN globaalia `express.json()`-middlewarea, muuten JSON-parserin olisi jo kuluttanut rungon ennen allekirjoituksen tarkistusta.
- Sama LUKITTU 3,5%/35€-komissiosääntö ja 14 vrk:n 0%-tutustumisjakso (`computeCommissionCents`/`getEffectiveCommissionOverride`) siirretty sellaisenaan `paytrail.ts`:stä — logiikka ei riipu maksupalveluntarjoajasta.
- Vanhat `Order.paytrailPaymentId`/`paytrailProductTxId`/`paytrailShippingTxId`/`paytrailAttemptId`-kentät **jätetty schemaan koskemattomina** (ei pudotettu) — olemassa olevat maksetut tilaukset viittaavat oikeisiin historiallisiin Paytrail-transaktioihin. Uudet kentät `Order.stripeSessionId`/`stripePaymentIntentId` lisätty rinnalle, vain näitä täytetään uusilla tilauksilla. `User.stripeAccountId`/`stripeCustomerId` olivat jo valmiiksi schemassa (löytyi käyttämättömänä, luultavasti aiemmin varalta lisättynä) — `stripeAccountId` otettu käyttöön, `stripeCustomerId` jää toistaiseksi käyttämättä.

**Uudet reitit:**
- `POST /users/me/stripe-onboarding` — luo Connect-tilin jos ei vielä ole (`createConnectedAccount`, tallentaa `User.stripeAccountId`:iin), palauttaa Account Links -onboarding-URL:n (`type: 'account_onboarding'`, toimii v2-tileille sellaisenaan).
- `GET /users/me/stripe-status` — palauttaa `{connected, transfersEnabled, payoutsEnabled}`.
- `POST /orders/:id/pay`: uusi esto — jos `seller.stripeAccountId` puuttuu tai `transfersEnabled !== true`, `400`-virhe ennen kuin Checkout Sessionia edes yritetään luoda.
- `POST /orders/:id/refund`: käyttää `order.stripePaymentIntentId`:tä (ei enää `paytrailProductTxId`:tä).
- `GET/POST /webhooks/stripe`: kuuntelee `checkout.session.completed`-tapahtumaa, sama idempotenssiperiaate kuin Paytrail-webhookilla (`if (order.status === 'PENDING_PAYMENT')`).
- Frontend: `/dashboard/tilitykset`-sivulle uusi "Maksujen vastaanotto (Stripe)" -kortti (`StripeConnectCard`) — näyttää yhdistämistilan, "Yhdistä Stripe-tili" / "Jatka onboardingia" -nappi ohjaa Stripen hostatulle onboarding-lomakkeelle.

**⚠️ Kriittinen jatkokorjaus samana päivänä, löytyi oikealla tuotantotestillä (ei arvattu):** ensimmäinen versio `getAccountStatus()`:sta tarkisti vain `payouts`-kapasiteettia — `POST /orders/:id/pay` epäonnistui aidolla Stripe-virheellä testissä: *"your destination account needs to have the configurations.recipient.capabilities.stripe_balance.stripe_transfers capability enabled."* Kaksi ERI kapasiteettia, tarkoituksella eroteltu: **`stripe_transfers`** = voiko tili YLIPÄÄTÄÄN vastaanottaa destination-chargen (tämä on se joka estää maksun aloituksen, ei `payouts`) vs. **`payouts`** = voiko myyjä NOSTAA jo vastaanotetun saldon pankkitililleen (eri, myöhempi onboarding-vaihe). Korjattu: `getAccountStatus()` palauttaa nyt molemmat, `POST /orders/:id/pay`:n esto ja `StripeConnectCard`:n tila/nappi käyttävät `transfersEnabled`:iä, ei `payoutsEnabled`:iä.

**Testattu tuotannossa (Stripen testitila, `sk_test_...`/`pk_test_...`):** luotu oikea Connect-tili yksityishenkilölle (`identity.entity_type:"individual"`, `country:"fi"`) — onnistui suoraan v2 API:a vasten. `michaelbacklund`-myyjätilille luotu oikea (test-mode) Connect-tili onboarding-reitin kautta (`acct_1UDiNEJZ1HxywDs4`), tallentui `User.stripeAccountId`:iin. Kertakäyttöinen testituote + tilaus luotiin päästä-päähän-testiin `POST /orders/:id/pay`:n eston varmistamiseksi (esto laukesi oikein ennen korjausta koska `transfersEnabled` oli `false` — tämä JUURI paljasti yllä kuvatun kapasiteettibugin), testidata siivottu tuotannosta korjauksen jälkeen.

**Ei vielä testattu, rehellinen rajaus (sama kategoria kuin Paytrailin/Postin "vaatii oikean selaimen" -rajoitukset):** oikean myyjän täysi Stripe-hostattu onboarding-lomake (nimi/osoite/syntymäaika/pankkitili suomalaisella pankkitunnistuksella) loppuun asti selaimessa, ja sen jälkeinen oikea ostajan maksu Checkout Sessionin läpi — ei automatisoitavissa turvallisesti tästä ympäristöstä. `michaelbacklund`in Connect-tili on siis olemassa mutta `transfersEnabled` on vielä `false` kunnes joku oikeasti täyttää onboarding-lomakkeen selaimessa.

**✅ TEHTY JA DEPLOYATTU 2026-09-09 — kaikki jäljellä olleet "Paytrail"-maininnat korjattu.** Käyttöehdot/tietosuoja/FAQ/välityspalkkiot päivitetty kaikissa kolmessa kielessä: `kayttoehdot/content.ts` (§5.4-5.5, "Stripe"), `tietosuoja/content.ts` (tietojenkäsittelijä-luettelo → "Stripe Payments Europe, Limited" — vahvistettu suoraan Stripen omasta DPA:sta oikeaksi EU/ETA-sopimusosapuoleksi, irlantilainen yhtiö), `faq/page.tsx` ("Miten maksan ostokseni?" — MobilePay vahvistettu WebSearchilla suoraan tukevan Suomea Stripe Checkoutissa, Apple Pay/Google Pay ovat Stripen vakiolompakoita), `frontend/lib/i18n/{fi,en,sv}.ts` (`paymentFee`). Maksunkäsittelymaksun luku (~1,5% + 0,25€) EI muuttunut — vahvistettu Stripen omasta hinnastosta että ETA-korttien perusmaksu on sama kuin Paytraililla oli. Myös kaikki pelkät koodikommentit (`cart.ts`, `auctionOrder.ts`, `orders.ts`, `deliveryTimeline.ts`, `kori/page.tsx`, `ostot/page.tsx`, `proxy.ts`, `auth-context.tsx`) päivitetty — mukaan lukien kolme vanhentunutta "TODO: Paytrail capture" -kommenttia jotka olivat jo ennenkin virheellisiä (destination charge siirtää myyjän osuuden jo maksuhetkellä, ei koskaan ollut erillistä capture-vaihetta kummallakaan tarjoajalla).

**✅ TEHTY 2026-09-09 — siirretty oikeisiin Stripe-tuotantotunnuksiin, LIVE-TILA PÄÄLLÄ.** Ei vaatinut Stripen puoleista yhteydenottoa/sopimusta (toisin kuin Paytrail/Posti) — Stripen oma dokumentaatio vahvistaa itsepalveluprosessin: omistaja täyttää itse tilin vahvistuslomakkeen Dashboardissa, Stripe tarkistaa automaattisesti ja ottaa itse yhteyttä vain jos tarvitsee lisätietoja. **Vahvistettu suoraan live-avaimella ennen käyttöönottoa:** platform-tili oli jo täysin aktivoitu (`charges_enabled:true`, `payouts_enabled:true`, `details_submitted:true`, ei avoimia `requirements`-kohtia).

**Tehty:**
- `.env`: `STRIPE_SECRET_KEY` vaihdettu `sk_live_...`:iin, `frontend/.env.local`:n `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` päivitetty `pk_live_...`:iin (kosmeettinen — arvoa ei käytetä missään koodissa, Checkout on palvelinpuolinen uudelleenohjaus, ei Stripe.js:ää, vahvistettu grepillä koko frontendistä).
- **Uusi live-tilan v1 webhook** (`checkout.session.completed`, `https://habahub.com/api/webhooks/stripe`) ja **uusi live-tilan v2 Event Destination** (`v2.core.account[configuration.recipient].capability_status_updated`, `events_from:['@self']` — sama korjattu skooppi kuin testitilassa, ks. yllä oleva "@self vs @accounts" -löydös) luotu API:lla live-avaimella. Kumpikin on oma, testitilasta erillinen rekisteröinti — Stripen webhookit/Event Destinationit ovat aina tila-kohtaisia, eivät siirry automaattisesti.
- **Google Pay + MobilePay kytketty päälle myös live-tilan maksutapa-konfiguraatioon** (`paymentMethodConfigurations`, eri objekti kuin testitilan — vahvistettu olevan oletuksena `off` myös live-puolella, sama korjaus toistettu). Apple Pay oli jo `on`.
- **⚠️ Kriittinen korjaus tehty ENNEN avainten vaihtoa:** `POST /orders/:id/pay` ja `GET /users/me/stripe-status` kutsuivat `getAccountStatus()`:ia ilman `try/catch`:ia — testitilassa luotu `stripeAccountId` ei ole olemassa live-avaimella (Stripe palauttaa "No such account"), mikä olisi kaatanut molemmat reitit käsittelemättömään 500-virheeseen JOKAISELLE myyjälle heti avainten vaihdon jälkeen. Korjattu molemmat, palauttavat nyt siistin "onboarding kesken" -vastauksen virheen sattuessa.
- **Kaikkien myyjien testitilassa luodut `stripeAccountId`-arvot tyhjennetty** (`Larzmoi`, `michaelbacklund` — ainoat kaksi joilla oli arvo) ennen avainten vaihtoa, koska ne ovat merkityksettömiä/virheellisiä live-avaimen alla — molemmat joutuvat painamaan "Yhdistä Stripe-tili" uudelleen tehdäkseen oikean live-onboardingin. **`User.verified` EI nollattu** näiltä kahdelta — se on alun perin admin-asettama käsinkytkin (2026-09-08), irrallinen tästä yhdestä Stripe-kentästä, ja pysyy tarkoituksellisesti yllä olevan "ylikirjoitus säilyy" -periaatteen mukaisesti; ainoa käytännön seuraus on että he eivät voi vastaanottaa maksuja ennen kuin yhdistävät Stripen uudelleen (jo hoidettu selkeällä virheviestillä `POST /orders/:id/pay`:ssa).
- Kaikki kertakäyttöiset skriptit jotka sisälsivät raa'an `sk_live_`-avaimen poistettu palvelimelta heti käytön jälkeen — avainta ei tallennettu mihinkään tähän tiedostoon eikä git-historiaan.

**⚠️ EI testattu, tarkoituksella:** yhtään oikeaa live-maksua ei ole ajettu läpi — se siirtäisi oikeaa rahaa. Kaikki yllä oleva on vahvistettu turvallisilla, ei-rahaa-liikuttavilla lukukutsuilla (tilin tila, webhook-rekisteröinti, maksutapakonfiguraatio). **Ensimmäinen oikea live-maksu on omistajan oma päätös/testi**, ei jotain jonka tekisin automaattisesti.
...
## Puuttuva ilmoitus "huutokaupassa olleesta kohteesta" 2026-09-09 — ✅ OIKEA BUGI LÖYTYI JA KORJATTU

Omistaja epäili ettei jokin huutokaupassa ollut kohde saanut ilmoitusta. Tutkittu suoraan tuotannosta ennen johtopäätöksiä: koko tietokannan historiassa ei ole koskaan ollut yhtään `saleType:"auction"`-tuotetta eikä yhtään `OUTBID`/`AUCTION_ENDED`/`AUCTION_SOLD`/`ORDER_WON`-ilmoitusta — ainoa koskaan bidattu tuote on "Reiska" (omistajan oma itsetesti 2026-08-13, ennen itsehuutamisen estävää korjausta).

**Ensimmäinen johtolanka ("Charmeleon (DR 99)", poistettu admin-oikeuksilla 2026-09-08) osoittautui vääräksi kohteeksi** — omistaja tarkensi: oikea kohde oli kala-/säynenaiminen testituote, lähtöhinta 1€, jompikumpi Backlund-veljeksistä teki 1,10€ testihuudon. Tuotetta ei enää löytynyt tietokannasta millään haulla (nimi, `Bid`-historia tyhjä).

**✅ OIKEA JUURISYY LÖYTYI JA KORJATTU 2026-09-09:** `DELETE /products/:id` (myyjän oma, tavallinen tuotepoisto — EI se uusi admin-nappi) esti poiston sitovan huudon takia **vain kun `saleType === "auction"`** (perinteinen huutokauppa). Live-tuotteet käyttävät lähes aina `saleType: "both"` tai `"live"` — täsmälleen samaa `Product.currentBid`/`reservePrice`-mekanismia, mutta tämä suoja ei koskenut niitä ollenkaan. Myyjä pystyi siis poistamaan kala-/säynetuotteen huolimatta aidosta 1,10€ huudosta — `Bid`-rivit siivoutuivat `deleteMany`:lla hiljaa, EI ilmoitusta kenellekään (ei huutajalle eikä edes myyjälle). Tämä rikkoi suoraan LUKITTU-sääntöä "Kaikki huudot sitovia — ei peruutuksia" kirjaimellisesti kaikilla muilla myyntitavoilla paitsi puhtaalla perinteisellä huutokaupalla.

**Korjaus:** ehto laajennettu `saleType === "auction"`:sta `saleType !== "buy_now"`:hen (`backend/src/routes/products.ts`) — vain puhdas suoramyynti ei koskaan voi saada huutoja, joten se on ainoa poikkeus. **Testattu tuotannossa oikealla HTTP-reitillä:** luotu testituote (`saleType:"both"`, `startPrice:1`), asetettu `currentBid:1.1` suoraan, poistoyritys `DELETE /products/:id` samalta myyjätililtä palautti nyt oikein `400 "Tuotetta ei voi poistaa — huuto on sitova"` (aiemmin olisi onnistunut `200`:lla). Testidata siivottu tietokannasta suoraan koska API-poisto on nyt tarkoituksella estetty.
...
**Todellinen syy löytyi nginx-lokeista:** samana päivänä aiemmin lisätty yleinen rate limiting (`100 pyyntöä/15min per IP`, ks. "Rate limiting puuttuu kokonaan" -osio) oli **liian tiukka oikealle liikenteelle**. Nginx-lokista näkyi yhden oikean Firefox-session saavan `429`:n peräjälkeen `/api/ad`, `/api/notifications`, `/api/products`, `/api/shows`, `/api/auctions`, `/api/cart`, `/api/messages` -reiteiltä ~3 minuutin sisällä. Frontend ei erottele 429-virhettä muista virheistä (`.catch(() => {})`-kuvio kaikkialla) — näytti siis tyhjän listan täsmälleen samalla tavalla kuin oikeasti tyhjä tietokanta olisi näyttänyt, vaikka data oli koko ajan tallessa.

**Juurisyy rajan liian tiukkaan asetukseen:** etusivun yksi lataus ampuu jo ~7 rinnakkaista API-kutsua (ad/notifications/products/shows/auctions/cart/messages), ja `/shows`-listaus **pollataan 20s välein** koko ajan kun etusivu on auki (ks. `frontend/app/page.tsx` — 20s pollaus valittu koska socket-pohjainen reaaliaikapäivitys jätettiin pois listaussivuilta) — pelkästään tästä kertyy 45 kutsua/15min yhdeltä avoimelta välilehdeltä. Tavallinen selailu muutamalla sivulla (Selaa, Huutokaupat, Live-kaikki, muutama tuotesivu) päällä ylitti 100:n rajan helposti ilman mitään väärinkäytöstä — pahentaa erityisesti jos samalla julkisella IP:llä (esim. kotitalous/toimisto) on useampi käyttäjä yhtä aikaa.

**Korjaus:** yleinen raja nostettu `100`:sta `1000`:aan/15min (`backend/src/index.ts`) — riittää reilusti normaaliin käyttöön mukaan lukien pollaus ja jaettu IP, rajoittaa silti selvästi poikkeavan jatkuvan automaattisen raapimisen. Kirjautumis-/rekisteröinti-/salasananpalautus-reittien oma tiukka raja (8/15min) EI muuttunut — se ei liittynyt tähän tapaukseen, on tarkoituksella tiukka bruteforce-suojaus jolla on luontaisesti matala legitiimi käyttömäärä. Palvelimen uudelleenkäynnistys deployn yhteydessä nollasi myös rate limiterin muistivaraisen laskurin kaikille, joten kukaan ei jäänyt jumiin vanhaan rajoitukseen.

**Ei vielä korjattu, tietoinen rajaus:** frontend näyttää yhä tyhjän listan (ei erillistä "liikaa pyyntöjä, yritä hetken kuluttua" -viestiä) jos 429 joskus toistuu tulevaisuudessa — täysi korjaus vaatisi jokaisen `.catch(() => {})`-kohdan läpikäynnin koko frontendissä erottelemaan 429:n muista virheistä, isompi erillinen tehtävä. Nyt nostettu raja tekee tästä käytännössä erittäin epätodennäköisen skenaarion normaalikäytössä, joten ei korjattu tässä yhteydessä.
...
**✅ PÄÄTÖS 2026-09-09: SIGNICAT/CRIIPTO HYLÄTTY KOKONAAN, EI TARVITA ERILLISTÄ TUNNISTUSPALVELUA.** Omistajan oivallus: koska Stripe Connect -tilin luonti vaatii JO NYT passikuva+selfie-tunnistautumisen osana omaa KYC-prosessiaan (yksityishenkilöille), tämä SAMA vaihe toimii myös "Vahvistettu käyttäjä" -porttivahtina — ei tarvita kahta erillistä tunnistautumiskerrosta. Kaikki aiemmat Signicat/Criipto-osiot (mukaan lukien HETU-varoitus, "ei koskaan raakaa HETU:a" -periaate) ovat nyt **historiallista referenssiä, ei enää sovellettavia** — Stripe hoitaa oman KYC:nsä kokonaan omalla vastuullaan, Habahub ei koskaan näe/käsittele passikuvia/selfieitä/HETU:a itse, ne jäävät Stripen omalle hostatulle onboarding-sivulle.

**✅ TEHTY JA DEPLOYATTU 2026-09-09 — `User.verified` asetetaan nyt automaattisesti Stripe-tilin valmiuden mukaan.**

**Toteutus:**
- Uusi `POST /webhooks/stripe-accounts` (`backend/src/routes/webhooks.ts` `handleStripeAccountWebhook`) kuuntelee Stripen v2 Core Accounts -mallin **"thin events"** -tapahtumia — ERI mekanismi kuin v1:n `checkout.session.completed`-webhook: rekisteröity omana **Event Destination**-resurssina (`POST /v2/core/event_destinations`, kertakäyttöisellä palvelinskriptillä, ei jäänyt repoon — sama periaate kuin projektin muutkin kertaluontoiset ulkoisten API:en asetuskutsut), ei Dashboardin klassisena webhook-URL:na. Oma signing secret (`STRIPE_ACCOUNT_EVENTS_SECRET`), oma reitti montattu `index.ts`:ssä samalla raaka-runko-periaatteella kuin `/webhooks/stripe`.
- Tilattu tapahtuma: `v2.core.account[configuration.recipient].capability_status_updated`.
- Kun tapahtuma saapuu, käsittelijä EI luota thin eventin omaan runkoon (se sisältää vain `related_object.id`:n, ei kapasiteetin uutta arvoa) — hakee aina tuoreen tilan `getAccountStatus()`:lla suoraan Stripeltä, ja jos `stripe_transfers`-kapasiteetti on `active`, asettaa `User.verified = true` (`updateMany({ where: { stripeAccountId, verified: false } })` — idempotentti, ei kirjoita jos jo vahvistettu).
- Admin-paneelin käsinkytkin säilyy YLIKIRJOITUKSENA, ei enää ensisijaisena reittinä.

**⚠️ KOLME virhettä löytyi ja korjattiin, jokainen vasta oikeaa tuotantotestiä vasten — ei yksikään olisi paljastunut lukemalla dokumentaatiota tai koodia:**
1. **Väärä julkinen URL.** Ensimmäinen Event Destination rekisteröitiin osoitteeseen `https://habahub.com/webhooks/stripe-accounts` — mutta nginx (`/etc/nginx/sites-available/habahub`) reitittää backendille VAIN `/api/`-alkuiset polut, kaikki muu menee frontendille (`location /`). Sama koskee jo olemassa olevaa v1-webhookia, joka on rekisteröity Stripen dashboardiin `https://habahub.com/api/webhooks/stripe`:nä — vahvistettu suoraan `stripe.webhookEndpoints.list()`:lla. Korjattu: vanha (väärä) Event Destination poistettu, uusi luotu oikealla `/api/`-etuliitteellä.
2. **Väärä allekirjoituksen tarkistusmetodi.** Alun perin käytettiin samaa `stripe.webhooks.constructEvent()`:ia kuin v1:llä (WebSearchin löydös väitti tämän toimivan molemmille) — **vahvistettu VÄÄRÄKSI suoraan Stripeä vasten**: `constructEvent` hylkäsi thin eventin virheellä *"You passed a thin event notification to a function that expects a webhook. Use the corresponding EventNotification method instead."* Oikea metodi löydetty tutkimalla asennetun `stripe`-npm-paketin prototyyppiä suoraan (ei arvattu): **`stripe.parseEventNotification(payload, signature, secret)`** (top-level, ei `stripe.webhooks`-alla), palauttaa `Stripe.V2.Core.EventNotification`-tyypin.
3. **⚠️ VAKAVIN, LÖYDETTY VASTA 2026-09-09 KUN OMISTAJA TEKI OIKEAN ONBOARDINGIN: väärä `events_from`-skooppi, koko automaatio ei koskaan olisi toiminut oikealle myyjälle.** Event Destination rekisteröitiin alun perin `events_from: ['@accounts']`:lla (looginen oletus — "haluamme tapahtumia YHDISTETYILTÄ tileiltä"), mutta Stripen oma Connect-webhook-dokumentaatio (`docs.stripe.com/connect/webhooks`) paljastaa poikkeuksen nimenomaan v2-tileille: *"For events triggered by connected accounts, v2 Events use the **Your account** scope, while v1 Events use the **Connected accounts** scope, even when triggered by the same v2 Account."* — eli juuri PÄINVASTOIN kuin v1:llä totuttu ajattelutapa. Oikea skooppi on `@self`, ei `@accounts`. **Todiste että tämä oli aidosti rikki:** omistajan tekemän oikean, täyden Stripe-onboardingin (passikuva+selfie+pankkitili, `stripe_transfers` meni `active`-tilaan klo 19:37 ja 19:41) jälkeen `grep webhooks/stripe-accounts /var/log/nginx/access.log` ei näyttänyt YHTÄÄN Stripen omaa pyyntöä — vain omat aiemmat testikutsuni. `stripe.v2.core.events.list({object_id: acct_...})` vahvisti että Stripe KYLLÄ loi oikeat `capability_status_updated`-tapahtumat oikeaan aikaan, ne eivät vain koskaan saapuneet meille väärän skoopin takia. **Korjaus:** vanha Event Destination poistettu, uusi luotu `events_from: ['@self']`:lla (uusi signing secret, `.env` päivitetty, backend uudelleenkäynnistetty). **Vahvistettu korjatuksi:** luotu uusi kertakäyttöinen testimyyjä, kutsuttu `POST /users/me/stripe-onboarding` (uusi tili), ja `nginx access.log` näytti VÄLITTÖMÄSTI kaksi oikeaa Stripen lähettämää pyyntöä (`User-Agent: Stripe/2.0 (+https://docs.stripe.com/event-destinations)`, HTTP 200) — ensimmäinen kerta koko prosessissa jolloin Stripen OMA infrastruktuuri (ei oma synteettinen testi) todella toimitti tapahtuman meille asti.

**Testattu tuotannossa (kertakäyttöiset testitilit + omistajan oma oikea onboarding, kaikki testidata siivottu):**
- **Allekirjoituksen tarkistus** (ennen `@self`-korjausta, mutta mekanismi ei muuttunut): oikein `stripe.webhooks.generateTestHeaderString()`:lla signeerattu synteettinen thin event julkista `/api/webhooks/stripe-accounts`-osoitetta vasten → `200`; väärällä secretillä → `400`.
- **Negatiivinen polku (oikea Stripe, oikea tili, ei vielä aktiivinen):** käsittelijä kutsui `getAccountStatus()`:ia, sai `transfersEnabled:false`, ei kirjoittanut mitään — vahvistettu sekä ensimmäisellä testimyyjällä (synteettinen kutsu) että TOISELLA, jolle Stripe lähetti aidon `@self`-korjauksen jälkeisen tapahtuman (`verified` pysyi `false`:na).
- **Kirjoituslogiikka eristettynä:** `updateMany({where:{stripeAccountId, verified:false}, data:{verified:true}})` — ensimmäinen ajo `count:1`, toinen `count:0` (idempotentti).
- **✅ POSITIIVINEN POLKU LOPULTA VAHVISTETTU TODEKSI, EI ENÄÄ RAJAUS:** omistaja teki oikean Stripe-onboardingin omalla Larzmoi-tililleen selaimessa (`acct_1UDrJMJZ1HMkFLjA`) — `stripe_transfers`/`payouts` molemmat `active` suoraan Stripeä vasten tarkistettuna. Koska Larzmoi oli jo ennestään `verified:true` (2026-09-08 admin-asetus), tästä yksin ei voitu päätellä toimiko koko ketju automaattisesti — siksi edellä kuvattu erillinen tuore testitili+`@self`-korjaus tarvittiin todistamaan mekanismi aidosti toimivaksi.

**⚠️ Jäljellä, ei vielä todennettu:** koko ketju "oikea uusi myyjä tekee oikean onboardingin JA hänen `User.verified`-lippunsa muuttuu `false→true` automaattisesti" ei ole nähty tapahtuvan samalle tilille päästä päähän (Larzmoi oli jo valmiiksi verified, testitili ei koskaan mennyt läpi asti active-tilaan) — mutta jokainen erillinen lenkki ketjussa (oikea tapahtuma syntyy → toimitetaan oikeaan URL:iin → allekirjoitus tarkistuu → kirjoituslogiikka toimii) on nyt vahvistettu erikseen oikeaa Stripeä/tuotantoa vasten, ei yhtäkään arvattu.

**Alla oleva kuvaus käsinkytkimestä on tekninen pohja jonka päälle yllä oleva automaatio rakennettiin — logiikka (`requireVerifiedSeller`, `POST /shows`-tarkistus) ei muuttunut, vain `verified`:n ASETTAMISTAPA muuttui manuaalisesta automaattiseksi:**

**✅ JATKO 2026-09-08, SAMANA PÄIVÄNÄ — `verified` muutettu kosmeettisesta oikeaksi porttivahdiksi.** Omistaja tarkensi: oikea suunnitelma on että JOKAINEN myyjä joutuu tekemään Signicat-tunnistautumisen (ks. "Signicat — pankkitunnistautuminen" -osio, LUKITTU, vaatii OY:tä eikä ole vielä rakennettu) ennen kuin saa myydä sivustolla — kun se on tehty, tili saa "vahvistettu käyttäjä" -merkinnän. **Kunnes oikea Signicat-integraatio on olemassa, omistaja toimii itse väliaikaisena "tunnistuspalveluna" ja myöntää/poistaa `verified`-lipun käsin admin-paneelista** yllä kuvatulla kytkimellä — tämä kytkin oli äsken vielä pelkkä kosmeettinen badge, muutettu nyt oikeaksi pääsynhallinnaksi:

- `backend/src/routes/products.ts`: uusi `requireVerifiedSeller()`-tarkistus sekä `POST /products`:ssa että `POST /products/bulk`:ssa (kattaa kaikki myyntitavat — suoramyynti, perinteinen huutokauppa, ja `/lahetys`-konsolin pikalisäys, koska kaikki kolme luovat tuotteen tämän saman reitin kautta) — palauttaa `403 "Myyminen vaatii vahvistetun käyttäjätilin. Ota yhteyttä ylläpitoon."` jos `User.verified !== true`.
- `backend/src/routes/shows.ts`: `POST /shows` (striimin aloitus) vaatii nyt SEKÄ `canStream` ETTÄ `verified`-lipun (kaksi erillistä tarkistusta, kumpikin oma selkeä virheviestinsä) — `canStream`-kytkin pysyy erillisenä (omistaja voi yhä hallita ketkä verifioiduista käyttäjistä saavat kokeilla striimausta), mutta `verified` on nyt pakollinen molemmille oikeuksille yhdessä.
...
## Mainostilan linkki rajoittui sisäisiin polkuihin 2026-09-05 — ✅ TEHTY JA DEPLOYATTU

Omistaja halusi voida linkittää mainoksen MIHIN TAHANSA osoitteeseen, ei vain sivuston omiin reitteihin. `AdBanner`:n CTA käytti aina `next/link`:iä, joka on tarkoitettu nimenomaan sivuston sisäiseen client-side-navigointiin — ulkoinen `https://`-osoite olisi periaatteessa toiminut senkin kanssa, mutta ei ole oikea komponentti ulkoisille linkeille (ei avaa uuteen välilehteen, ei `rel="noopener noreferrer"`-turvalippua). Korjattu: `frontend/app/page.tsx`:n `AdBanner` tunnistaa nyt `http(s)://`-alkuiset osoitteet ja renderöi ne tavallisena `<a target="_blank" rel="noopener noreferrer">`:na, sisäiset polut (esim. `/huutokaupat`) käyttävät yhä `next/link`:iä. Admin-lomakkeen (`AdminAdManagement.tsx`) kenttäselite päivitetty kertomaan tämä selvästi.

## Etusivun footer ei pysynyt sivun alareunassa lyhyillä sivuilla 2026-09-08 — ✅ TEHTY JA DEPLOYATTU

Omistaja raportoi: "myytävissä (suoramyynti kohteissa)" footer nousee ylös eikä pysy sivun alareunassa. Juurisyy löytyi koodista: etusivun (`frontend/app/page.tsx`) ulompi kääre käytti pelkkää `minHeight:'100vh'`:tä ILMAN `display:'flex', flexDirection:'column'`:ia — `/selaa` ja `/huutokaupat` käyttävät jo oikeaa flex-column + `flex:1`-kaavaa footerin kiinnittämiseksi näytön alareunaan kun sisältö on lyhyempi kuin yksi ruutu, mutta etusivu oli ainoa jäljellä oleva sivu jolta tämä puuttui. Ilman flexiä Footer vain seurasi sisältöä normaalissa dokumenttivirtauksessa — jos sisältö (esim. harvat suoramyyntituotteet/huutokaupat samassa kategoriafokuksessa) jäi lyhyemmäksi kuin ruutu, footer jäi kellumaan sisällön alle kesken ruutua sen sijaan että täyttäisi loppuruudun ja asettuisi ruudun alareunaan. **Korjaus:** ulompi kääre sai `display:'flex', flexDirection:'column'`, ja sivupalkki+listaus-rivi (sisältö Navbar/mainosbanneri/heron JA Footerin välissä) sai `flex:1`:n — sama muutos kuin `/selaa`:lla jo on, ei vaikuta sivuihin joilla sisältöä on enemmän kuin yksi ruudullinen (footer käyttäytyy täysin ennallaan silloin).

## Mainostilan kuva täyttää nyt koko laatikon 2026-09-07 — ✅ TEHTY JA DEPLOYATTU

Omistajan pyyntö: mainostilan kuva oli aiemmin pieni 56×56px kuvake tekstin vieressä — halusi voida käyttää koko laatikon kokoisia kuvia, ja samalla laatikon hieman korkeammaksi. `frontend/app/page.tsx`:n `AdBanner` kirjoitettu uudelleen: kun `ad.imageUrl` on asetettu, kuva renderöityy `position:absolute, inset:0, objectFit:cover`-taustana koko laatikon kokoisena, tumma liukuväri (scrim, oikealta/alhaalta tummempi) sen päällä pitää eyebrow/otsikko/kuvaus/CTA-tekstin luettavana kuvasta riippumatta — sama periaate kuin tavanomaisessa hero-bannerissa. Ilman kuvaa laatikko näyttää yhä vanhan tumman taustan + oletusikonin (ei muutosta tälle tapaukselle). Laatikon `minHeight` nostettu (260px mobiili / 320px desktop) jotta kuva ehtii näkyä kunnolla, sisältö asemoitu laatikon alaosaan (`alignItems:'flex-end'` desktopilla). Otsikon fonttikoko kasvatettu (16/19px → 18/24px) sopimaan paremmin isompaan laatikkoon.

**Kuvan laatu:** admin-lomakkeen (`AdminAdManagement.tsx`) kuvanpienennys nostettu `resizeImage(..., 400)`:sta `resizeImage(..., 1600)`:aan — 400px riitti pienelle kuvakkeelle mutta venyisi rakeiseksi koko leveän bannerin (jopa ~1440px) taustalla. Admin-lomakkeen esikatselulaatikko vaihdettu 56×56-neliöstä 220×90-suorakaiteeksi paremmin edustamaan uutta bannerimuotoa, kenttäselite päivitetty kertomaan että kuva täyttää koko laatikon liukuvärin kanssa.

## Paytrail ja myyjien tilinumerot — selvennys 2026-09-05, EI vaadi profiilikenttää

Omistaja kysyi pitäisikö profiiliin lisätä kenttä myyjän tilinumerolle Paytrailia varten. **Vastaus: ei — tämä ei ole tarpeen eikä oikea ratkaisu Shop-in-Shop-mallissa**, ks. tarkempi selvitys "Paytrail-maksuintegraatio"-osiosta ja "Viisi myöhemmin tarkistettavaa asiaa 2026-09-04" kohta 1:

- Paytrailin Shop-in-Shop-malli EI toimi niin että Habahub kerää/tallentaa myyjien pankkitilinumeroita ja tekee itse tilisiirtoja. Sen sijaan **jokainen myyjä rekisteröityy omaksi "sub-merchantiksi" suoraan Paytrailin kanssa** (erillinen sopimusprosessi Paytrailin oman onboarding-virran kautta, ei meidän sovelluksemme läpi) — myyjän omat pankkitiedot annetaan ja tallennetaan PAYTRAILIN järjestelmään sen oman onboardingin yhteydessä, ei koskaan Habahubin omaan tietokantaan.
- Koodi on jo rakennettu tätä mallia varten: jokaisen tilausrivin `items[].merchant`-kenttä (`backend/src/lib/paytrail.ts`) osoittaa myyjän sub-merchant-ID:hen, ja Paytrail itse jakaa maksun automaattisesti Habahubin komission ja myyjän osuuden välillä maksutapahtuman yhteydessä — ei vaadi omaa "pidätetty saldo" -kirjanpitoa eikä tilinumeroiden käsittelyä sovelluksessa ollenkaan.
- **Miksi tämä on myös oikea valinta, ei vain teknisesti helpompi:** raakojen pankkitilinumeroiden/IBAN:ien tallentaminen omaan tietokantaan toisi turvallisuus-/vastuuriskin (PCI-tyyppinen arkaluontoinen data) jota ei tarvitse ottaa kantaakseen kun Paytrail hoitaa tämän jo valmiiksi omassa, säännellyssä järjestelmässään.
- **Ainoa oikeasti kesken oleva asia on sub-merchant-onboarding Paytrailin KANSSA, ei koodi:** testivaiheessa kaikki myyjät jakavat saman testisopimuksen submerchant-ID:n (695874), oikeat per-myyjä submerchant-ID:t vaativat sopimusprosessin Paytrailin kanssa tuotantoon siirryttäessä. Tämä on jumissa duplikaattitilien takia (ks. "Viisi myöhemmin tarkistettavaa asiaa 2026-09-04" kohta 1: "emoyhtiön/toisen yhtiön nimissä oli avattu duplikaattitunnukset") — puhtaasti sopimusasia Paytrailin kanssa, ei jää odottamaan mitään profiilikenttää tai muuta koodia meidän puoleltamme.

## Paytrail-alakauppiaan (sub-merchant) onboarding — tutkittu 2026-09-05, KAKSI AVOINTA KYSYMYSTÄ ratkaisematta

Omistaja toivoi että myyjän Paytrail-rekisteröinti olisi "ei sen ihmeellisempi kuin nopea mobiilipankkivarmenne". Tutkittu suoraan Paytrailin julkisesta dokumentaatiosta (WebSearch+WebFetch, `support.paytrail.com`-artikkelit palauttivat 403:n botille, luotettu hakutuloshahmotelmiin+kahteen onnistuneeseen fetchiin `paytrail.com/en/sis-sub-merchant` ja `/en/shop-in-shop`:sta).

**Prosessi kaksiosainen, vain jälkimmäinen osa on toivotun kaltainen:**
1. **Habahubin (ei myyjän) pitää ensin luoda alakauppias-rivi Paytrailin omassa Kumppaniportaalissa (Partner Portal)** — manuaalisesti, "Lisää uusi kauppias" -painike, syöttäen myyjän Y-tunnus, kauppapaikan URL (habahub.com) ja myyjän sähköposti. **Ei löydetty julkista API:a tälle luontivaiheelle** — vain jälkikäteinen "Merchant API" (hyvitykset/tilitykset), ei alakauppiaan luontiin. Tämä tarkoittaa: täysin itsepalvelu (myyjä rekisteröityy itse ilman Habahubin väliintuloa) ei ole mahdollista ainakaan julkisen dokumentaation perusteella.
2. **Vasta tämän jälkeen myyjä saa sähköpostikutsun**, kirjautuu omaan uuteen Paytrailin Merchant Panel -tiliinsä, täyttää lisätiedot ja **allekirjoittaa sopimuksen sähköisesti OMILLA VERKKOPANKKITUNNUKSILLAAN** — tämä osa VASTAA täsmälleen omistajan toivetta (sama tunnistautumiskokemus kuin mobiilipankkivarmenne), ei vaadi mitään erillistä integraatiota meiltä. Jos myyjällä ei ole suomalaisia pankkitunnuksia, varapolku on paperinen (tulosta, allekirjoita, passikopio, sähköposti Paytrailille) — hitaampi, koskee lähinnä ulkomaisia myyjiä.

**⚠️ KRIITTINEN, RATKAISEMATON KYSYMYS — määrää koko arkkitehtuurin:** dokumentaatio mainitsee erillisen polun "yksityisille elinkeinonharjoittajille" (private trader), jossa Y-tunnusrivin sijaan annetaan henkilön nimi+henkilötunnus+osoite ("tosiasiallinen edunsaaja") — mutta EI selvinnyt vaatiiko tämä silti kevyttä toiminimi-rekisteröintiä (Y-tunnus YTJ:stä, ilmainen/nopea mutta silti erillinen askel), vai voiko TÄYSIN REKISTERÖIMÄTÖN yksityishenkilö (= suurin osa Habahubin todellisista myyjistä, esim. kortteja myyvä harrastaja ilman mitään yritystä) ylipäätään toimia alakauppiaana. **Jos vastaus on "ei, tarvitaan aina vähintään Y-tunnus"**, nykyinen "jokainen myyjä oma automaattinen sub-merchant" -tavoite ei toimi suoraan yksityismyyjille — silloin täytyy joko (a) vaatia jokaiselta myyjältä toiminimi-rekisteröinti ennen myyntiä (iso kynnys), tai (b) pitää yksityismyyjät yhä yhteisen/jaetun submerchant-tilin alla ja hoitaa HABAHUBIN oma sisäinen tilitys heille jotenkin muuten (mikä avaisi uudelleen kysymyksen tilinumeroiden tallennuksesta/käsittelystä, ks. edellinen "Paytrail ja myyjien tilinumerot" -osio).

**⬜ Seuraava askel, ei koodattu ennen vastauksia:** kysy Paytraililta suoraan (esim. tech@paytrail.com tai olemassa oleva yhteyshenkilö):
1. Voiko täysin rekisteröimätön yksityishenkilö (ei Y-tunnusta ollenkaan) toimia Shop-in-Shop-alakauppiaana henkilötunnuksellaan, vai vaaditaanko aina vähintään kevyt toiminimi-rekisteröinti?
2. Onko alakauppiaan LUONTIIN (askel 1 yllä, ei vain jälkikäteisiin hyvityksiin/tilityksiin) olemassa mitään API:a, jolla Habahub voisi automatisoida Kumppaniportaalin klikkaukset omasta sovelluksesta sen sijaan että joku käy tekemässä sen käsin jokaiselle uudelle myyjälle?

Ei rakennettu mitään UI:ta/profiilikenttää/koodia tähän vielä — nämä kaksi vastausta määräävät suoraan minkä muotoinen koko onboarding-virta voi edes olla, samalla periaatteella kuin muutkin Paytrail/Posti-integraatiot tässä projektissa (ei arvata, odotetaan vahvistusta ennen koodausta).
...
1. **✅ TEHTY JA DEPLOYATTU 2026-09-05.** `ProductPreset.startPrice Float?` lisätty schemaan (valinnainen — osa pohjista tarkoituksella ilman kiinteää oletushintaa). `/dashboard/esiasetukset`-lomakkeeseen lisätty syöttökenttä + näkyy pohjan listarivillä. Sekä `/lahetys`-live-konsolin pikalisäys että `/dashboard/tuotteet`-lomakkeen esiasetuspoiminta (ks. kohta 2 alla) esitäyttävät lähtöhinnan pohjan arvosta jos sellainen on asetettu — myyjä voi silti muuttaa sitä, ei lukittu.

2. **✅ TEHTY JA DEPLOYATTU 2026-09-05.** Lisätty "Tallenna tämä esiasetukseksi" -valintaruutu `dashboard/tuotteet`-lomakkeeseen (näkyy vain uutta tuotetta lisättäessä). Rastittuna luo `ProductPreset`-rivin samoilla arvoilla (nimi/kunto/kategoria/alakategoria/tyyppi/ensimmäinen kuva/kuvaus/lähtöhinta) tuotteen onnistuneen tallennuksen yhteydessä.

3. **✅ TEHTY JA DEPLOYATTU 2026-09-05.** Väärinymmärrys korjattiin jo 2026-09-02: kyse ei ollut esiasetusten suosikeista vaan **ostajan Shop-paneelin tuotejärjestyksestä livessä**. Jatkotutkinnassa selvisi että `Product.order Int @default(0)` -kenttä oli itse asiassa jo olemassa schemassa (ei tiedetä milloin lisätty, dokumentoimaton — ks. "Työskentelytapa"-osio rinnakkaisesta kanavasta), ja `GET /shows` + `GET /shows/:id` käyttivät sitä jo `orderBy`:na — mutta MIKÄÄN ei koskaan KIRJOITTANUT siihen, joten se oli aina 0 kaikilla tuotteilla. `/lahetys`-sivun Jono-paneelin raahaus (`handleDrop`) oli edelleen puhtaasti paikallista React-tilaa. **Korjaus:** uusi `PATCH /shows/:id/reorder` (`backend/src/routes/shows.ts`) ottaa tuote-ID-listan halutussa järjestyksessä ja kirjoittaa `order`-kentän (indeksin mukaan) transaktiossa; `handleDrop` kutsuu tätä jokaisen raahauksen jälkeen. `GET /products/mine` (jota `/lahetys` käyttää jonon lataukseen) vaihdettu `createdAt desc`:stä `order asc, createdAt desc`:hen, jotta sivun päivitys ei enää nollaa järjestystä takaisin luontijärjestykseen. Ostajan Shop-paneelin "Järjestys"-oletus ei vaatinut mitään muutosta — se jo luotti suoraan `products`-propin järjestykseen, joka nyt tulee oikeana backendiltä. **Testattu tuotannossa oikealla PATCH-kutsulla:** kahden tuotteen järjestys käännetty, `GET /shows/:id` palautti uuden järjestyksen heti oikein.
...
## Lomamoodin kuvausteksti vanhentunut 2026-09-05 — ✅ TEHTY JA DEPLOYATTU 2026-09-06
...
## Myyjän virheviesti ohjaa yhä "ota yhteyttä ylläpitoon" 2026-09-09 — ✅ TEHTY JA DEPLOYATTU

`requireVerifiedSeller()` (`backend/src/routes/products.ts`) ja `POST /shows`:n `verified`-tarkistus (`backend/src/routes/shows.ts`) palauttivat `403 "...Ota yhteyttä ylläpitoon."` — vanhentunut teksti, koska vahvistus tapahtuu nyt automaattisesti Stripe Connect -onboardingin kautta (ks. "PÄÄTÖS 2026-09-09: SIGNICAT/CRIIPTO HYLÄTTY" -osio), ei ottamalla yhteyttä kehenkään.

**Toteutus:**
- Kaikki kolme 403-vastausta (`POST /products`, `POST /products/bulk`, `POST /shows`) päivitetty: *"Myyminen/Striimaaminen vaatii vahvistetun maksutilin. Siirry Tilitykset-sivulle vahvistaaksesi tilisi."* + uusi `code: 'SELLER_NOT_VERIFIED'`-kenttä vastauksessa.
- `frontend/lib/api.ts`:n jaettu `request()`-funktio kiinnittää nyt `code`-kentän (jos backend palautti sellaisen) heitetyn `Error`-olion `.code`-propertyyn — antaa kutsujan erottaa TÄMÄN 403:n muista virhetilanteista ilman hauraata suomenkielisen tekstin vertailua.
- `frontend/app/dashboard/tuotteet/page.tsx` (tuotteen lisäyslomake, sekä yksittäis- että bulkkiluonti) ja `frontend/app/lahetys/page.tsx` (sekä lähetyksen luonti `createShow()` että pikalisäyksen `quickAddProduct()`) näyttävät nyt selkeän "Siirry Tilitykset-sivulle" -painikkeen (`<Link href="/dashboard/tilitykset">`) suoraan virheviestin vieressä kun `errorCode === 'SELLER_NOT_VERIFIED'`, ei jätetä käyttäjää pelkän tekstin varaan.
- Typecheck+build vihreä molemmilla puolilla, deployattu tuotantoon.
...
- **Käännökset — JATKETTU 2026-09-09, EI VIELÄ TÄYSI AUDITOINTI.** Aiemmin korjattu tuotesivun (`tuotteet/[id]`) jaa/tarjous/osto-virheviestit, perinteisen huutokaupan (`huutokauppa/[id]`) huuto/automaattihuuto/osta-viestit, ja huutokauppalistan (`huutokaupat`) otsikko/lajittelu/laskuri/"Päättynyt". **✅ TEHTY JA DEPLOYATTU 2026-09-09: `/ostot`-sivu (ostohistoria).** Kaikki tilarivien otsikot ("Odottaa maksua"/"Odottaa lähetystä"/"Reklamoitu" + uudelleenkäytetty jo olemassa olleet `shipped`/`delivered`-avaimet), maksu-/toimitusaika-laskurit, virheviestit, painikkeet, placeholderit ja reklamaatio-/arvostelulomakkeiden tekstit siirretty `t.purchases.*`-avaimiin (~42 uutta avainta fi/en/sv) — sivu käytti aiemmin kokonaan raakaa suomea näissä kohdissa vaikka `t.purchases.title`/`subtitle`/`trackingCode`/`pickupCodeLabel`/`pickupCodeHint` olivat jo käytössä. **Ei kattava koko-sivuston auditointi** — muita sivuja ei ole vielä käyty systemaattisesti läpi. Jäljellä oleva täysi auditointi on oma, isompi tehtävänsä.
...
- **Kommenttikentästä pitäisi poimia enemmän kuin pelkkä vapaateksti** (nyt koko kommentti menee sellaisenaan `description`-kenttään, mikä on hyvä perusta mutta ei riitä rakenteelliseen hakuun/näyttöön):
  - **✅ TEHTY JA DEPLOYATTU 2026-09-05.** `/lahetys`-konsolin Jono-paneeliin lisätty hakukenttä (näkyy kun jonossa on yli 5 tuotetta), suodattaa nimen JA `description`-kentän (johon fyysinen sijainti kuten "Map W1" tallentuu) perusteella — myyjä löytää kortin sijainnin mukaan ilman koko listan selaamista. Suodattaa vain näkyvää listaa, ei `products`-taulukon järjestystä, joten raahaus (`handleDrop`, indeksit `products.indexOf(p)`) pysyy oikeana.
  ...
  - **✅ TEHTY JA DEPLOYATTU 2026-09-05.** Puolikunnot lisätty additiivisesti `frontend/lib/conditions.ts`:n `CARDMARKET_KUNTOLUOKAT`-taulukkoon (NM+/NM-/EX+/EX-/GD+/GD- keskimmäisille asteille, M ja PO ennallaan) — vanhat 7 tunnusta pysyvät muuttumattomina, jo tallennettu data on yhä validia. Sama jaettu taulukko kattaa suodattimet (`CategorySidebar.tsx`) ja bulkkiparserin validoinnin (`dashboard/tuotteet`) automaattisesti ilman erillistä muutosta niihin.
  - **✅ TEHTY JA DEPLOYATTU 2026-09-05.** `Product.reverseHolo Boolean @default(false)` — valintaruutu tuotelomakkeessa kun `tyyppi === 'irtokortit'`, näkyy myyjän omassa tuotelistassa. **Ei laajennettu** julkisiin tuotekortteihin/-sivuihin (ProductCard, tuotesivu) ajan puutteessa — tallentuu ja näkyy myyjälle, mutta ostaja ei vielä näe merkintää selatessaan.

### 2. Live-lähetykset / "Live Shop" — uusia ongelmia ja ideoita

**✅ KAIKKI NELJÄ KOHTAA TEHTY 2026-09-03/2026-09-05.** Lähtöhinnan muokkaus + esiasetukset/suosikit/haku tehtiin ensin 2026-09-03 (ks. alla tarkat toteutuspäätökset). Loput kaksi tehty 2026-09-05:

- **Tuplamyyntiriski — ✅ TEHTY JA DEPLOYATTU, todellinen bugi vahvistettu koodista.** `socket.ts`:n `start_auction`-käsittelijä ei koskaan tarkistanut tuotteen `status`-kenttää ennen huudon käynnistämistä. `saleType:"both"`-tuote joka ostettiin suoraan (`POST /cart/add`, `source:"direct"`) samaan aikaan kun se oli yhä myyjän live-jonossa, olisi voitu silti huutaa myytäväksi livenä — kaksi eri Orderia samasta fyysisestä esineestä. Korjattu: tarkistetaan `product.status === 'PENDING'` ennen käynnistystä, muuten huutokauppa ei käynnisty ja myyjä näkee selkeän syyn feedissä (uusi `start_auction_error`-socket-tapahtuma).
- **Automaattinen varastosaldon päivitys — ✅ TEHTY JA DEPLOYATTU.** Varaston atominen vähennys itsessään oli jo oikein (`cart.ts`:n `updateMany`-ehto estää ylivarauksen), mutta MIKÄÄN kolmesta varastoa muuttavasta kohdasta (`/cart/add`, vanhentuneiden korituotteiden vapautus, checkoutin lopullinen SOLD-merkintä) ei koskaan ilmoittanut asiasta kyseisen tuotteen show'lle — katsojan Shop-paneeli näytti siis vanhentunutta saldoa kunnes jokin muu tapahtuma (esim. seuraava huuto) sattumalta laukaisi uudelleenhaun. Kaikki kolme kohtaa lähettävät nyt `products_updated`-tapahtuman kyseiselle show'lle (`emitToShow`) jos tuotteella on `showId`.
...
- **Välityspalkkio (myyjältä) — PÄIVITETTY 2026-09-10: 3,5%, väh. 0,30€, max 35€** (3,5% on oikea prosentti, 35€ kattoa ei muutettu — uusi on 0,30€ minimikomissio, ks. "Stripen minimimaksu..." -osio: alle n. 12,50€ hintaisilla tuotteilla pelkkä 3,5% ei kattaisi Stripen omaa ~1,5%+0,25€-käsittelymaksua. Minimi koskee VAIN oletuslaskentaa — ei 14pv-tutustumispromoa eikä adminin asettamaa mukautettua komissiota, molemmat pysyvät tarkoituksellisina poikkeuksina).
...
- **Yhdistetty lähetys — MUUTETTU 2026-09-14, oli aiemmin 6h:** sama myyjä + 24h aikaikkuna (laskettuna ensimmäisestä maksamattomasta ostoksesta) = yksi tilaus, yksi postikulut. Toimii vain niin kauan kuin tilausta ei ole vielä maksettu — maksun jälkeen yhdistäminen lukkiutuu, uusi ostos samalta myyjältä aloittaa aina oman erillisen tilauksensa (ks. "Tilausten yhdistämisikkuna 6h → 24h" -osio).
...
## Postihinnat (ostaja maksaa) — PÄIVITETTY 2026-09-11, LUKITTU (korvaa 2026-08-26-version)
**Yksi kiinteä hinta ostajalle: 7,90€ (nostettu 6,90€:sta 2026-09-11, omistajan päätös), ei koskaan muutu.** **Ei kokovalintaa ostajalle eikä tuotteen lisäyslomakkeessa** — tämä osa pysyy ennallaan 2026-08-25:n päätöksestä.

**PAKETTIKOON VALINTA POISTETTU KOKONAAN 2026-09-11 (korvaa aiemman "myyjä valitsee lähetysvaiheessa" -mallin).** Aiempi kaksi vaihtoehtoa (PIENI/ISO, eri `serviceId`) korvattu YHDELLÄ kiinteällä Posti-konfiguraatiolla kaikelle (`backend/src/lib/postiClient.ts`: `POSTI_SERVICE_ID`/`POSTI_PACKAGE_CODE`/`POSTI_WEIGHT_KG`) — PIENI-vaihtoehto (PO2102) oli aina vain vahvistamaton arvaus, ei koskaan oikeasti testattu, joten yksinkertaistus poisti samalla mahdollisen riskin. Myyjä ei enää tee mitään pakettikokoon liittyvää valintaa missään vaiheessa.

**Toimitusvaihtoehdot (ennallaan):**
1. **Postitus** — kiinteä 7,90€ ostajalle, sama Posti-konfiguraatio joka lähetykselle
2. **Nouto** — kuuluu maksuturvan piiriin noutokoodilla (ks. "Noutotuotteet"-sääntö)

**Tekninen huomio VS Coden Claudelle:** älä lisää pakettikoon valintaa takaisin mihinkään (ei tuotteen lisäyslomakkeeseen, ei checkoutiin, eikä myyjän lähetys-/tilaustenhallintanäkymään) — tämä on nyt tarkoituksella yksi kiinteä arvo koko flow'lle, ei koskaan käyttäjän valittavissa.
...
3. ✅ **Maksut — Stripe Connect, TEHTY 2026-09-09, testitunnuksilla** (korvasi Paytrailin kokonaan koska Paytrailin sub-merchant-onboarding vaati aina Y-tunnuksen — ks. "Paytrail → Stripe Connect" -osio tiedoston alussa täydelliselle selvitykselle. Alla oleva "Paytrail-maksuintegraatio"-osio on historiallista referenssiä, ei enää ajantasainen.)
...
## Paytrail-maksuintegraatio — ⚠️ HISTORIALLINEN, KORVATTU STRIPE CONNECTILLA 2026-09-09

**⚠️ Paytrail on poistettu koodista kokonaan (`backend/src/lib/paytrail.ts` ei enää olemassa) — ks. "Paytrail → Stripe Connect" -osio tiedoston alussa ajantasaiselle maksuarkkitehtuurille. Kaikki alla oleva on säilytetty vain historiallisena referenssinä (miksi Shop-in-Shop-malli rakennettiin, miten se testattiin) — älä koodaa tämän mukaan.**
...
**✅ RISTIRIITA KORJATTU 2026-09-05: aktiivinen hyväksyntä vs. passiivinen aikaraja eroteltu.** Aiempi teksti antoi ymmärtää että 3 vrk:n reklamaatio-oikeus (6.3) säilyisi aina, myös sen jälkeen kun ostaja on itse aktiivisesti klikannut "Hyväksyn" — looginen ristiriita, koska aktiivisen hyväksynnän koko tarkoitus on "kaikki kunnossa tuotteessa". **Korjattu sääntö:**
- **Ostaja klikkaa aktiivisesti "Hyväksyn"** → lopullinen, EI enää 3 vrk:n reklamaatio-oikeutta sen jälkeen
- **Ostaja EI tee mitään, 24h/14pv-automaattivapautus laukeaa passiivisesti** → 3 vrk:n reklamaatio-oikeus SÄILYY, koska ostaja ei koskaan itse vahvistanut mitään olevan kunnossa
- Päivitä käyttöehdot 6.3 + FAQ + i18n-tekstit tämän eron mukaisiksi, jos ne eivät jo tee tätä eroa selväksi

**✅ UUSI LÖYDÖS 2026-09-05: Habahubin oma vastuu Posti-reklamaatiosta puuttuu eskalaatiosuunnitelmasta.** Jos paketti oikeasti katoaa (ei vain viivästy, päivä 5-14 -eskalaatio ei koskaan johda toimitusvahvistukseen), **Habahub — ei ostaja eikä myyjä — on se joka voi/pitäisi reklamoida Postille**, koska Habahub on logistiikkasopimuksen (691317) haltija, ei yksittäinen myyjä tai ostaja. Nykyinen päivä 5/10/14-eskalaatio kertoo vain osapuolille tilanteesta, mutta ei mainitse mitään Habahubin omasta aktiivisesta toimenpiteestä (Posti-reklamaation aloittaminen mahdollisen korvauksen saamiseksi). **Lisää tämä konkreettisena toimenpiteenä päivä 10-14 väliin**, esim. admin-paneelin muistutuksena/tehtävälistana kadonneista paketeista.

**✅ TEHTY JA DEPLOYATTU 2026-09-06.** `backend/src/jobs/deliveryTimeline.ts`:n päivä 14 -haaraan lisätty uusi `NotificationType.ADMIN_LOST_PACKAGE_REVIEW` -ilmoitus kaikille `role: 'ADMIN'`-käyttäjille (haettu kannasta, ei kovakoodattu) heti kun automaattinen vapautus laukeaa ilman toimitusvahvistusta — kertoo suoraan tilausId:n ja muistuttaa Habahubin omasta mahdollisuudesta reklamoida Postille sopimuksenhaltijana (691317). Käyttää olemassa olevaa ilmoitusjärjestelmää (näkyy `/ilmoitukset`-sivulla, jota admin — eli omistaja — jo käyttää) sen sijaan että rakennettaisiin kokonaan uusi erillinen tehtävälista-UI — riittävä "muistutus" ilman ylimääräistä käyttöliittymää.

**✅ TEHTY JA DEPLOYATTU 2026-09-06 — reklamaatio-oikeuden erottelu, aktiivinen vs. passiivinen.** Tekstien lisäksi paljastui ETTÄ TÄTÄ EI OLLUT TOTEUTETTU KOODISSA OLLENKAAN: `POST /orders/:id/dispute` (`backend/src/routes/orders.ts`) esti reklamaation heti kun `order.status !== 'SHIPPED'` — eli heti kun tilaus siirtyi DELIVERED-tilaan (joko ostajan omalla aktiivisella kuittauksella TAI passiivisella 14pv-automaatiolla), reklamointi oli jo mahdotonta MOLEMMISSA tapauksissa. Tämä oli suorassa ristiriidassa käyttöehtojen 6.3:n lupaaman 3 vrk:n reklamaatio-oikeuden kanssa passiiviselle tapaukselle — pelkkä tekstikorjaus olisi luvannut oikeuden jota sovellus ei olisi teknisesti sallinut käyttää. **Korjattu:** reklamointi sallitaan nyt myös kun `status === 'DELIVERED' && deliveryConfirmedAt === null` (= passiivinen vapautus, ei uutta kenttää tarvittu) ja alle 3 vrk on kulunut `order.updatedAt`:sta (= DELIVERED-siirtymän ajankohta, koska mikään muu ei koske riviä sen jälkeen). Frontend (`frontend/app/ostot/page.tsx`): DELIVERED-osioon lisätty sama "Ilmoita ongelmasta" -lomake kuin SHIPPED-osiolla jo oli, näkyy vain kun `!order.deliveryConfirmedAt` ja jäljellä olevat päivät lasketaan ja näytetään ostajalle. Aktiivisen kuittauksen tapaus ei vaatinut koodimuutosta — se toimi jo oikein (dispute estyy heti). **Käyttöehdot 6.3 + 7 (FI/EN/SV), FAQ (uusi kysymys ostajan oppaassa kaikissa kolmessa kielessä) ja i18n `protectionDesc`/`protectionPoints` (fi/en/sv) päivitetty tekemään ero selväksi**, mukaan lukien `frontend/app/kayttoehdot/content.ts`:n rivi 146 (lomamoodin kuvaus) "48 tunnista" → "4 vuorokaudesta" — SV-versio oli jo entuudestaan oikein, EN-versiolla ei ole erillistä lomamoodi-kohtaa (lyhennetty käännös). **Testattu tuotannossa kahdella kertakäyttöisellä DELIVERED-tilaisella testitilauksella** (testiuser/testi2user): passiivinen (`deliveryConfirmedAt: null`) → reklamaatio onnistui (`200`, `status:"DISPUTED"`); aktiivinen (`deliveryConfirmedAt` asetettu) → reklamaatio torjuttiin oikein (`400 "Tilaus ei ole enää reklamoitavissa"`). Testidata siivottu.
...
- **Stripe Connect** — maksut ovat jo LUKITTU Paytrail, ei muuteta *(vanhentunut 2026-08-09-päätös, koskee vain tuon ajan videoinfra-keskustelua — Paytrail korvattiin Stripe Connectilla 2026-09-09, ks. "Paytrail → Stripe Connect" -osio tiedoston alussa)*
...
**⚠️ Huom, ei oteta käyttöön Whatnotilta:** "Swipe to Bid/Buy" ja "Auto-Authorize" (automaattinen kortilta veloitus heti voiton jälkeen) — ristiriidassa LUKITTU-sääntöjen kanssa (ei pakollista kortintallennusta, 2h maksuaika, ostaja valitsee maksutavan). *(Alkuperäinen "ei myöskään Stripe" -maininta vanhentunut: Stripe Connect korvasi Paytrailin 2026-09-09, ks. "Paytrail → Stripe Connect" -osio tiedoston alussa — Auto-Authorize-tyyppinen pakkoveloitus on silti yhä LUKITTU-sääntöjen vastainen riippumatta maksupalveluntarjoajasta.)*
...
**✅ KORJATTU 2026-09-06 — omistajan pyyntö: ennakkotarjous ei saa ohjata pois livehuutokaupasta.** Katsojan `/live/[showId]`-sivun Shop-paneelin "Pre-bid"-nappi teki `router.push('/tuotteet/${productId}')` — vei katsojan kokonaan pois streamista pelkän ennakkotarjouksen jättämiseksi. Korjattu: nappi avaa nyt saman sivun sisäisen `ProductDetailModal`in (sama jota rivin klikkaus jo käyttää tuotteen tietojen näyttöön), ja modaali sisältää nyt oikean tarjouslomakkeen (määrä, minimitarjous, `POST /products/:id/prebid`) kun tuote ei ole juuri sillä hetkellä aktiivisena huudettava lotti (`isPreBiddable = product.id !== auction.productId`, sama sääntö kuin ShopPanelin riveillä jo oli). `goToPreBid`-funktio ja `onPreBid`-propti poistettu kokonaan. **Sivulöydös samalla:** `POST /products/:id/prebid` ei koskaan lähettänyt `products_updated`-socket-tapahtumaa show'lle — muut samaa lähetystä katsovat eivät nähneet päivittynyttä `currentBid`-arvoa reaaliajassa, Shop-paneeli/modaali näytti vanhentunutta hintaa kunnes jokin muu tapahtuma sattumalta laukaisi uudelleenhaun. Lisätty sama `emitToShow(showId, 'products_updated', {})` -kutsu kuin muuallakin (ks. "Live-lähetykset" -osion varastosaldo-korjaus). Testattu tuotannossa oikealla kertakäyttöisellä show+tuote-parilla: ennakkotarjous 15€ meni läpi, `currentBid`/`currentBidderId` päivittyivät oikein, ei kaatunut emit-kutsuun. Testidata siivottu.
