...
**✅ TEHTY 2026-08-14:** `/lahetys`-konsolin oma esikatselu (`HlsPreview`) näyttää nyt pysyvän pienen tilastobadgen ("1280×720 · 30fps · 1850 kbps") vasemmassa alakulmassa aina kun kuva on aktiivinen — punakeltainen jos korkeus jää alle 720p:n, muuten vihreä. Lukemat tulevat oikeasta WebRTC-tilastosta (`getRTCStatsReport()`:n `inbound-rtp`-video-entry: `frameWidth`/`frameHeight`/`framesPerSecond` + laskettu bittinopeus `bytesReceived`-deltasta 2s välein), ei arviota. Toimii riippumatta julkaisutavasta (OBS/RTMP-Ingress tai puhelimen suora WebRTC-julkaisu), koska molemmat päätyvät samaan trackiin samassa huoneessa.

**23. Striimin kuvanlaatu liian matala — arviolta 200-360p, tavoite 720p-1080p.** Omistaja striimasi puhelimella (todennäköisesti RTMP-sovelluksella, ei OBS työpöydällä), ja kuvanlaatu vaikutti olevan selvästi matalampi kuin odotettu suurimman osan ajasta. **Vaatii tutkimista kahdesta suunnasta:**
1. **Lähdepään asetukset** — millä resoluutiolla/bittinopeudella puhelimen RTMP-sovellus lähettää? Jos lähde itsessään lähettää matalalla resoluutiolla, mikään palvelinpään säätö ei auta.
2. **LiveKit Ingress -asetukset** — onko RTMP-vastaanotossa (Ingress) jokin oletusresoluutiokatto tai transkoodausasetus joka rajoittaa laatua? Tarkista myös onko käytössä simulcast/adaptiivinen bittinopeus joka voisi laskea laatua epävakaan verkon vuoksi (mahdollisesti oikein/tarkoituksenmukaista mobiiliverkolla, mutta tavoite on silti päästä 720p-1080p kun yhteys sen sallii).
**Tavoite: 720p vähintään, mieluiten 1080p, kun lähde-/verkko-olosuhteet sen sallivat.**
**✅ JUURISYY LÖYTYI JA KORJATTU 2026-08-14:** LiveKit Ingress transkoodaa RTMP-tulon AINA uudelleen (ei koskaan pass-through), ja ilman eksplisiittistä `video`-asetusta se käyttää oletuspresettiä `H264_720P_30FPS_3_LAYERS` — **1280×720 / 1900kbps KATTONA riippumatta siitä millä resoluutiolla lähde (OBS tai puhelimen RTMP-sovellus) oikeasti lähettää.** `backend/src/lib/livekit.ts`:n `createSellerIngress()` ei asettanut `video`-kenttää ollenkaan — korjattu asettamaan `H264_1080P_30FPS_3_LAYERS`-preesetti (1920×1080 pääkerros, 3500kbps, 3 simulcast-kerrosta säilyy joten heikko mobiiliverkko silti mukautuu alaspäin automaattisesti — vain yläraja nousi 720p:stä 1080p:hen). `getOrCreateStreamKey()` päivittää nyt myös olemassa olevien myyjien Ingressin (`updateIngress`) tähän asetukseen ilman että streamKey/OBS-konfiguraatio muuttuu.
- **Toinen löytynyt syy (WebRTC-julkaisupolku, "Ilman OBS:aa"):** `frontend/app/lahetys/page.tsx`:n `startCamera()` kutsui `getUserMedia({ video: true })` ilman resoluutiovaatimusta — selain valitsee tällöin usein kameran maksimia paljon pienemmän oletusresoluution. Korjattu lisäämällä `width: {ideal:1920}, height: {ideal:1080}, frameRate: {ideal:30}` — `ideal` ei kaadu laitteilla joiden kamera ei yllä 1080p:hen, selain valitsee lähimmän tuetun.
- **Ei korjattavissa koodista:** jos lähde on kolmannen osapuolen puhelimen RTMP-sovellus (esim. Larix), sen OMAT resoluutio-/bittinopeusasetukset ovat sovelluksen sisällä — nyt kun Ingressin katto on 1080p/3500kbps, kannattaa tarkistaa että RTMP-sovellus on itsekin asetettu lähettämään ~1080p/3-4.5 Mbps, muuten katto ei auta koska syöte on jo pienempi.
- Simulcast oli jo oletuksena päällä (`simulcast: true` on `livekit-client`:n oletus, ei muutettu) — automaattinen laadun pudotus heikolla yhteydellä toimii jo, ei vaatinut erillistä työtä.
