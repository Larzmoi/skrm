...
- **KRIITTINEN TUOTEPÄÄTÖS-TASON LÖYDÖS: OmaPosti Pro API v2 ei ole labelless.** Vastaus sisältää `prints[]`-taulukon jossa `pdf_type:"ADDRESSLABEL"`, `shipment.status:"PRINTED"` — API palauttaa aina tulostettavan PDF-osoitetarran, ei koskaan Vinted-tyylistä käsin kirjoitettavaa koodia. Tämä ei ole pelkkä tekninen yksityiskohta — se tarkoittaa myyjän pitää tulostaa ja kiinnittää fyysinen tarra, ei voi vain kirjoittaa koodia pakettiin. "Sending Code API" (jos sitä ylipäätään on tarpeen käyttää) olisi Postin eri, erillinen tuote, ei tämän saman kutsun sivutuote.
