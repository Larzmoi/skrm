...
- Tilattu tapahtuma: `v2.core.account[configuration.recipient].capability_status_updated`, `events_from: ['@accounts']` (VÄLTTÄMÄTÖN — ilman tätä destination kuulisi vain platform-tilin omia tapahtumia, ei myyjien Connect-tilejä).
...
**⚠️ Kaksi virhettä löytyi ja korjattiin testauksessa, kumpikaan ei olisi paljastunut ilman oikeaa API-kutsua tuotantoa vasten:**
...
**Testattu tuotannossa testitilassa (kertakäyttöinen testimyyjä + testiskriptit, kaikki siivottu ajon jälkeen):**
- Luotu läpi oikean `POST /users/me/stripe-onboarding`-reitin uusi Connect-tili (`identity.entity_type:"individual"`, `country:"fi"`).
- **Allekirjoituksen tarkistus vahvistettu molempiin suuntiin oikeaa, julkista `https://habahub.com/api/webhooks/stripe-accounts`-osoitetta vasten** (ei paikallinen kutsu): oikein `stripe.webhooks.generateTestHeaderString()`:lla signeerattu synteettinen thin event → `200`; väärällä secretillä signeerattu → `400 "No signatures found matching..."`. Sama testausperiaate kuin aiemmin Paytrail-webhookilla ("oikein signeerattu synteettinen callback").
- **Negatiivinen polku vahvistettu oikeaa Stripeä vasten:** koska testitilin `stripe_transfers`-kapasiteetti oli aidosti yhä `restricted` (ks. alla oleva rajaus), käsittelijä kutsui `getAccountStatus()`:ia, sai `transfersEnabled:false`, eikä kirjoittanut mitään — `User.verified` pysyi `false`:na koko testin ajan, vahvistettu suoraan tietokannasta.
- **Kirjoituslogiikka (positiivinen polku) vahvistettu erikseen, eristettynä:** täsmälleen sama `prisma.user.updateMany({ where: { stripeAccountId, verified:false }, data:{verified:true} })` -kysely ajettu suoraan — ensimmäinen ajo `count:1` (asetti todeksi), toinen ajo `count:0` (idempotentti no-op), lopputila vahvistettu `verified:true`.

**⚠️ Rehellinen rajaus, ei vielä testattu — sama kategoria kuin Paytrailin pankkiläpivienti / Postin oikea lähetys / OBS-striimaus: täyttä positiivista polkua EI voitu ajaa aidosti läpi itse webhook-reitin kautta**, koska se vaatisi oikean Stripe-tilin jonka `stripe_transfers`-kapasiteetti on OIKEASTI `active` — ja tämän saavuttaminen vaatii Stripen hostatun onboarding-lomakkeen täyttämisen oikeassa selaimessa. **Uusi, aiemmin dokumentoimaton löydös:** koska tilit luodaan `dashboard:"express"`-tilassa (`configuration.recipient`, hostattu Account Link -onboarding), Stripe omistaa vaadittujen kenttien (nimi/osoite/syntymäaika/TOS-hyväksyntä) keräämisen kokonaan — suora API-kutsu (`PATCH /v2/core/accounts/:id`) näiden täyttämiseksi hylättiin selkeällä virheellä (*"You cannot accept the Terms of Service on behalf of accounts where requirement collection is owned by Stripe"* / *"You do not have permission to write to identity.individual.date_of_birth... onboarded via an Account Link"*) — tätä ei siis voi kiertää edes testitilassa API:lla, toisin kuin Stripen oma dokumentaatio antaa ymmärtää `dashboard:"none"`-tileille. Kaikki muu ketjussa (allekirjoitus, reititys, event-tyypin tunnistus, tuoreen tilan haku, idempotentti kirjoitus) on vahvistettu erikseen edellä kuvatulla tavalla — ainoa aidosti todentamaton lenkki on "täyttääkö oikea selaimen kautta tehty onboarding oikeasti juuri tämän event-tyypin", mikä on Stripen oman dokumentaation mukaan odotettu käytös (ei arvattu), muttei riippumattomasti vahvistettavissa ilman selainta.
