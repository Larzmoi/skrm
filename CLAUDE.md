...
**⬜ TEKEMÄTTÄ, tunnistettu mutta tarkoituksella myöhemmäksi jätetty (isompi työ, ei kuulunut ydinarkkitehtuuriin):** käyttöehdot/tietosuoja/FAQ/välityspalkkiot-sivut mainitsevat yhä "Paytrail" nimeltä käyttäjälle näkyvässä tekstissä — `frontend/app/kayttoehdot/content.ts` (§5.4-5.5, maksunkäsittelykulu-maininta, kolme kieltä), `frontend/app/tietosuoja/content.ts` (tietojenkäsittelijä-luettelo "Paytrail Oyj", kolme kieltä), `frontend/app/faq/page.tsx` ("Miten maksan ostokseni?" -vastaus, kolme kieltä — **huom: MobilePay/Google Pay/Apple Pay -maininta pitää vielä vahvistaa mitä Stripe Checkout oikeasti tukee Suomessa ennen kuin tekstiä kirjoitetaan uudestaan, ei toisteta vanhaa väitettä varmistamatta**), `frontend/lib/i18n/fi.ts` (`paymentFee: 'Maksunkäsittely (Paytrail)'`, näkyy `/valityspalkkiot`-sivulla). Myös muutama pelkkä koodikommentti (`backend/src/routes/cart.ts`, `backend/src/lib/auctionOrder.ts`, `backend/src/jobs/deliveryTimeline.ts`, `frontend/app/kori/page.tsx`) mainitsee yhä "Paytrail"-sanan arkkitehtuuriperiaatetta selittävässä kommentissa — ei toiminnallista vaikutusta, kosmeettinen siivous jos joskus tarvitaan.
